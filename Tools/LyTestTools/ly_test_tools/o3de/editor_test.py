...
def compile_test_case_name(request, test_spec):
    ...
    try:         
        ...
        logging.warning(f"Error reading test case name for TIAF. {EditorTestFrameworkException(e)}")
...
class EditorTestFrameworkException(Exception):
    """ Indicates that an Exception resulted from an Editor Test """

    def __init__(self, inner_exception = None):
        self.inner_exception = inner_exception

    def __str__(self):
        return f"This exception was generated from inside the EditorTest framework. The inner exception is {self.inner_exception}"
...
class EditorTestSuite:
                    ...
                    def single_run(self, request, workspace, editor, editor_test_data, launcher_platform):
                        ...
                        if is_single_test:
                            ...
                            if not isinstance(wrap, types.GeneratorType):
                                raise EditorTestFrameworkException("wrap_run must return a generator, did you forget 'yield'?")
                            next(wrap, None)
                            # Setup step
    ...
    def filter_session_shared_tests(session_items: list[_pytest.python.Function(EditorTest)],
        ...
        def will_run(item):
            try:
                ...
                warnings.warn(f"Test deselected from execution queue due to {EditorTestFrameworkException(ex)}")
                return False
    ...
    def _prepare_asset_processor(workspace: ly_test_tools._internal.managers.workspace.AbstractWorkspaceManager,
        ...
        try:
            ...
            raise EditorTestFrameworkException(ex)
        ...
        try:
            ...
            if return_code == 0:
                ...
                if not len(results) == len(test_spec_list):
                    raise EditorTestFrameworkException("bug in _get_results_using_output(), the number of results don't match the tests ran")
    ...
    def _run_parallel_tests(self, request: _pytest.fixtures.FixtureRequest,
        ...
        if not parallel_editors > 0:
            raise EditorTestFrameworkException("Must have at least one editor")
            ...
            for i in range(total_threads):
                def make_func(test_spec, index, my_editor):
                    def run(request, workspace, extra_cmdline_args):
                        ...
                        if results is None:
                            raise EditorTestFrameworkException("Results were none.")
