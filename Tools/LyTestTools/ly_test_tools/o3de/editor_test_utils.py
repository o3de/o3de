...
def compile_test_case_name_from_request(request):
    ...
    try:
        ...
        logging.warning(f"Error reading test case name in LyTestTools for TIAF. {e}")
    ...
    for filename in files_to_cycle:
        ...
        if os.path.exists(filepath):
            try:
                ...
                logger.warning(f"Couldn't cycle file {filepath}. Error: {str(exceptions.LyTestToolsFrameworkException(ex))}")
...
def retrieve_editor_log_content(run_id: int, log_name: str,
    ...
    try:
        ...
        editor_info = f"-- Error reading {log_name}: {str(exceptions.LyTestToolsFrameworkException(ex))} --"
    return editor_info
        ...
        for log_file in walk_tuple[2]:
            ...
            if _check_log_errors_warnings(full_log_path):
                try:
                    ...
                    logger.warning(f"Error in LyTestTools when saving log at path: {full_log_path}\n{exceptions.LyTestToolsFrameworkException(e)}")
