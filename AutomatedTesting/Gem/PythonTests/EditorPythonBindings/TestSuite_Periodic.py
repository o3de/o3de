...
#    class Editor_ComponentCommands_Works(EditorBatchedTest):
...
#ERROR. Failed to pre-enable component with name=Terrain World Debugger, uuid={D09BA0B9-FB51-446B-BD7B-3C40743D2E39}
    #class Editor_LevelComponentCommands_Works(EditorBatchedTest):
...
@pytest.mark.parametrize("launcher_platform", ['windows_editor'])
@pytest.mark.parametrize("project", ["AutomatedTesting"])
class TestAutomation_Utility(EditorTestSuite):
    ...
    class Editor_LevelCommands_Works(EditorBatchedTest):
...
# Exception: select_objects works ['Level', 'Atom Default Environment'] : FAILED
# Exception: delete_selected works : FAILED
