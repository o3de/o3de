...
def Editor_EntitySearchCommands_Works():
    # Description: 
    ...
    def CreateEntity(name, parentId):
        ...
        check_result(testName != name, "FAILURE: testName {} does not equal set name {}".format(testName, name))
        return newEntityId
    ...
    def SearchResultCheck(searchFilter, testName, resultId):
        ...
        if(len(entities) == 1) and (CompareEntityIds(entities[0], resultId)):
            ...
            check_result(False, "FAILURE: " + testName)
    ...
    def SearchResultsCheck(searchFilter, testName, resultSize):
        ...
        if(len(entities) == resultSize):
            ...
            check_result(False, "FAILURE: " + testName + " (size is " + 
                         str(len(entities)) + ", should be " + str(resultSize) + ")")
    ...
    if(len(rootEntities) == 1 + rootNum):
        ...
        check_result(False, "FAILURE: Root Entities")
    ...
    # all tests worked
    Report.result("Editor_EntitySearchCommands_Works ran", True)

if __name__ == "__main__":
    from editor_python_test_tools.utils import Report
    Report.start_test(Editor_EntitySearchCommands_Works)
