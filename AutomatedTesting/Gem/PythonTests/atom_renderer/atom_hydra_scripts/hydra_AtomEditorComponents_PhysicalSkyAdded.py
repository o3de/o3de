...
def AtomEditorComponents_PhysicalSky_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, len(hydra.find_entity_by_name(physical_sky)) == 0)

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, len(hydra.find_entity_by_name(physical_sky)) == 1)
        ...
        Report.result(Tests.entity_deleted, len(hydra.find_entity_by_name(physical_sky)) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, len(hydra.find_entity_by_name(physical_sky)) == 1)

        # 9. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, len(hydra.find_entity_by_name(physical_sky)) == 0)
