...
Hydra script that creates an entity and attaches the Physical Sky component to it for test verification.
"""
...
def AtomEditorComponents_PhysicalSky_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        has_component = hydra.has_components(physical_sky_entity.id, [physical_sky])
        Report.result(Tests.physical_sky_creation, physical_sky_entity.id.IsValid())
        Report.critical_result(Tests.physical_sky_component, has_component)

        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, physical_sky_entity.id.IsValid())

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, physical_sky_entity.id.isValid())
        ...
        # 7. Delete Physical Sky entity.
        editor.ToolsApplicationRequestBus(bus.Broadcast, "DeleteEntityById", physical_sky_entity.id)
        deleted_entity = hydra.find_entity_by_name(physical_sky)
        Report.result(Tests.entity_deleted, len(deleted_entity) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, physical_sky_entity.id.isValid())

        # 9. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, not physical_sky_entity.id.isValid())
