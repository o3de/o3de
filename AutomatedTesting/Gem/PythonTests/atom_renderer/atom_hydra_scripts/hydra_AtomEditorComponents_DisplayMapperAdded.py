...
def AtomEditorComponents_DisplayMapper_AddedToEntity():
    ...
    Test Steps:
    1) Creation of entity with Display Mapper component.
    2) UNDO the entity creation.
    3) REDO the entity creation.
    4) Enter/Exit game mode.
    5) Hide test.
    6) Visible test.
    7) Delete Display Mapper entity.
    8) UNDO deletion.
    9) REDO deletion.
    10) Look for errors.
    ...
    with Tracer() as error_tracer:
        ...
        display_mapper_entity = hydra.Entity(f"{display_mapper}")
        display_mapper_entity.create_entity(math.Vector3(512.0, 512.0, 34.0), [display_mapper])
        has_component = hydra.has_components(display_mapper_entity.id, [display_mapper])
        Report.result(Tests.display_mapper_creation, display_mapper_entity.id.IsValid())
        Report.critical_result(Tests.display_mapper_component, has_component)

        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, len(hydra.find_entity_by_name(display_mapper_entity)) == 0)

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, len(hydra.find_entity_by_name(display_mapper_entity)) == 1)
        ...
        Report.result(Tests.entity_deleted, len(hydra.find_entity_by_name(display_mapper_entity)) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, len(hydra.find_entity_by_name(display_mapper_entity)) == 1)

        # 9. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, len(hydra.find_entity_by_name(display_mapper_entity)) == 0)
