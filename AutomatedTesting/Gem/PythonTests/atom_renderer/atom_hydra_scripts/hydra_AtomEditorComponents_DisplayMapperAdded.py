...
Hydra script that creates an entity & attaches the Display Mapper component to it for test verification.
"""
...
def AtomEditorComponents_DisplayMapper_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, display_mapper_entity.id.IsValid())

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, display_mapper_entity.id.isValid())
        ...
        deleted_entity = hydra.find_entity_by_name(display_mapper)
        Report.result(Tests.entity_deleted, len(deleted_entity) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, display_mapper_entity.id.isValid())

        # 9. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, not display_mapper_entity.id.isValid())
