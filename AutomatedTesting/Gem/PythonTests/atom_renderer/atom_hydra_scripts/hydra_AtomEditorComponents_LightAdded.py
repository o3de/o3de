...
Hydra script that creates an entity & attaches the Light component to it for test verification.
"""
...
def AtomEditorComponents_Light_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, light_entity.id.IsValid())

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, light_entity.id.isValid())
        ...
        deleted_entity = hydra.find_entity_by_name(light)
        Report.result(Tests.entity_deleted, len(deleted_entity) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, light_entity.id.isValid())
