...
def AtomEditorComponents_Light_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # 1. Creation of entity with Light component.
        light = "Light"
        light_entity = hydra.Entity(f"{light}")
        light_entity.create_entity(math.Vector3(512.0, 512.0, 34.0), [light])
        has_component = hydra.has_components(light_entity.id, [light])
        Report.result(Tests.light_creation, light_entity.id.IsValid())
        Report.critical_result(Tests.light_component, has_component)

        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, len(hydra.find_entity_by_name(light_entity)) == 0)

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, len(hydra.find_entity_by_name(light_entity)) == 1)
        ...
        # 5. Hide test.
        editor.EditorEntityAPIBus(bus.Event, "SetVisibilityState", light_entity.id, False)
        is_hidden = editor.EditorEntityInfoRequestBus(bus.Event, 'IsHidden', light_entity.id)
        Report.result(Tests.is_hidden, is_hidden is True)

        # 6. Visible test.
        editor.EditorEntityAPIBus(bus.Event, "SetVisibilityState", light_entity.id, True)
        is_visible = editor.EditorEntityInfoRequestBus(bus.Event, 'IsVisible', light_entity.id)
        Report.result(Tests.is_visible, is_visible is True)

        # 7. Delete Light entity.
        editor.ToolsApplicationRequestBus(bus.Broadcast, "DeleteEntityById", light_entity.id)
        Report.result(Tests.entity_deleted, len(hydra.find_entity_by_name(light_entity)) == 0)

        # 8. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, len(hydra.find_entity_by_name(light_entity)) == 1)

        # 9. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, len(hydra.find_entity_by_name(light_entity)) == 0)
