...
Hydra script that creates an entity and attaches the Exposure Control component to it for test verification.
"""
...
def AtomEditorComponents_ExposureControl_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # 2. UNDO the entity creation.
        general.undo()
        Report.result(Tests.creation_undo, exposure_control_entity.id.IsValid())

        # 3. REDO the entity creation.
        general.redo()
        Report.result(Tests.creation_redo, exposure_control_entity.id.isValid())
        ...
        deleted_entity = hydra.find_entity_by_name(exposure_control)
        Report.result(Tests.entity_deleted, len(deleted_entity) == 0)

        # 9. UNDO deletion.
        general.undo()
        Report.result(Tests.deletion_undo, exposure_control_entity.id.isValid())

        # 10. REDO deletion.
        general.redo()
        Report.result(Tests.deletion_redo, not exposure_control_entity.id.isValid())
