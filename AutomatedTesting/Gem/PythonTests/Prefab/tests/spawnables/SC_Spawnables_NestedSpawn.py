...
def SC_Spawnables_NestedSpawn():
    ...
    def validate_entities_in_edit_mode(condition):
        ...
        search_filter.names = ["Nested_SC_Spawner"]
        ...
        assert len(nested_spawner_entity) == 0, f"Unexpectedly found Nested_SC_Spawner entity {condition}"
        assert len(flower_entity) == 0, f"Unexpectedly found PinkFlower entity {condition}"

    helper.init_idle()
    helper.open_level("Prefab", "SC_Spawnables_NestedSpawn")
    ...
    helper.wait_for_condition(lambda: EditorEntity(general.find_game_entity("Nested_SC_Spawner")) is not None, 3.0)
    nested_spawner_entity = EditorEntity(general.find_game_entity("Nested_SC_Spawner"))
    ...
    assert nested_spawner_entity, "Failed to find Nested_SC_Spawner entity at runtime"
...
if __name__ == "__main__":
    ...
    Report.start_test(SC_Spawnables_NestedSpawn)
