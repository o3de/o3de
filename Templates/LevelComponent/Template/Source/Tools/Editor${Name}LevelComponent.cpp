...
#include "Editor${SanitizedCppName}LevelComponent.h"
...
namespace ${GemName}
{
     ...
     */
    AZ_COMPONENT_IMPL(Editor${SanitizedCppName}LevelComponent, "Editor${SanitizedCppName}LevelComponent", "{${Random_Uuid}}");
     ...
     *   AppearsInAddComponentMenu(AZ_CRC_CE("Level")) makes it available on Level entities.
     *   Use ->DataElement() for each member variable you want editable in the Inspector.
     *
     * IMPORTANT: Because this editor component exists, the runtime ${SanitizedCppName}LevelComponent
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::Reflect(AZ::ReflectContext* context)
    {
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serializeContext->Class<Editor${SanitizedCppName}LevelComponent, AzToolsFramework::Components::EditorComponentBase>()
                ->Version(1)
                ;

            if (AZ::EditContext* editContext = serializeContext->GetEditContext())
            {
                editContext->Class<Editor${SanitizedCppName}LevelComponent>("${SanitizedCppName}LevelComponent", "[Description of functionality provided by this component]")
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::GetProvidedServices(AZ::ComponentDescriptor::DependencyArrayType& provided)
    {
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::GetIncompatibleServices([[maybe_unused]] AZ::ComponentDescriptor::DependencyArrayType& incompatible)
    {
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::GetRequiredServices([[maybe_unused]] AZ::ComponentDescriptor::DependencyArrayType& required)
    {
    }
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::GetDependentServices([[maybe_unused]] AZ::ComponentDescriptor::DependencyArrayType& dependent)
    {
    }
     ...
     * Use gameEntity->CreateComponent<${SanitizedCppName}LevelComponent>(...) and pass your
     ...
     */
    void Editor${SanitizedCppName}LevelComponent::BuildGameEntity(AZ::Entity* gameEntity)
    {
        gameEntity->CreateComponent<${SanitizedCppName}LevelComponent>();
    }
