...
macro(o3de_conditional_warning verbose message)
    if(verbose)
...
function(o3de_engine_compatible engine_path expected_engine_name version_op version_specifier out_result out_engine_version verbose)
    ...
    if(json_error)
        o3de_conditional_warning(verbose "Unable to read key 'engine_name' from '${engine_path}/engine.json'\nError: ${json_error}")
    elseif(NOT engine_name STREQUAL expected_engine_name)
        o3de_conditional_warning(verbose " Engine name '${engine_name}' found in '${engine_path}/engine.json' does not match expected engine name '${expected_engine_name}'")
    else()
        string(JSON engine_version ERROR_VARIABLE json_error GET ${engine_json} version)
        if(json_error)
            o3de_conditional_warning(verbose "Unable to verify engine compatibility because we could not read key 'version' from '${engine_path}/engine.json'\nError: ${json_error}")
        else()
            ...
            if(version_op AND version_specifier)
                ...
                if(NOT is_compatible)
                    o3de_conditional_warning(verbose "The engine ${engine_name} version ${engine_version} at ${engine_path} is not compatible with the project's version specifier '${version_op}${version_specifier}'\n Please use the o3de CLI to register the project to a compatible engine or update the project or engine.")
                endif()
            else()
