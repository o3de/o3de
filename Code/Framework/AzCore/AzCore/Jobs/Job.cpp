...
AZ::Job::Job(bool isAutoDelete, AZ::JobContext* context, bool isCompletion, AZ::s8 priority)
{
    if (context)
    {
        m_context = context;
...
void AZ::Job::Reset(bool isClearDependent)
{
#ifdef AZ_DEBUG_JOB_STATE
...
void AZ::Job::SetDependentChild(Job* dependent)
{
    AZ_Assert(!GetDependent(), ("Job already has a dependent, should be cleared after the job is done"));
#ifdef AZ_DEBUG_JOB_STATE
    AZ_Assert(m_state == STATE_SETUP, ("Dependent can only be set before the jobs are started"));
...
bool AZ::Job::IsCompletion() const
{
...
void AZ::Job::IncrementDependentCount()
{
...
void AZ::Job::IncrementDependentCountAndSetChildFlag()
{
    AZ_Assert(GetDependentCount() < FLAG_DEPENDENTCOUNT_MASK, "Dependent count overflow");
#ifdef AZCORE_JOBS_IMPL_SYNCHRONOUS
...
void AZ::Job::DecrementDependentCount()
{
#ifdef AZ_DEBUG_JOB_STATE
...
AZ::s8 AZ::Job::GetPriority() const
{
    return (GetDependentCountAndFlags() >> FLAG_PRIORITY_START_BIT) & 0xff;
}

#ifdef AZCORE_JOBS_IMPL_SYNCHRONOUS
void AZ::Job::StoreDependent(Job* job)
{
    m_dependent = job;
}

AZ::Job* AZ::Job::GetDependent() const
{
    return m_dependent;
}

void AZ::Job::SetDependentCountAndFlags(unsigned int countAndFlags)
{
    m_dependentCountAndFlags = countAndFlags;
}

unsigned int AZ::Job::GetDependentCountAndFlags() const
{
