...
namespace AZ::IO
{
    ...
    void StorageDrive::ReadFile(FileRequest* request)
    {
        ...
        if (!file)
        {
            ...
            bool isOpen = newFile->Open(data->m_path.GetAbsolutePathString(), SystemFile::OpenMode::SF_OPEN_READ_ONLY);
            if (!isOpen)
            {
    ...
    void StorageDrive::FileExistsRequest(FileRequest* request)
    {
        ...
        {
            fileExists.m_found = SystemFile::Exists(fileExists.m_path.GetAbsolutePathString());
        }
        m_context->MarkRequestAsCompleted(request);
    }
    ...
    void StorageDrive::FileMetaDataRetrievalRequest(FileRequest* request)
    {
        ...
        if (cacheIndex != s_fileNotFound)
        {
            AZ_Assert(m_fileHandles[cacheIndex],
                "File path '%s' doesn't have an associated file handle.", m_filePaths[cacheIndex].GetRelativePathString());
        ...
        else
        {
            ...
            u64 size = SystemFile::Length(command.m_path.GetAbsolutePathString());
