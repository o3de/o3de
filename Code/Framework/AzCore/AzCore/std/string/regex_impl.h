  ...
  #define AZ_REGEX_VALT(iter)   typename iterator_traits<iter>::value_type
    ...
    public:
        ...
        {   // map [first, last) to character class mask value
            int index = 0;
            for (; GetNames()[index].m_element; ++index)
            {
                if (IsSame(first, last,
                        GetNames()[index].m_element, GetNames()[index].m_element + GetNames()[index].m_length,
    ...
    private:
        static const ClassNames<Element>* GetNames();
    };
    ...
    template<>
    inline const ClassNames<char>* RegexTraits<char>::GetNames()
    {
        static const ClassNames<char> names[] = {
    ...
    template<>
    inline const ClassNames<wchar_t>* RegexTraits<wchar_t>::GetNames()
    {
        static const ClassNames<wchar_t> names[] = {
            // map class names to numeric constants
