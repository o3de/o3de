...
namespace UnitTest
{
    ...
    TEST_F(ArchiveTestFixture, TestArchiveFGetCachedFileData_PakFile)
    {
...

        {
            ...
            EXPECT_TRUE(archive->OpenPack("@projectproductassets@", testArchivePath_withSubfolders.c_str()));
    ...
    TEST_F(ArchiveTestFixture, TestArchivePackImmediateReading)
    {
        ...
        // * all of the above even though the mount point for the archive is @projectproductassets@ wheras the physical pack lives in @usercache@
        ...
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@", testArchivePath_withSubfolders));
        // ---- BARRAGE OF TESTS
    ...
    TEST_F(ArchiveTestFixture, FilesInArchive_AreSearchable)
    {
        ...
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@\\uniquename\\mylevel2", testArchivePath_withMountPoint));
        ...
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@\\uniquename\\mylevel2\\mylevel3\\mylevel4", testArchivePath_withMountPoint));
        ...
        // for example, if a file is mounted at "@projectproductassets@\\uniquename\\mylevel2\\mylevel3\\mylevel4"
        // and the file "@projectproductassets@\\somethingelse" is requested it should not be found
        // in addition if the file "@projectproductassets@\\uniquename\\mylevel3" is requested it should not be found
    ...
    TEST_F(ArchiveTestFixture, TestArchiveViaFileIO)
    {
        ...
        cpfio.CreatePath("@projectproductassets@");
        ...
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@", genericArchiveFileName));

        // ---- BARRAGE OF TESTS
        EXPECT_TRUE(cpfio.Exists("testfile.xml"));
        EXPECT_TRUE(cpfio.Exists("@projectproductassets@/testfile.xml"));  // this should be hte same file
        ...
        EXPECT_TRUE(cpfio.IsDirectory("@projectproductassets@"));
        EXPECT_TRUE(cpfio.IsReadOnly("testfile.xml"));
        EXPECT_TRUE(cpfio.IsReadOnly("@projectproductassets@/testfile.xml"));
        ...
        EXPECT_TRUE(cpfio.ResolvePath(resolvedTestFilePath, AZ::IO::PathView("@projectproductassets@/testfile.xml")));
        bool foundIt = false;
        // note that this file exists only in the archive.
        cpfio.FindFiles("@projectproductassets@", "*.xml", [&foundIt, &cpfio, &resolvedTestFilePath](const char* foundName)
            {
        ...
        // EXPECT_NE(ResultCode::Success, cpfio.Remove("@projectproductassets@/testfile.xml")); // may not delete archive files

        // make sure it works with and without alias:
        EXPECT_TRUE(cpfio.Exists("@projectproductassets@/testfile.xml"));
        EXPECT_TRUE(cpfio.Exists("testfile.xml"));
    ...
    TEST_F(ArchiveTestFixture, TestArchiveFolderAliases)
    {
        ...
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@", realNameBuf));
        EXPECT_TRUE(archive->IsFileExist("@projectproductassets@/foundit.dat"));
        EXPECT_FALSE(archive->IsFileExist("@usercache@/foundit.dat")); // do not find it in the previous location!
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/foundit.dat", AZ::IO::IArchive::eFileLocation_OnDisk));
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/notfoundit.dat"));
        EXPECT_TRUE(archive->ClosePack(realNameBuf));

        // try sub-folders
        EXPECT_TRUE(archive->OpenPack("@projectproductassets@/mystuff", realNameBuf));
        EXPECT_TRUE(archive->IsFileExist("@projectproductassets@/mystuff/foundit.dat"));
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/foundit.dat")); // do not find it in the previous locations!
        EXPECT_FALSE(archive->IsFileExist("@usercache@/foundit.dat")); // do not find it in the previous locations!
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/foundit.dat", AZ::IO::IArchive::eFileLocation_OnDisk));
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/mystuff/foundit.dat", AZ::IO::IArchive::eFileLocation_OnDisk));
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/notfoundit.dat")); // non-existent file
        EXPECT_FALSE(archive->IsFileExist("@projectproductassets@/mystuff/notfoundit.dat")); // non-existent file
        EXPECT_TRUE(archive->ClosePack(realNameBuf));
    }
    ...
    TEST_F(ArchiveTestFixture, IResourceList_Add_AbsolutePath_RemovesAndReplacesWithAlias)
    {
        ...
        const char* assetsPath = ioBase->GetAlias("@projectproductassets@");
        ...
        EXPECT_TRUE(ioBase->ReplaceAlias(resolvedAddedPath, "@projectproductassets@/textures/test.dds"));
