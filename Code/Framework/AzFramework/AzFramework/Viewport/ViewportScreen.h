...
namespace AzFramework
{
    ...
    AZF_API AZ::Vector3 WorldToScreenNdc(const AZ::Vector3& worldPosition, const AZ::Matrix3x4& cameraView, const AZ::Matrix4x4& cameraProjection);

    //! Projects a position in world space to screen space for the given camera.
    AZF_API ScreenPoint WorldToScreen(const AZ::Vector3& worldPosition, const CameraState& cameraState);
    ...
    AZF_API ScreenPoint WorldToScreen(
    ...
    AZF_API AZ::Vector3 ScreenToWorld(const ScreenPoint& screenPosition, const CameraState& cameraState);

    //! Overload of ScreenToWorld that accepts camera values that can be precomputed if this function
    //! is called many times in a loop.
    AZF_API AZ::Vector3 ScreenToWorld(
    ...
    //! Note: The position returned will be on the near clip plane of the camera in world space.
    AZF_API AZ::Vector3 ScreenNdcToWorld(
    ...
    AZF_API AZ::Matrix4x4 CameraProjection(const CameraState& cameraState);

    //! Returns the inverse of the camera projection for the current camera state.
    AZF_API AZ::Matrix4x4 InverseCameraProjection(const CameraState& cameraState);
    ...
    AZF_API AZ::Matrix3x4 CameraView(const CameraState& cameraState);
    ...
    AZF_API AZ::Matrix3x4 InverseCameraView(const CameraState& cameraState);
    ...
    AZF_API AZ::Matrix3x4 CameraTransform(const CameraState& cameraState);
    ...
    AZF_API AZ::Matrix3x4 CameraTransformFromCameraView(const AZ::Matrix3x4& cameraView);
    ...
    AZF_API AZ::Matrix3x4 CameraViewFromCameraTransform(const AZ::Matrix3x4& cameraTransform);

    //! Returns a frustum representing the camera transform and view volume in world space.
    AZF_API AZ::Frustum FrustumFromCameraState(const CameraState& cameraState);

    //! Returns a structure representing frustum attributes for the current camera state.
    AZF_API AZ::ViewFrustumAttributes ViewFrustumAttributesFromCameraState(const CameraState& cameraState);
