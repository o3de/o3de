...
namespace AzPhysics
{
        ...
        public SceneQueryRequest
    {
        ...
        SceneQuery::HitFlags m_hitFlags = SceneQuery::HitFlags::Default | SceneQuery::HitFlags::MTD; //!< Query behavior flags. MTD Is On by default to correctly report objects that are initial in contact with the shape.
        SceneQuery::FilterCallback m_filterCallback = nullptr; //!< Hit filtering function
        bool m_reportMultipleHits = false; //!< flag to have the cast stop after the first hit or return all hits along the query.
    };
