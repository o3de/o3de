...
{
    static const QString g_clearButtonActionNameC = QStringLiteral("_q_qlineeditclearaction");
    ...
    bool BrowseEdit::polish(Style* style, QWidget* widget, const Config& config, const LineEdit::Config& lineEditConfig)
    {
        ...
        if (browseEdit)
        {
            ...
            QAction* action = lineEdit->findChild<QAction*>(g_clearButtonActionNameC);
            if (action)
            {
