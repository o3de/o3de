...
namespace AzToolsFramework
{
    namespace Prefab
    {
        ...
        bool InstanceDomGenerator::GenerateInstanceDom(PrefabDom& instanceDom, const Instance& instance) const
        {
            ...
            InstanceOptionalConstReference focusedOrRootInstance = climbUpResult.reachedInstance;
            if (!focusedOrRootInstance.has_value())
            {
                AZ_Assert(false, "Prefab - InstanceDomGenerator::GenerateInstanceDom - "
            ...
            AZStd::string relativePathFromTop = PrefabInstanceUtils::GetRelativePath(climbUpResult.climbedInstances);
            ...
            focusedOrRootTemplateDom.CopyFrom(
                m_prefabSystemComponentInterface->FindTemplateDom(focusedOrRootInstance->get().GetTemplateId()),
            ...
            if (PrefabInstanceUtils::CompareInstances(instance, focusedInstance))
            {
                UpdateContainerEntityInDomFromRoot(instanceDom, focusedInstance->get());
            }
            // If the focused instance is a proper descendant of the given instance, then update the corresponding
            ...
            else if (PrefabInstanceUtils::IsProperlyHierarchical(focusedInstance->get(), instance))
            {
                PrefabDom focusedTemplateDom;
                focusedTemplateDom.CopyFrom(
        ...
        void InstanceDomGenerator::UpdateContainerEntityInDomFromRoot(PrefabDom& instanceDom, const Instance& instance) const
        {
            ...
            InstanceOptionalConstReference rootInstance = climbUpResult.reachedInstance;
            AZStd::string relativePathFromRoot = PrefabInstanceUtils::GetRelativePath(climbUpResult.climbedInstances);

            // No need to update the instance DOM if the given instance is the root instance.
            if (PrefabInstanceUtils::CompareInstances(instance, *rootInstance))
            {
                return;
            }
            ...
            const PrefabDom& rootTemplateDom = m_prefabSystemComponentInterface->FindTemplateDom(rootInstance->get().GetTemplateId());
