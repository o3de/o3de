...
namespace AzToolsFramework
{
    ...
    bool IsDocumentPathPreviewable(const AZStd::string& path)
    {
        ...
        for (const auto& [storedPath, flag] :
             AzToolsFramework::GetSettingsObject<AZStd::unordered_map<AZStd::string, bool>>("/O3DE/Atom/Tools/PreviewablePathSettings"))
        {
            if (pathWithoutAlias == GetPathWithoutAlias(storedPath))
            {
                return flag;
    ...
    bool SaveSettingsToFile(const AZ::IO::FixedMaxPath& savePath, const AZStd::vector<AZStd::string>& filters)
    {
        ...
        if (registry == nullptr)
        {
            AZ_Warning("AzToolsFramework", false, "Unable to access global settings registry.");
            return false;
        }
        ...
        {
            AZ_Warning("AzToolsFramework", false, R"(Unable to save changes to the registry file at "%s"\n)", savePath.c_str());
            return false;
        }
        ...
        AZ_Warning("AzToolsFramework", saved, R"(Unable to save registry file to path "%s"\n)", savePath.c_str());
        return saved;
    }
    ...
    AZStd::set<AZStd::string> GetPathsFromMimeData(const QMimeData* mimeData)
    {
        ...
        if (mimeData->hasFormat(AzToolsFramework::EditorAssetMimeDataContainer::GetMimeType()))
        {
            ...
            if (container.FromMimeData(mimeData))
            {
                ...
                {
                    AZStd::string path = GetSourcePathByAssetId(asset.m_assetId);
