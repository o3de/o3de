...
namespace AzToolsFramework
{
    namespace AssetSystem
    {
        ...
        public:
            ...
            virtual bool GetAbsoluteAssetDatabaseLocation(AZStd::string& /*result*/)
            {
                return false;
            }
            ...
            virtual bool GetRelativeProductPathFromFullSourceOrProductPath(
                const AZStd::string& fullPath, AZStd::string& relativeProductPath) = 0;
            ...
            //! retrieve an Az::Data::AssetInfo class for the given assetId.  this may map to source too in which case rootFilePath will be
            //! non-empty.
            virtual bool GetAssetInfoById(
                const AZ::Data::AssetId& assetId,
                const AZ::Data::AssetType& assetType,
                const AZStd::string& platformName,
                AZ::Data::AssetInfo& assetInfo,
                AZStd::string& rootFilePath) = 0;
             ...
             * @param assetInfo is a /ref AZ::Data::AssetInfo filled out with details about the asset including its relative path to its
             * watch folder note that inside assetInfo is a AssetId, but only the UUID-part will ever have a value since we are dealing with
             * a source file (no subid)
             ...
             * a source file (no subid)
             * @param watchFolder is the scan folder that it was found inside (the path in the assetInfo is relative to this folder).
             *           If you Path::Join the watchFolder and the assetInfo relative path, you get the full path to the source file on
             * physical media returns false if it cannot find the source, true otherwise.
             */
            virtual bool GetSourceInfoBySourceUUID(
                const AZ::Uuid& sourceUuid, AZ::Data::AssetInfo& assetInfo, AZStd::string& watchFolder) = 0;
             ...
             * returns false if it cannot find the source, true otherwise.
             */
            virtual bool GetAssetsProducedBySourceUUID(
                const AZ::Uuid& sourceUuid, AZStd::vector<AZ::Data::AssetInfo>& productsAssetInfo) = 0;
        ...
        //! Please note that these job status are written to the database, so it is very important that any new status should be added at
        //! the end else the database might get corrupted
        enum class JobStatus
        {
            Any = -1, // used exclusively by the database to indicate any state query, no job should ever actually be in this state, we
                      // start in Queued and progress from there
        ...
        struct AZTF_API JobInfo
        {
            ...
            //! the file from which this job was originally spawned.  Is just the relative source file name ("whatever/something.tif", not
            //! an absolute path)
            ...
            //! Job Key is arbitrarily defined by the builder.  Used to differentiate between different jobs emitted for the same input
            //! file, for the same platform, for the same builder. for example, you might want to split a particularly complicated and time
            //! consuming job into multiple sub-jobs.  In which case they'd all have the same input file, the same platform, the same
            //! builder UUID (since its the UUID of the builder itself) but would have different job keys.
        ...
        public:
            ...
            virtual AZ::Outcome<JobInfoContainer> GetAssetJobsInfoByAssetID(
                const AZ::Data::AssetId& assetId, const bool escalateJobs, bool requireFencing) = 0;
