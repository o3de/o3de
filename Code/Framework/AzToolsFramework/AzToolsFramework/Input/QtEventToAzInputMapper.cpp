...
namespace AzToolsFramework
{
    ...
    void QtEventToAzInputMapper::SetCursorMode(AzToolsFramework::CursorInputMode mode)
    {
        if (mode != m_cursorMode)
        {
            ...
            if (m_cursorMode != CursorInputMode::CursorModeCaptured && m_nativeCursorCapture)
            {
                m_nativeCursorCapture->End();
            }
        }
    }
    ...
    bool QtEventToAzInputMapper::eventFilter(QObject* object, QEvent* event)
    {
        ...
        // Hover mouse moves (no button held) do not always reach the source widget. On macOS Qt never considers a native
        // child window that is not painted by Qt (the viewport is rendered by Atom) to be 'under the mouse', so it delivers
        // hover moves to the top level window instead - only presses and drags reach the viewport through hit testing.
        // Without this the cursor position (used for selection, picking and pivots) would only update while a button is held.
        if (eventType == QEvent::Type::MouseMove && object != m_sourceWidget && qobject_cast<QWidget*>(object) != nullptr)
        {
            const auto* mouseEvent = static_cast<const QMouseEvent*>(event);
            const QPoint globalCursorPosition = mouseEvent->globalPosition().toPoint();
            if (mouseEvent->buttons() == Qt::NoButton && QApplication::widgetAt(globalCursorPosition) == m_sourceWidget)
            {
    ...
    void QtEventToAzInputMapper::HandleNativeMotionDelta(const QPoint& delta)
    {
        ...
        m_mouseDevice->m_cursorPositionData2D->m_normalizedPositionDelta = WidgetPositionToNormalizedPosition(delta);
        ProcessPendingMouseEvents(delta);
    }
