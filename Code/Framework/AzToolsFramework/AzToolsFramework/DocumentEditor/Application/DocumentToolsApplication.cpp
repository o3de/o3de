...
{
    DocumentToolsApplication* DocumentToolsApplication::m_instance = {};

    DocumentToolsApplication::DocumentToolsApplication(const char* targetName, int* argc, char*** argv)
        ...
        , m_toolId(targetName)
    {
        ...
        ToolsMainWindowNotificationBus::Handler::BusConnect(m_toolId);
    }

    DocumentToolsApplication ::~DocumentToolsApplication()
    {
        m_instance = {};
        m_styleManager.reset();
        ToolsMainWindowNotificationBus::Handler::BusDisconnect();
        AzToolsFramework::AssetDatabase::AssetDatabaseRequestsBus::Handler::BusDisconnect();
        AzToolsFramework::EditorPythonConsoleNotificationBus::Handler::BusDisconnect();
    }

    void DocumentToolsApplication::CreateReflectionManager()
    {
        Base::CreateReflectionManager();
        GetSerializeContext()->CreateEditContext();
    }

    const char* DocumentToolsApplication::GetCurrentConfigurationName() const
    {
#if defined(_RELEASE)
        return "ReleaseTools";
#elif defined(_DEBUG)
    ...
    void DocumentToolsApplication::Reflect(AZ::ReflectContext* context)
    {
        ...
        {
            // This will put these methods into the 'azlmbr.tools.general' module
            auto addGeneral = [](AZ::BehaviorContext::GlobalMethodBuilder methodBuilder)
            {
                methodBuilder->Attribute(AZ::Script::Attributes::Scope, AZ::Script::Attributes::ScopeFlags::Automation)
                    ->Attribute(AZ::Script::Attributes::Category, "Editor")
                    ->Attribute(AZ::Script::Attributes::Module, "tools.general");
            };

            addGeneral(behaviorContext->Method(
                "idle_wait_frames", &DocumentToolsApplication::PyIdleWaitFrames, nullptr,
                "Waits idling for a frames. Primarily used for auto-testing."));
            addGeneral(behaviorContext->Method(
                "exit", &DocumentToolsApplication::PyExit, nullptr,
                "Exit application. Primarily used for auto-testing."));
            addGeneral(behaviorContext->Method(
                "crash", &DocumentToolsApplication::PyCrash, nullptr,
                "Crashes the application, useful for testing crash reporting and other automation tools."));
            addGeneral(behaviorContext->Method(
                "test_output", &DocumentToolsApplication::PyTestOutput, nullptr,
                "Report test information."));
        }
    }

    void DocumentToolsApplication::RegisterCoreComponents()
    {
    ...
    AZ::ComponentTypeList DocumentToolsApplication::GetRequiredSystemComponents() const
    {
    ...
    void DocumentToolsApplication::CreateStaticModules(AZStd::vector<AZ::Module*>& outModules)
    {
        Base::CreateStaticModules(outModules);
        outModules.push_back(aznew AzToolsFramework::AzToolsFrameworkModule);
    }

    void DocumentToolsApplication::StartCommon(AZ::Entity* systemEntity)
    {
        ...
        const bool clearLogFile = GetSettingsValue("/O3DE/AzToolsFramework/Application/ClearLogOnStart", false);
        ...
        const bool enableSourceControl = GetSettingsValue("/O3DE/AzToolsFramework/Application/EnableSourceControl", false);
       ...
       // m_assetBrowserInteractions.reset(aznew AzToolsFramework::ToolsAssetBrowserInteractions);
    ...
    void DocumentToolsApplication::Destroy()
    {
       // m_assetBrowserInteractions.reset();
        ...
        const AZStd::vector<AZStd::string> filters = {
            "/O3DE/AzToolsFramework", "/O3DE//Tools", AZStd::string::format("/O3DE//%s", m_targetName.c_str()) }; 
...
#if AZ_TRAIT_AzToolsFramework_SKIP_APP_DESTROY
        ::_exit(0);
#else
        Base::Destroy();
#endif
    ...
    void DocumentToolsApplication::RunMainLoop()
    {
    ...
    void DocumentToolsApplication::OnIdle()
    {
        ...
        if (!WasExitMainLoopRequested())
        {
            ...
            const int updateInterval = (applicationState() & Qt::ApplicationActive)
                ? aznumeric_cast<int>(GetSettingsValue<AZ::u64>("/O3DE/AzToolsFramework/Application/UpdateIntervalWhenActive", 1))
                : aznumeric_cast<int>(GetSettingsValue<AZ::u64>("/O3DE/AzToolsFramework/Application/UpdateIntervalWhenNotActive", 250));
            QTimer::singleShot(updateInterval, this, &DocumentToolsApplication::OnIdle);
            return;
        }
    ...
    void DocumentToolsApplication::OnMainWindowClosing()
    {
        AzFramework::ApplicationRequests::Bus::Broadcast(&AzFramework::ApplicationRequests::ExitMainLoop);
    }

    AZStd::vector<AZStd::string> DocumentToolsApplication::GetCriticalAssetFilters() const
    {
        return AZStd::vector<AZStd::string>({});
    }

    void DocumentToolsApplication::ConnectToAssetProcessor()
    {
    ...
    void DocumentToolsApplication::CompileCriticalAssets()
    {
        ...
        if (!failedAssets.empty())
        {
            QMessageBox::critical(
                ...
                .arg("Make sure this is an  project."));
            AzFramework::ApplicationRequests::Bus::Broadcast(&AzFramework::ApplicationRequests::ExitMainLoop);
        }
    ...
    void DocumentToolsApplication::SaveSettings()
    {
    ...
    void DocumentToolsApplication::LoadSettings()
    {
        AZ::SerializeContext* context = nullptr;
        AZ::ComponentApplicationBus::BroadcastResult(context, &AZ::ComponentApplicationRequests::GetSerializeContext);
        AZ_Assert(context, "No serialize context");
    ...
    void DocumentToolsApplication::UnloadSettings()
    {
        if (m_activatedLocalUserSettings)
        {
            SaveSettings();
    ...
    void DocumentToolsApplication::ProcessCommandLine(const AZ::CommandLine& commandLine)
    {
        if (commandLine.HasSwitch("activatewindow"))
        {
            ToolsMainWindowRequestBus::Event(m_toolId, &ToolsMainWindowRequestBus::Handler::ActivateWindow);
        }
    ...
    void DocumentToolsApplication::PrintAlways(const AZStd::string& output)
    {
        m_stdoutRedirection.WriteBypassingRedirect(output.c_str(), static_cast<unsigned int>(output.size()));
    }

    void DocumentToolsApplication::RedirectStdoutToNull()
    {
        m_stdoutRedirection.RedirectTo(AZ::IO::SystemFile::GetNullFilename());
    }

    bool DocumentToolsApplication::LaunchLocalServer()
    {
#if !DISABLED_FOR_NOW
        return false;
#else
        ...
        const bool allowMultipleInstances = GetSettingsValue("/O3DE/AzToolsFramework/Application/AllowMultipleInstances", false);
    ...
    bool DocumentToolsApplication::GetAssetDatabaseLocation(AZStd::string& result)
    {
    ...
    void DocumentToolsApplication::QueryApplicationType(AZ::ApplicationTypeQuery& appType) const
    {
        appType.m_maskValue = AZ::ApplicationTypeQuery::Masks::Tool;
    }

    void DocumentToolsApplication::OnTraceMessage([[maybe_unused]] AZStd::string_view message)
    {
    ...
    void DocumentToolsApplication::OnErrorMessage(AZStd::string_view message)
    {
        // Use AZ_TracePrintf instead of AZ_Error or AZ_Warning to avoid all the metadata noise
        OnTraceMessage(message);
    }

    void DocumentToolsApplication::OnExceptionMessage([[maybe_unused]] AZStd::string_view message)
    {
        AZ_Error(m_targetName.c_str(), false, "Python: " AZ_STRING_FORMAT, AZ_STRING_ARG(message));
    }

    void DocumentToolsApplication::PyIdleWaitFrames(uint32_t frames)
    {
        ...
        QObject::connect(&timer, &QTimer::timeout, &loop, [&]() {
            auto app = DocumentToolsApplication::GetInstance();
    ...
    void DocumentToolsApplication::PyExit()
    {
        AzFramework::ApplicationRequests::Bus::Broadcast(&AzFramework::ApplicationRequests::ExitMainLoop);
    }

    void DocumentToolsApplication::PyCrash()
    {
        AZ_Crash();
    }

    void DocumentToolsApplication::PyTestOutput(const AZStd::string& output)
    {
        DocumentToolsApplication::GetInstance()->PrintAlways(output);
    }

    DocumentToolsApplication* DocumentToolsApplication::GetInstance()
    {
        return m_instance;
    }
} // namespace AzToolsFramework
