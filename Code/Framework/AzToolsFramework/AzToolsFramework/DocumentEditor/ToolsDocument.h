...
{
    //! ToolsDocument is intended to be used as a base class for a tools' concrete document types.
    //! It implements most of the basic functionality expected by ToolsDocumentRequestBus, validating file paths, file types, providing
    ...
    class ToolsDocument
        : public ToolsDocumentRequestBus::Handler
        , private AzToolsFramework::AssetSystemBus::Handler
    {
    public:
        AZ_RTTI(ToolsDocument, "{B8E524AA-7391-499D-BFAA-5A16E5E92FD1}");
        AZ_CLASS_ALLOCATOR(ToolsDocument, AZ::SystemAllocator, 0);
        AZ_DISABLE_COPY_MOVE(ToolsDocument);

        static void Reflect(AZ::ReflectContext* context);

        ToolsDocument() = default;
        ToolsDocument(const AZ::Crc32& toolId, const DocumentTypeInfo& documentTypeInfo);
        virtual ~ToolsDocument();

        // ToolsDocumentRequestBus::Handler overrides...
