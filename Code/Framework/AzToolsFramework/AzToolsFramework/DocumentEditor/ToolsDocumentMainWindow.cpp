...
#include <AzToolsFramework/DocumentEditor/ToolsDocumentMainWindow.h>
...
{
    ToolsDocumentMainWindow::ToolsDocumentMainWindow(const AZ::Crc32& toolId, const QString& objectName, QWidget* parent)
    ...
    ToolsDocumentMainWindow::~ToolsDocumentMainWindow()
    {
        ToolsDocumentNotificationBus::Handler::BusDisconnect();
    }

    void ToolsDocumentMainWindow::CreateMenus(QMenuBar* menuBar)
    {
        ...
        m_actionSaveAsCopy = CreateActionAtPosition(m_menuFile, insertPostion, "Save &As...", [this]() {
            ...
            if (const auto& savePath = GetSaveDocumentParams(documentPath.toUtf8().constData(), documentId); !savePath.empty())
            {
                bool result = false;
                ToolsDocumentSystemRequestBus::EventResult(
                    result, m_toolId, &ToolsDocumentSystemRequestBus::Events::SaveDocumentAsCopy, documentId, savePath);
                if (!result)
                {
        ...
        m_actionSaveAsChild = CreateActionAtPosition(m_menuFile, insertPostion, "Save As &Child...", [this]() {
            ...
            if (const auto& savePath = GetSaveDocumentParams(documentPath.toUtf8().constData(), documentId); !savePath.empty())
            {
                bool result = false;
                ToolsDocumentSystemRequestBus::EventResult(
                    result, m_toolId, &ToolsDocumentSystemRequestBus::Events::SaveDocumentAsChild, documentId, savePath);
                if (!result)
    {
                    SetStatusError(tr("Document save failed: %1").arg(documentPath).toUtf8().constData());
                }
                ...
                if (!SaveDocument(documentId))
    {
        ...
        m_actionUndo = CreateActionAtPosition(m_menuEdit, insertPostion, "&Undo", [this]() {
            const AZ::Uuid documentId = GetCurrentDocumentId();
            bool result = false;
            ToolsDocumentRequestBus::EventResult(result, documentId, &ToolsDocumentRequestBus::Events::Undo);
            if (!result)
            {
        ...
        m_actionRedo = CreateActionAtPosition(m_menuEdit, insertPostion, "&Redo", [this]() {
            const AZ::Uuid documentId = GetCurrentDocumentId();
            bool result = false;
            ToolsDocumentRequestBus::EventResult(result, documentId, &ToolsDocumentRequestBus::Events::Redo);
            if (!result)
            {
    ...
    bool ToolsDocumentMainWindow::SaveDocument(const AZ::Uuid& documentId)
    {
        ...
        if (documentInfo.IsSupportedExtensionToSave(documentPath))
        {
            bool result = false;
            ToolsDocumentSystemRequestBus::EventResult(
                result, m_toolId, &ToolsDocumentSystemRequestBus::Events::SaveDocument, documentId);
            if (!result)
            {
    ...
    bool ToolsDocumentMainWindow::CloseDocumentCheck(const AZ::Uuid& documentId)
    {
        AZStd::string documentPath;
        ToolsDocumentRequestBus::EventResult(documentPath, documentId, &ToolsDocumentRequestBus::Events::GetAbsolutePath);
    ...
    {
                AZ_TracePrintf("ToolsDocument", "Close document canceled: %s\n", documentPath.c_str());
                return false;
            }
            if (selection == QMessageBox::Yes)
        {
                if (!SaveDocument(documentId))
            {
                    ...
                    AZ_Error("ToolsDocumentMainWindow", false, "%s: %s", title.toUtf8().constData(), text.toUtf8().constData());
    ...
    bool ToolsDocumentMainWindow::CloseDocuments(const AZStd::vector<AZ::Uuid>& documentIds)
    {
        for (const auto& documentId : documentIds)
        {
            ...
            bool result = false;
            ToolsDocumentSystemRequestBus::EventResult(result, m_toolId, &ToolsDocumentSystemRequestBus::Events::CloseDocument, documentId);
    ...
    const AZStd::vector<AZ::Uuid> ToolsDocumentMainWindow::GetOpenDocumentIds() const
    {
    ...
    void ToolsDocumentMainWindow::UpdateMenus(QMenuBar* menuBar)
    {
        ...
        bool isOpen = false;
        ToolsDocumentRequestBus::EventResult(isOpen, documentId, &ToolsDocumentRequestBus::Events::IsOpen);
        bool canSaveAsChild = false;
        ToolsDocumentRequestBus::EventResult(canSaveAsChild, documentId, &ToolsDocumentRequestBus::Events::CanSaveAsChild);
        bool canUndo = false;
        ToolsDocumentRequestBus::EventResult(canUndo, documentId, &ToolsDocumentRequestBus::Events::CanUndo);
        bool canRedo = false;
        ToolsDocumentRequestBus::EventResult(canRedo, documentId, &ToolsDocumentRequestBus::Events::CanRedo);
    ...
    void ToolsDocumentMainWindow::PopulateSettingsInspector(InspectorWidget* inspector) const
    {
        ...
        m_documentSystemSettingsGroup = CreateSettingsPropertyGroup(
            ...
            { CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/DisplayWarningMessageDialogs",
                  "Display Warning Message Dialogs",
                  "Display message boxes for warnings opening documents",
                  true),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/DisplayErrorMessageDialogs",
                  "Display Error Message Dialogs",
                  "Display message boxes for errors opening documents",
                  true),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/EnableAutomaticReload",
                  "Enable Automatic Reload",
                  "Automatically reload documents after external modifications",
                  true),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/EnableAutomaticReloadPrompts",
                  "Enable Automatic Reload Prompts",
                  "Confirm before automatically reloading modified documents",
                  true),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/AutoSaveEnabled",
                  "Enable Auto Save",
                  "Automatically save documents after they are modified",
                  false),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/ToolsDocumentSystem/AutoSaveInterval",
                  "Auto Save Interval",
    ...
    void ToolsDocumentMainWindow::BuildCreateMenu(QAction* insertPostion)
    {
        DocumentTypeInfoVector documentTypes;
        ToolsDocumentSystemRequestBus::EventResult(
            documentTypes, m_toolId, &ToolsDocumentSystemRequestBus::Events::GetRegisteredDocumentTypes);
        ...
        for (const auto& documentType : documentTypes)
        {
            ...
            CreateActionAtPosition(parentMenu, insertPostion, name, [documentType, toolId = m_toolId, this]() {
                ...
                {
                    ToolsDocumentSystemRequestBus::Event(
                        toolId,
                        &ToolsDocumentSystemRequestBus::Events::CreateDocumentFromFilePath,
    ...
    void ToolsDocumentMainWindow::BuildOpenMenu(QAction* insertPostion)
    {
        DocumentTypeInfoVector documentTypes;
        ToolsDocumentSystemRequestBus::EventResult(
            documentTypes, m_toolId, &ToolsDocumentSystemRequestBus::Events::GetRegisteredDocumentTypes);
            ...
            if (!documentType.m_supportedExtensionsToOpen.empty())
            {
                ...
                CreateActionAtPosition(parentMenu, insertPostion, name, [documentType, toolId = m_toolId]() {
                    ...
                    AZ::SystemTickBus::QueueFunction([toolId, paths]() {
                        ...
                        {
                            ToolsDocumentSystemRequestBus::Event(toolId, &ToolsDocumentSystemRequestBus::Events::OpenDocument, path);
                }
                    });
    ...
    void ToolsDocumentMainWindow::AddDocumentTabBar()
    {
    ...
    void ToolsDocumentMainWindow::UpdateRecentFileMenu()
    {
        m_menuOpenRecent->clear();

        AZStd::vector<AZStd::string> absolutePaths;
        ToolsDocumentSystemRequestBus::EventResult(
            absolutePaths, m_toolId, &ToolsDocumentSystemRequestBus::Handler::GetRecentFilePaths);
        ...
        m_menuOpenRecent->addAction(tr("Clear Recent Files"), [this]() {
            AZ::SystemTickBus::QueueFunction([toolId = m_toolId]() {
                ToolsDocumentSystemRequestBus::Event(toolId, &ToolsDocumentSystemRequestBus::Handler::ClearRecentFilePaths);
        });
        });
    }

    QString ToolsDocumentMainWindow::GetDocumentPath(const AZ::Uuid& documentId) const
    {
        AZStd::string absolutePath;
        ToolsDocumentRequestBus::EventResult(absolutePath, documentId, &ToolsDocumentRequestBus::Handler::GetAbsolutePath);
        return absolutePath.c_str();
    }

    AZ::Uuid ToolsDocumentMainWindow::GetDocumentTabId(const int tabIndex) const
    {
    ...
    AZ::Uuid ToolsDocumentMainWindow::GetCurrentDocumentId() const
    {
        return GetDocumentTabId(m_tabWidget->currentIndex());
    }

    int ToolsDocumentMainWindow::GetDocumentTabIndex(const AZ::Uuid& documentId) const
    {
    ...
    bool ToolsDocumentMainWindow::HasDocumentTab(const AZ::Uuid& documentId) const
    {
        return GetDocumentTabIndex(documentId) >= 0;
    }

    bool ToolsDocumentMainWindow::AddDocumentTab(const AZ::Uuid& documentId, QWidget* viewWidget)
    {
    ...
    void ToolsDocumentMainWindow::RemoveDocumentTab(const AZ::Uuid& documentId)
    {
    ...
    void ToolsDocumentMainWindow::UpdateDocumentTab(const AZ::Uuid& documentId)
    {
        ...
        if (const int tabIndex = GetDocumentTabIndex(documentId); tabIndex >= 0)
        {
            ...
            ToolsDocumentRequestBus::EventResult(isModified, documentId, &ToolsDocumentRequestBus::Events::IsModified);
            AZStd::string absolutePath;
            ToolsDocumentRequestBus::EventResult(absolutePath, documentId, &ToolsDocumentRequestBus::Events::GetAbsolutePath);
    ...
    void ToolsDocumentMainWindow::SelectPrevDocumentTab()
    {
    ...
    void ToolsDocumentMainWindow::SelectNextDocumentTab()
    {
        if (m_tabWidget->count() > 1)
        {
    ...
    void ToolsDocumentMainWindow::OpenDocumentTabContextMenu()
                {
    ...
    void ToolsDocumentMainWindow::PopulateTabContextMenu(const AZ::Uuid& documentId, QMenu& menu)
        {
    ...
    AZStd::string ToolsDocumentMainWindow::GetSaveDocumentParams(const AZStd::string& initialPath, const AZ::Uuid& documentId) const
    {
        DocumentTypeInfo documentType;
        ToolsDocumentRequestBus::EventResult(
            documentType, documentId, &ToolsDocumentRequestBus::Events::GetDocumentTypeInfo);
        return GetSaveFilePathFromDialog(initialPath, documentType.m_supportedExtensionsToSave, documentType.m_documentTypeName);
    }

    void ToolsDocumentMainWindow::OnDocumentOpened(const AZ::Uuid& documentId)
    {
        AZStd::string absolutePath;
        ToolsDocumentRequestBus::EventResult(absolutePath, documentId, &ToolsDocumentRequestBus::Events::GetAbsolutePath);
    ...
    void ToolsDocumentMainWindow::OnDocumentClosed(const AZ::Uuid& documentId)
        {
        RemoveDocumentTab(documentId);
        SetStatusMessage(tr("Document closed: %1").arg(GetDocumentPath(documentId)).toUtf8().constData());
    }

    void ToolsDocumentMainWindow::OnDocumentCleared(const AZ::Uuid& documentId)
    {
        UpdateDocumentTab(documentId);
        QueueUpdateMenus(true);
    ...
    void ToolsDocumentMainWindow::OnDocumentError(const AZ::Uuid& documentId)
    {
        UpdateDocumentTab(documentId);
        QueueUpdateMenus(true);
    ...
    void ToolsDocumentMainWindow::OnDocumentDestroyed(const AZ::Uuid& documentId)
    {
        RemoveDocumentTab(documentId);
    }

    void ToolsDocumentMainWindow::OnDocumentModified(const AZ::Uuid& documentId)
    {
        UpdateDocumentTab(documentId);
    }

    void ToolsDocumentMainWindow::OnDocumentUndoStateChanged(const AZ::Uuid& documentId)
    {
    ...
    void ToolsDocumentMainWindow::OnDocumentSaved(const AZ::Uuid& documentId)
    {
        UpdateDocumentTab(documentId);
        SetStatusMessage(tr("Document saved: %1").arg(GetDocumentPath(documentId)).toUtf8().constData());
    }

    void ToolsDocumentMainWindow::closeEvent(QCloseEvent* closeEvent)
    {
    ...
    void ToolsDocumentMainWindow::dragEnterEvent(QDragEnterEvent* event)
    {
    ...
    void ToolsDocumentMainWindow::dragMoveEvent(QDragMoveEvent* event)
    {
    ...
    void ToolsDocumentMainWindow::dragLeaveEvent(QDragLeaveEvent* event)
    {
        Base::dragLeaveEvent(event);
    }

    void ToolsDocumentMainWindow::dropEvent(QDropEvent* event)
    {
    ...
    QAction* ToolsDocumentMainWindow::CreateActionAtPosition(
