...
//#include <AzToolsFramework/PerformanceMonitor/PerformanceMonitorRequestBus.h>
#include <AzToolsFramework/DocumentEditor/SettingsDialog/SettingsDialog.h>
#include <AzToolsFramework/API/Utils.h>
#include <AzToolsFramework/DocumentEditor/ToolsMainWindow.h>
#include <AzToolsFramework/DocumentEditor/ToolsMainWindowNotificationBus.h>
#include <AzToolsFramework/DocumentEditor/Inspector/InspectorWidget.h>
...
{
    ToolsMainWindow::ToolsMainWindow(const AZ::Crc32& toolId, const QString& objectName, QWidget* parent)
        : Base(parent)
        , m_toolId(toolId)
        , m_advancedDockManager(new AzQtComponents::FancyDocking(this, objectName.toUtf8().constData()))
    {
        m_mainWindowWrapper = new AzQtComponents::WindowDecorationWrapper(AzQtComponents::WindowDecorationWrapper::OptionAutoTitleBarButtons, parent);
        ...
        m_assetBrowser = new ToolsAssetBrowser(this);
        ...
        ToolsMainWindowRequestBus::Handler::BusConnect(m_toolId);
        ToolsMainMenuRequestBus::Handler::BusConnect(m_toolId);
    ...
    ToolsMainWindow::~ToolsMainWindow()
    {
        ...
        ToolsMainWindowRequestBus::Handler::BusDisconnect();
        ToolsMainMenuRequestBus::Handler::BusDisconnect();
    }

    void ToolsMainWindow::ActivateWindow()
    {
        show();
        raise();
        activateWindow();
    ...
    bool ToolsMainWindow::AddDockWidget(const AZStd::string& name, QWidget* widget, uint32_t area)
    {
    ...
    void ToolsMainWindow::RemoveDockWidget(const AZStd::string& name)
    {
    ...
    void ToolsMainWindow::SetDockWidgetVisible(const AZStd::string& name, bool visible)
    {
        for (auto dockWidget : findChildren<QDockWidget*>())
        {
            if (dockWidget->windowTitle().compare(name.c_str(), Qt::CaseInsensitive) == 0)
    ...
    bool ToolsMainWindow::IsDockWidgetVisible(const AZStd::string& name) const
    {
        for (auto dockWidget : findChildren<QDockWidget*>())
        {
            if (dockWidget->windowTitle().compare(name.c_str(), Qt::CaseInsensitive) == 0)
    ...
    AZStd::vector<AZStd::string> ToolsMainWindow::GetDockWidgetNames() const
    {
    ...
    void ToolsMainWindow::SetStatusMessage(const AZStd::string& message)
    {
        m_statusMessage->setText(QString("<font color=\"White\">%1</font>").arg(message.c_str()));
    }

    void ToolsMainWindow::SetStatusWarning(const AZStd::string& message)
    {
        m_statusMessage->setText(QString("<font color=\"Yellow\">%1</font>").arg(message.c_str()));
    }

    void ToolsMainWindow::SetStatusError(const AZStd::string& message)
    {
        m_statusMessage->setText(QString("<font color=\"Red\">%1</font>").arg(message.c_str()));
    }

    void ToolsMainWindow::QueueUpdateMenus(bool rebuildMenus)
    {
        ...
        if (!m_updateMenus)
        {
            ...
            QTimer::singleShot(0, this, [this]() {
                if (m_rebuildMenus)
                {
                    ...
                    ToolsMainMenuRequestBus::Event(m_toolId, &ToolsMainMenuRequestBus::Events::CreateMenus, menuBar());
                }

                ToolsMainMenuRequestBus::Event(m_toolId, &ToolsMainMenuRequestBus::Events::UpdateMenus, menuBar());
                m_updateMenus = false;
    ...
    void ToolsMainWindow::CreateMenus(QMenuBar* menuBar)
    {
        ...
        connect(m_menuEdit, &QMenu::aboutToShow, menuBar, [toolId = m_toolId](){
            ToolsMainWindowRequestBus::Event(toolId, &ToolsMainWindowRequestBus::Events::QueueUpdateMenus, false);
        });
    ...
    void ToolsMainWindow::UpdateMenus([[maybe_unused]] QMenuBar* menuBar)
    {
    }

    void ToolsMainWindow::PopulateSettingsInspector(InspectorWidget* inspector) const
    {
        m_applicationSettingsGroup = CreateSettingsPropertyGroup(
            "Application Settings",
            "Application Settings",
            {  CreateSettingsPropertyValue(
                    "/O3DE/AzToolsFramework/Application/ClearLogOnStart",
                    "Clear Log On Start",
                    "Clear the application log on startup",
                    false),
                CreateSettingsPropertyValue(
                    "/O3DE/AzToolsFramework/Application/EnableSourceControl",
                    "Enable Source Control",
                    "Enable source control for the application if it is available",
                    false),
                CreateSettingsPropertyValue(
                    "/O3DE/AzToolsFramework/Application/UpdateIntervalWhenActive",
                ...
                CreateSettingsPropertyValue(
                    "/O3DE/AzToolsFramework/Application/UpdateIntervalWhenNotActive",
                  ...
                  aznumeric_cast<AZ::s64>(1000)),
                CreateSettingsPropertyValue(
                    "/O3DE/AzToolsFramework/Application/AllowMultipleInstances",
        ...
        m_assetBrowserSettingsGroup = CreateSettingsPropertyGroup(
            ...
            "Asset Browser Settings",
            { CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/AssetBrowser/PromptToOpenMultipleFiles",
                    "Prompt To Open Multiple Files",
                    "Confirm before opening multiple files",
                    true),
              CreateSettingsPropertyValue(
                  "/O3DE/AzToolsFramework/AssetBrowser/PromptToOpenMultipleFilesThreshold",
    ...
    void ToolsMainWindow::OpenSettingsDialog()
    {
    ...
    AZStd::string ToolsMainWindow::GetHelpDialogText() const
    {
        return AZStd::string();
    }

    void ToolsMainWindow::OpenHelpDialog()
    {
        QMessageBox::information(this, windowTitle(), GetHelpDialogText().c_str());
    }

    void ToolsMainWindow::OpenAboutDialog()
    {
        QMessageBox::about(this, windowTitle(), QApplication::applicationName());
    }

    void ToolsMainWindow::showEvent(QShowEvent* showEvent)
    {
        if (!m_shownBefore)
        {
            m_shownBefore = true;
    ...
    void ToolsMainWindow::closeEvent(QCloseEvent* closeEvent)
    {
        if (closeEvent->isAccepted())
        {
            const QByteArray windowState = m_advancedDockManager->saveState();
            SetSettingsObject("/O3DE/AzToolsFramework/MainWindow/WindowState", AZStd::string(windowState.begin(), windowState.end()));
        }

        Base::closeEvent(closeEvent);
    }

    void ToolsMainWindow::BuildDockingMenu()
    {
    ...
    void ToolsMainWindow::BuildLayoutsMenu()
    {
    ...
    void ToolsMainWindow::BuildScriptsMenu()
    {
        QMenu* scriptsMenu = m_menuFile->addMenu(tr("Python Scripts"));
        connect(scriptsMenu, &QMenu::aboutToShow, this, [scriptsMenu]() {
            scriptsMenu->clear();
            AddRegisteredScriptToMenu(scriptsMenu, "/O3DE/AzToolsFramework/MainWindow/FileMenuScripts", {});
        });
    }

    void ToolsMainWindow::RestoreDefaultLayout()
    {
    ...
    void ToolsMainWindow::RestoreSavedLayout()
    {
        // Attempt to restore the layout that was saved the last time the application was closed. 
        const AZStd::string windowState = GetSettingsObject("/O3DE/AzToolsFramework/MainWindow/WindowState", AZStd::string());
    ...
    void ToolsMainWindow::SetupMetrics()
    {
        ...
        connect(&m_metricsTimer, &QTimer::timeout, this, &ToolsMainWindow::UpdateMetrics);
    ...
    void ToolsMainWindow::UpdateMetrics()
    {
    ...
    void ToolsMainWindow::UpdateWindowTitle()
    {
        AZ::Name apiName = AZ::Name("UNTITLED");// AZ::RHI::Factory::Get().GetName();
        if (!apiName.IsEmpty())
        {
