...
#include <AzToolsFramework/DocumentEditor/ToolsDocumentApplication.h>
...
{
    ToolsDocumentApplication::ToolsDocumentApplication(const char* targetName, int* argc, char*** argv)
        : Base(targetName, argc, argv)
    {
    }

    void ToolsDocumentApplication::StartCommon(AZ::Entity* systemEntity)
    {
        Base::StartCommon(systemEntity);

        m_documentSystem.reset(aznew ToolsDocumentSystem(m_toolId));

        m_assetBrowserInteractions->RegisterContextMenuActions(
            [](const ToolsAssetBrowserInteractions::AssetBrowserEntryVector& entries)
            {
                return entries.front()->GetEntryType() == AzToolsFramework::AssetBrowser::AssetBrowserEntry::AssetEntryType::Source;
            },
            [this]([[maybe_unused]] QWidget* caller, QMenu* menu, const ToolsAssetBrowserInteractions::AssetBrowserEntryVector& entries)
            {
                        ...
                        menu->addAction(createActionName, [entries, documentType, this]() {
                            ...
                            if (!savePath.empty())
                            {
                                ToolsDocumentSystemRequestBus::Event(
                                    m_toolId, &ToolsDocumentSystemRequestBus::Events::CreateDocumentFromFilePath,
        ...
        m_assetBrowserInteractions->RegisterContextMenuActions(
            ...
            [this](QWidget* caller, QMenu* menu, const ToolsAssetBrowserInteractions::AssetBrowserEntryVector& entries)
            {
                DocumentTypeInfoVector documentTypes;
                ToolsDocumentSystemRequestBus::EventResult(
                    documentTypes, m_toolId, &ToolsDocumentSystemRequestBus::Events::GetRegisteredDocumentTypes);
    ...
    void ToolsDocumentApplication::Destroy()
    {
        m_documentSystem.reset();
        Base::Destroy();
    }

    void ToolsDocumentApplication::ProcessCommandLine(const AZ::CommandLine& commandLine)
    {
        ...
        for (size_t openDocumentIndex = 0; openDocumentIndex < openDocumentCount; ++openDocumentIndex)
        {
            ...
            ToolsDocumentSystemRequestBus::Event(m_toolId, &ToolsDocumentSystemRequestBus::Events::OpenDocument, openDocumentPath);
        }
