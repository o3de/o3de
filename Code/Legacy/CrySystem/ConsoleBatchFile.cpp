...
bool CConsoleBatchFile::ExecuteConfigFile(const char* sFilename)
{
    ...
    if (sFilename[0] != '@') // console config files are actually by default in @projectproductassets@ instead of @projectproductassets@
    {
        // However, if we've passed in a relative or absolute path that matches an existing file name,
        // don't change it.  Only change it to "@projectproductassets@/filename" and strip off any relative paths
        ...
        {
            filename = PathUtil::Make("@projectproductassets@", PathUtil::GetFile(sFilename));
        }
    }
    else
    {
