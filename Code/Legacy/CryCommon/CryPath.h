...
namespace PathUtil
{
    ...
    inline stack_string MakeGamePath(const stack_string& path)
    {
        ...
        {
            if (AZ::StringFunc::Equal(relativePath.c_str(), "@projectproductassets@/", false, 9))
            {
        ...
        const char* rootValue = gEnv->pFileIO->GetAlias("@projectproductassets@");
        if (rootValue)
        {
                ...
                )
            {
                ...
                stack_string rooted = stack_string("@projectproductassets@") + chopped_string;
                return rooted;
            }
        }
