...
struct Matrix44_tpl
{
    ...
    */
    ...
    ILINE void SetRow4(int i, const Vec4_tpl& v)   {   assert(i < 4);    F* p = (F*)(&m00);    p[0 + 4 * i] = v.x;   p[1 + 4 * i] = v.y;   p[2 + 4 * i] = v.z;   p[3 + 4 * i] = v.w;   }
    ...
    ILINE Vec4_tpl GetColumn4(int i) const {   assert(i < 4);    F* p = (F*)(&m00);    return Vec4(p[i + 4 * 0], p[i + 4 * 1], p[i + 4 * 2], p[i + 4 * 3]);   }
...
//post-multiply
template<class F2>
ILINE Vec4_tpl operator*(const Matrix44_tpl<F2>& m, const Vec4_tpl& v)
{
    assert(m.IsValid());
    assert(v.IsValid());
    return Vec4_tpl(v.x * m.m00 + v.y * m.m01 + v.z * m.m02 + v.w * m.m03,
...
//pre-multiply
template<class F2>
ILINE Vec4_tpl operator*(const Vec4_tpl& v, const Matrix44_tpl<F2>& m)
{
    assert(m.IsValid());
    assert(v.IsValid());
    return Vec4_tpl(v.x * m.m00 + v.y * m.m10 + v.z * m.m20 + v.w * m.m30,
