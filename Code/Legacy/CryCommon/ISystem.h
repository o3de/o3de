...
#define REGISTER_COMMAND(_name, _func, _flags, _comment)                                                           (gEnv->pConsole == 0 ? false : gEnv->pConsole->AddCommand(_name, _func, (_flags), CVARHELP(_comment)))
// Summary:
//   Preferred way to unregister a CVar
#define UNREGISTER_CVAR(_name)                                                      (gEnv->pConsole == 0 ? (void)0 : gEnv->pConsole->UnregisterVariable(_name))
//   Preferred way to unregister a console command
#define UNREGISTER_COMMAND(_name)                                                           (gEnv->pConsole == 0 ? (void)0 : gEnv->pConsole->RemoveCommand(_name))
