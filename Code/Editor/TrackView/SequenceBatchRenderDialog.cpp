...
void CSequenceBatchRenderDialog::OnUpdateBeginPlayingSequence()
{
    ...
    renderItem.pSequence = GetIEditor()->GetMovieSystem()->FindLegacySequenceByName(seqName.c_str());
    ...
    GetIEditor()->GetMovieSystem()->AddSequence(renderItem.pSequence);

    [[maybe_unused]]const bool listenerAdded = GetIEditor()->GetMovieSystem()->AddMovieListener(renderItem.pSequence, this);
    AZ_Assert(listenerAdded, "Failed to add movie listener for sequence %s", renderItem.pSequence->GetSequenceEntityId().ToString().c_str());

    GetIEditor()->GetMovieSystem()->Reset(true, false);
...
bool CSequenceBatchRenderDialog::SetUpNewRenderItem(SRenderItem& item)
{
    ...
    // sequence
    item.pSequence = GetIEditor()->GetMovieSystem()->FindLegacySequenceByName(item.seqName.toUtf8().data());
