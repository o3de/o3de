...
void CCryEditApp::RunInitPythonScript(CEditCommandLineInfo& cmdInfo)
{
    ...
    if (cmdInfo.m_bRunPythonScript || cmdInfo.m_bRunPythonTestScript)
    {
        ...
        if (cmdInfo.m_pythonArgs.length() > 0 || cmdInfo.m_bRunPythonTestScript)
        {
            ...
            if (cmdInfo.m_bRunPythonTestScript)
            {
                // For each file, we also specify the name of the test for TIAF information
                ...
                // Multiple test case names can be provided separated by ';' 
                ...
                bool success = true;
                auto ExecuteByFilenamesAsTests = [&pythonArgs, &testList, &success](EditorPythonRunnerRequests* pythonRunnerRequests)
                {
                    for (const FileAndTestCase& fileAndTest : testList)
                    {
                        bool fileSuceeded = pythonRunnerRequests->ExecuteByFilenameAsTest(fileAndTest.m_file, fileAndTest.m_testCase, pythonArgs);
                        success = success && fileSuceeded;
                    }
                };
                EditorPythonRunnerRequestBus::Broadcast(ExecuteByFilenamesAsTests);
