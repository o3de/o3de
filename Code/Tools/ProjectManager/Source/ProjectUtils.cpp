...
namespace O3DE::ProjectManager::ProjectUtils
{
    ...
    int DisplayDetailedError(const QString& title, const AZ::Outcome<void, AZStd::pair<AZStd::string, AZStd::string>>& outcome, QWidget* parent)
    {
        return DisplayDetailedError(title, outcome.GetError().first, outcome.GetError().second, parent);
    }

    int DisplayDetailedError(const QString& title, const AZStd::string& generalError, const AZStd::string& detailedError, QWidget* parent, QMessageBox::StandardButtons buttons)
    {
