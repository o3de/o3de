...
namespace AZ
{
    namespace SceneAPI
    {
        namespace SceneBuilder
        {
            ...
            bool RecursiveHasChildBone(const aiNode* node, const AZStd::unordered_multimap<AZStd::string, const aiBone*>& boneByNameMap, const AZStd::unordered_set<AZStd::string>& animNodeByNameMap)
            {
                const bool isBone = boneByNameMap.contains(node->mName.C_Str()) || animNodeByNameMap.contains(node->mName.C_Str());
                if (isBone)
                {
                    return true;
                }
                ...
                for (unsigned int childIndex = 0; childIndex < node->mNumChildren; ++childIndex)
                {
                    ...
                    if (RecursiveHasChildBone(childNode, boneByNameMap, animNodeByNameMap))
                    {
                        return true;
                    }
                }
