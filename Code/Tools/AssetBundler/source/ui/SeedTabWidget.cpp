...
namespace AssetBundler
{
    ...
    void SeedTabWidget::SetModelDataSource()
    {
        ...
        m_filePathToGemNameMap = AssetBundler::GetDefaultSeedListFiles(g_cachedEngineRoot, projectName, m_guiApplicationManager->GetGemInfoList(), m_guiApplicationManager->GetEnabledPlatforms());

        // Get the list of default Seeds that are not stored in a Seed List file on-disk
        AZStd::vector<AZStd::string> defaultSeeds = GetDefaultSeeds(g_cachedEngineRoot, AZ::Utils::GetProjectPath(), projectName);
