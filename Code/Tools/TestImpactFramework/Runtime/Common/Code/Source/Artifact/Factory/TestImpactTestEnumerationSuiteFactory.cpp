...
namespace TestImpact
{
    ...
    namespace Python
    {
        ...
        AZStd::vector<AZStd::pair<AZStd::string, AZStd::vector<TestEnumerationSuite>>> TestEnumerationSuitesFactory(const AZStd::string& testEnumerationData) {
            const char newLineDelim = '\n';
            size_t startLine;
            size_t endLine = 0;

            AZStd::vector<AZStd::pair<AZStd::string, AZStd::vector<TestEnumerationSuite>>> pairs = AZStd::vector<AZStd::pair<AZStd::string, AZStd::vector<TestEnumerationSuite>>>();
            AZStd::map<AZStd::string, AZStd::map<AZStd::string, AZStd::set<AZStd::string>>> testMapping = AZStd::map<AZStd::string, AZStd::map<AZStd::string, AZStd::set<AZStd::string>>>();
            ...
            while ((startLine = testEnumerationData.find_first_not_of(newLineDelim, endLine)) != AZStd::string::npos)
            {
                ...
                // This regex matches pytest test names, in the form ScriptPath::TestFixture::TestName. It will accept both parameterized and unparamaterized tests (with or without [] at the end).
            ...
            // Lookup the map for that script path, and for each test class in that map, create a test suite object and add all the tests to that test suite.
            // Create a pair consisting of the script path and the test suites attached to that script, and add to our vector.
