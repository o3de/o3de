    ...
    namespace Native
    {
        ...
        public:
            ...
            TestEnumerator::JobInfo GenerateTestEnumerationJobInfo(
                const TestTarget* testTarget, TestEnumerator::JobInfo::Id jobId, TestEnumerator::JobInfo::CachePolicy cachePolicy) const;

            //! Generates the information for a test run job.
            //! @param testTarget The test target to generate the job information for.
            //! @param jobId The id to assign for this job.
            RegularTestRunner::JobInfo GenerateRegularTestRunJobInfo(
                const TestTarget* testTarget, RegularTestRunner::JobInfo::Id jobId) const;
            ...
            InstrumentedTestRunner::JobInfo GenerateInstrumentedTestRunJobInfo(
                const TestTarget* testTarget, InstrumentedTestRunner::JobInfo::Id jobId, CoverageLevel coverageLevel) const;
            ...
            AZStd::vector<TestEnumerator::JobInfo> GenerateTestEnumerationJobInfos(
                const AZStd::vector<const TestTarget*>& testTargets, TestEnumerator::JobInfo::CachePolicy cachePolicy) const;
            ...
            AZStd::vector<InstrumentedTestRunner::JobInfo> GenerateInstrumentedTestRunJobInfos(
                const AZStd::vector<const TestTarget*>& testTargets, CoverageLevel coverageLevel) const;
