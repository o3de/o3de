    ...
    const AZStd::vector<AZStd::string>& expectedNotMatchingPaths)
{
    ...
    TestAsset primaryFile("test_text");
    bool result = ProcessAsset(primaryFile, { { ".asset" }, {} }, { { wildcardDependency.c_str(), ProductPathDependencyType::ProductFile } });
    ASSERT_TRUE(result) << "Failed to Process main test asset";
...
{
    RunWildcardDependencyTestOnPaths("root/path/*.txt",
