...
namespace AssetProcessor
{
    ...
    void PathDependencyManager::ResolveDependencies(AssetBuilderSDK::ProductPathDependencySet& pathDeps, AZStd::vector<AssetBuilderSDK::ProductDependency>& resolvedDeps, const AZStd::string& platform, [[maybe_unused]] const AZStd::string& productName)
    {
        ...
        while (pathIter != pathDeps.end())
        {
            ...
            // The database uses % for wildcards, both path based searching uses *, so keep a copy of the path with the * wildcard for later use.
                ...
                if (!productInfoContainer.empty())
                {
                    ...
                    for (const auto& productDatabaseEntry : productInfoContainer)
                    {
                        if (m_stateData->GetSourceByJobID(productDatabaseEntry.m_jobPK, sourceDatabaseEntry))
                        {
                            ...
                            // The SQL wildcard search is greedy and doesn't match the path based, glob style wildcard search that is expected in this case.
                            // This also matches the behavior of resolving unmet dependencies later.
                            // There are two cases that wildcard dependencies resolve:
                            //  1. When the product with the wildcard dependency is first created, it resolves those dependencies against what's already in the database. That's this case.
                            //  2. When another product is created, all existing wildcard dependencies are compared against that product to see if it matches them.
