...
#define DXGL_COMPRESSED_GI_FORMATS(_Func) \
    ...
    _Func(PVRTC2_TYPELESS)                \
    _Func(PVRTC2_UNORM)                   \
    _Func(PVRTC2_UNORM_SRGB)              \
    _Func(PVRTC4_TYPELESS)                \
    _Func(PVRTC4_UNORM)                   \
    _Func(PVRTC4_UNORM_SRGB)              \
...
#undef _GI_FORMAT_ENUM_FUNC
    ...
    struct STextureFormat
    {
        ...
        //  Confetti BEGIN: Igor Lobanchikov
        MTLPixelFormat  m_eMetalFormat;
        MTLVertexFormat m_eMetalVertexFormat;
        //  Confetti End: Igor Lobanchikov
    };
    ...
    //  Igor: Motivation: texture swizzling is not supported by Metal. However, some texture formats swizzling is differnet from
    //  DX11 ones. It is possible swizzling control will be added to Metal in future.
    ...
    bool RGBAToSwizzleMask(const int aRGBA[4], TSwizzleMask& uMask);
};
