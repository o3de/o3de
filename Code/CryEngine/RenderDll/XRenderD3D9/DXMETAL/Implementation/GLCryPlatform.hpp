...
#undef CUSTOM_SCOPED_PROFILE
#define CUSTOM_SCOPED_PROFILE(_Name) FRAME_PROFILER_FAST(_Name, gEnv->pSystem, PROFILE_RENDERER, true);
...
namespace NCryMetal
{
    ...
    {
        AZ_Error("Metal Layer", false, "Unimplemented DirectX function %s being called in Metal in file %s line %d. Continuing but rendering artifacts may be present.", functionName, file, line);
    }
    ...
    struct STraceFile
    {
        ...
        void Printf(const char* szFormat, ...)
        {
            va_list kArgs;
            va_start(kArgs, szFormat);
            AZ::IO::Print(m_fileHandle, szFormat, kArgs);
            va_end(kArgs);
        }
