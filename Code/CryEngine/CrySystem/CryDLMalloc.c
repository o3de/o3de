...
#include "../../Framework/AzCore/AzCore/PlatformRestrictedFileDef.h"
...
static void* s_sbrkBase;
static void* s_sbrkEnd;
static void* s_sbrkMax;

extern volatile int dlmallocmapped;

void* SimSBrk(ptrdiff_t size)
{
    void* ret = NULL;
        ...
        if (size > 0)
        {
            ...
            if (newEnd <= (INT_PTR) s_sbrkMax)
            {
                ...
                if (ret)
                {
                    ...
                    CryInterlockedAdd(&dlmallocmapped, (LONG) size);
                }
            }
        }
        else if (size < 0)
...
#if !defined(ANDROID) && !defined(LINUX_CROSS_COMPILE)
#define PROT_READ 0
#define PROT_WRITE 0
#define MAP_PRIVATE 0
#define MAP_ANONYMOUS 1
...
typedef void* (* dlmmap_handler)(void*, size_t);
typedef int (* dlmunmap_handler)(void*, void*, size_t);
...
*/
int mspace_create_overhead(void);
...
*/
mspace create_mspace(size_t capacity, int locked, void* user, dlmmap_handler mmap, dlmunmap_handler munmap);
...
static void* mmap_alloc(mstate m, size_t nb)
{
    ...
    {
        char* mm = (char*)((*m->mmap)(m->extp, mmsize));
        if (mm != CMFAIL)
        {
...
static void* sys_alloc(mstate m, size_t nb)
{
    ...
    */
    ...
    if (HAVE_MMAP && tbase == CMFAIL)  /* Try MMAP */
    {
        ...
        {
            char* mp = (char*)((*m->mmap)(m->extp, rsize));
            if (mp != CMFAIL)
            {
                tbase = mp;
                tsize = rsize;
            ...
            if (RTCHECK(ok_address(fm, p) && ok_inuse(p)))
            {
                ...
                if (!pinuse(p))
                {
                    ...
                    if (is_mmapped(p))
                    {
                        ...
                        if ((*fm->munmap)(fm->extp, (char*)p - prevsize, psize) == 0)
                        {
                            fm->footprint -= psize;
                        }
                        goto postaction;
...
static mstate init_user_mstate(char* tbase, size_t tsize, void* user, dlmmap_handler mmap, dlmunmap_handler munmap)
{
...
static void* mmap_default(void* u, size_t sz)
{
    (void)u;
    return CALL_MMAP(sz);
}

static int munmap_default(void* u, void* p, size_t s)
{
    (void)u;
    return CALL_MUNMAP(p, s);
}

static void* mmap_missing(void* u, size_t sz)
{
    (void)u;
    (void)sz;
    return (void*) -1;
...
static int munmap_missing(void* u, void* p, size_t s)
{
    (void)u;
    (void)p;
    (void)s;
...
mspace create_mspace(size_t capacity, int locked, void* user, dlmmap_handler mmap, dlmunmap_handler munmap)
{
    ...
    if (capacity < (size_t) -(msize + TOP_FOOT_SIZE + mparams.page_size))
    {
        ...
        char* tbase = (char*)((*mmap)(user, tsize));
        if (tbase != CMFAIL)
        {
...
mspace create_mspace_with_base(void* base, size_t capacity, int locked)
{
    ...
    {
        m = init_user_mstate((char*)base, capacity, NULL, mmap_missing, munmap_missing);
