...
int Rijndael::init(RijndaelState &state, RijndaelMode mode, RijndaelDirection dir, const uint8 * key, RijndaelKeyLength keyLen, uint8 * initVector)
{
    // Not initialized yet
    state.m_state = RijndaelState::Invalid;

    // Check the mode
    if ((mode != RijndaelMode::CBC) && (mode != RijndaelMode::ECB) && (mode != RijndaelMode::CFB1)) return RIJNDAEL_UNSUPPORTED_MODE;
    state.m_mode = mode;

    // And the direction
    if ((dir != RijndaelDirection::Encrypt) && (dir != RijndaelDirection::Decrypt)) return RIJNDAEL_UNSUPPORTED_DIRECTION;
    state.m_direction = dir;
    ...
    if (initVector)
    {
        ...
        {
            state.m_initVector[i] = initVector[i];
        }
    }
    else
    {
    ...
    switch (keyLen)
    {
        case RijndaelKeyLength::Key16Bytes:
            uKeyLenInBytes = 16;
            state.m_uRounds = 10;
            break;
        case RijndaelKeyLength::Key24Bytes:
            uKeyLenInBytes = 24;
            state.m_uRounds = 12;
            break;
        case RijndaelKeyLength::Key32Bytes:
            uKeyLenInBytes = 32;
            state.m_uRounds = 14;
            break;
        default:
    ...
    // state.m_uRounds = (m_uKeyLenInBits / 32) + 6;
    ...
    keySched(state, keyMatrix);

    if (state.m_direction == RijndaelDirection::Decrypt) keyEncToDec(state);

    state.m_state = RijndaelState::Valid;

    return RIJNDAEL_SUCCESS;
}

int Rijndael::blockEncrypt(RijndaelState &state, const uint8 *input, int inputLen, uint8 *outBuffer)
{
    int i, k, numBlocks;
    uint8 block[16], iv[4][4];

    if (state.m_state != RijndaelState::Valid)return RIJNDAEL_NOT_INITIALIZED;
    if (state.m_direction != RijndaelDirection::Encrypt)return RIJNDAEL_BAD_DIRECTION;
    ...
    switch (state.m_mode)
    {
        case RijndaelMode::ECB:
            ...
            {
                encrypt(state, input, outBuffer);
                input += 16;
                outBuffer += 16;
            }
            break;
        case RijndaelMode::CBC:
            ((uint32*) block)[0] = ((uint32*) state.m_initVector)[0] ^ ((uint32*) input)[0];
            ((uint32*) block)[1] = ((uint32*) state.m_initVector)[1] ^ ((uint32*) input)[1];
            ((uint32*) block)[2] = ((uint32*) state.m_initVector)[2] ^ ((uint32*) input)[2];
            ((uint32*) block)[3] = ((uint32*) state.m_initVector)[3] ^ ((uint32*) input)[3];
            encrypt(state, block, outBuffer);
            input += 16;
            ...
            break;
        case RijndaelMode::CFB1:
#if STRICT_ALIGN 
            memcpy(iv, state.m_initVector, 16);
#else  /* !STRICT_ALIGN */
            *((uint32*) iv[0]) = *((uint32*) (state.m_initVector));
            *((uint32*) iv[1]) = *((uint32*) (state.m_initVector + 4));
            *((uint32*) iv[2]) = *((uint32*) (state.m_initVector + 8));
            *((uint32*) iv[3]) = *((uint32*) (state.m_initVector + 12));
#endif /* ?STRICT_ALIGN */
            for (i = numBlocks; i > 0; i--)
            {
                for (k = 0; k < 128; k++)
...
int Rijndael::padEncrypt(RijndaelState &state, const uint8 *input, int inputOctets, uint8 *outBuffer)
{
    int i, numBlocks, padLen;
    uint8 block[16], *iv;

    if (state.m_state != RijndaelState::Valid) return RIJNDAEL_NOT_INITIALIZED;
    if (state.m_direction != RijndaelDirection::Encrypt) return RIJNDAEL_NOT_INITIALIZED;
...
int Rijndael::blockDecrypt(RijndaelState &state, const uint8 *input, int inputLen, uint8 *outBuffer)
{
    int i, k, numBlocks;
    uint8 block[16], iv[4][4];

    if (state.m_state != RijndaelState::Valid) return RIJNDAEL_NOT_INITIALIZED;
    if ((state.m_mode != RijndaelMode::CFB1) && (state.m_direction == RijndaelDirection::Encrypt)) return RIJNDAEL_BAD_DIRECTION;
...
int Rijndael::padDecrypt(RijndaelState &state, const uint8 *input, int inputOctets, uint8 *outBuffer)
{
    int i, numBlocks, padLen;
    uint8 block[16];
    uint32 iv[4];
    ...
    if (state.m_state != RijndaelState::Valid) return RIJNDAEL_NOT_INITIALIZED;
    if (state.m_direction != RijndaelDirection::Decrypt) return RIJNDAEL_BAD_DIRECTION;
...
void Rijndael::keySched(RijndaelState &state, uint8 key[_MAX_KEY_COLUMNS][4])
{
    int j, rconpointer = 0;

    // Calculate the necessary round keys
    // The number of calculations depends on keyBits and blockBits
    int uKeyColumns = state.m_uRounds - 6;
    ...
    for (j = 0; (j < (int) uKeyColumns) && (r <= (int) state.m_uRounds);)
    {
        for (; (j < uKeyColumns) && (t < 4); j++, t++)
        {
            *((uint32*) state.m_expandedKey[r][t]) = *((uint32*) tempKey[j]);
        }
    ...
    while (r <= state.m_uRounds)
    {
        ...
        for (j = 0; (j < uKeyColumns) && (r <= state.m_uRounds);)
        {
            for (; (j < uKeyColumns) && (t < 4); j++, t++)
            {
                *((uint32*) state.m_expandedKey[r][t]) = *((uint32*) tempKey[j]);
...
void Rijndael::keyEncToDec(RijndaelState &state)
{
    uint32 r;
    uint8 *w;

    for (r = 1; r < state.m_uRounds; r++)
    {
        w = state.m_expandedKey[r][0];
        *((uint32*) w) = *((uint32*) U1[w[0]]) ^ *((uint32*) U2[w[1]]) ^ *((uint32*) U3[w[2]]) ^ *((uint32*) U4[w[3]]);
        w = state.m_expandedKey[r][1];
        *((uint32*) w) = *((uint32*) U1[w[0]]) ^ *((uint32*) U2[w[1]]) ^ *((uint32*) U3[w[2]]) ^ *((uint32*) U4[w[3]]);
        w = state.m_expandedKey[r][2];
        *((uint32*) w) = *((uint32*) U1[w[0]]) ^ *((uint32*) U2[w[1]]) ^ *((uint32*) U3[w[2]]) ^ *((uint32*) U4[w[3]]);
        w = state.m_expandedKey[r][3];
        *((uint32*) w) = *((uint32*) U1[w[0]]) ^ *((uint32*) U2[w[1]]) ^ *((uint32*) U3[w[2]]) ^ *((uint32*) U4[w[3]]);
    }
}

void Rijndael::encrypt(RijndaelState &state, const uint8 a[16], uint8 b[16])
{
    uint32 r;
    uint8 temp[4][4];

    *((uint32*) temp[0]) = *((uint32*) (a)) ^ *((uint32*) state.m_expandedKey[0][0]);
    *((uint32*) temp[1]) = *((uint32*) (a + 4)) ^ *((uint32*) state.m_expandedKey[0][1]);
    *((uint32*) temp[2]) = *((uint32*) (a + 8)) ^ *((uint32*) state.m_expandedKey[0][2]);
    *((uint32*) temp[3]) = *((uint32*) (a + 12)) ^ *((uint32*) state.m_expandedKey[0][3]);
    ...
    *((uint32*) (b + 12)) = *((uint32*) T1[temp[3][0]])
        ...
        ^ *((uint32*) T3[temp[1][2]])
        ^ *((uint32*) T4[temp[2][3]]);
    for (r = 1; r < state.m_uRounds - 1; r++)
    {
        *((uint32*) temp[0]) = *((uint32*) (b)) ^ *((uint32*) state.m_expandedKey[r][0]);
        *((uint32*) temp[1]) = *((uint32*) (b + 4)) ^ *((uint32*) state.m_expandedKey[r][1]);
        *((uint32*) temp[2]) = *((uint32*) (b + 8)) ^ *((uint32*) state.m_expandedKey[r][2]);
        *((uint32*) temp[3]) = *((uint32*) (b + 12)) ^ *((uint32*) state.m_expandedKey[r][3]);
    ...
    *((uint32*) temp[0]) = *((uint32*) (b)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds - 1][0]);
    *((uint32*) temp[1]) = *((uint32*) (b + 4)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds - 1][1]);
    *((uint32*) temp[2]) = *((uint32*) (b + 8)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds - 1][2]);
    *((uint32*) temp[3]) = *((uint32*) (b + 12)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds - 1][3]);
    b[0] = T1[temp[0][0]][1];
    b[1] = T1[temp[1][1]][1];
    b[2] = T1[temp[2][2]][1];
    b[3] = T1[temp[3][3]][1];
    ...
    b[14] = T1[temp[1][2]][1];
    b[15] = T1[temp[2][3]][1];
    *((uint32*) (b)) ^= *((uint32*) state.m_expandedKey[state.m_uRounds][0]);
    *((uint32*) (b + 4)) ^= *((uint32*) state.m_expandedKey[state.m_uRounds][1]);
    *((uint32*) (b + 8)) ^= *((uint32*) state.m_expandedKey[state.m_uRounds][2]);
    *((uint32*) (b + 12)) ^= *((uint32*) state.m_expandedKey[state.m_uRounds][3]);
}

void Rijndael::decrypt(RijndaelState &state, const uint8 a[16], uint8 b[16])
{
    int r;
    uint8 temp[4][4];

    *((uint32*) temp[0]) = *((uint32*) (a)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds][0]);
    *((uint32*) temp[1]) = *((uint32*) (a + 4)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds][1]);
    *((uint32*) temp[2]) = *((uint32*) (a + 8)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds][2]);
    *((uint32*) temp[3]) = *((uint32*) (a + 12)) ^ *((uint32*) state.m_expandedKey[state.m_uRounds][3]);
    ...
    *((uint32*) temp[0]) = *((uint32*) (b)) ^ *((uint32*) state.m_expandedKey[1][0]);
    *((uint32*) temp[1]) = *((uint32*) (b + 4)) ^ *((uint32*) state.m_expandedKey[1][1]);
    *((uint32*) temp[2]) = *((uint32*) (b + 8)) ^ *((uint32*) state.m_expandedKey[1][2]);
    *((uint32*) temp[3]) = *((uint32*) (b + 12)) ^ *((uint32*) state.m_expandedKey[1][3]);
    b[0] = S5[temp[0][0]];
    b[1] = S5[temp[3][1]];
    b[2] = S5[temp[2][2]];
    b[3] = S5[temp[1][3]];
    ...
    b[14] = S5[temp[1][2]];
    b[15] = S5[temp[0][3]];
    *((uint32*) (b)) ^= *((uint32*) state.m_expandedKey[0][0]);
    *((uint32*) (b + 4)) ^= *((uint32*) state.m_expandedKey[0][1]);
    *((uint32*) (b + 8)) ^= *((uint32*) state.m_expandedKey[0][2]);
    *((uint32*) (b + 12)) ^= *((uint32*) state.m_expandedKey[0][3]);
}
