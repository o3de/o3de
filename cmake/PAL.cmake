...
function(o3de_get_gem_candidate_paths gem_name registered_external_subdirs candidate_gem_paths)
    ...
    if(NOT cache_exists)
        foreach(external_subdir IN LISTS registered_external_subdirs)
            set(candidate_gem_path ${external_subdir}/gem.json)
            if(EXISTS ${candidate_gem_path})
                unset(candidate_paths)
                list(APPEND candidate_paths ${external_subdir})
                o3de_read_json_key(candidate_gem_name ${candidate_gem_path} "gem_name")

                set(cached_candidate_paths_property "${candidate_gem_name}:CandidatePaths")
                get_property(cached_candidate_paths GLOBAL PROPERTY ${cached_candidate_paths_property})
                if(cached_candidate_paths)
                    list(APPEND candidate_paths ${cached_candidate_paths})
                endif()

                set_property(GLOBAL PROPERTY ${cached_candidate_paths_property} ${candidate_paths})
            endif()
        endforeach()
        set_property(GLOBAL PROPERTY "CachedGemCandidatePaths" TRUE)
    endif()

    get_property(cached_candidate_paths GLOBAL PROPERTY "${gem_name}:CandidatePaths")
    if(cached_candidate_paths)
        set(${candidate_gem_paths} ${cached_candidate_paths} PARENT_SCOPE)
    endif()
endfunction()

#! o3de_find_most_compatible_gem_with_registered_external_subdirs: Query the path of a gem using its name
#  IMPORTANT NOTE: This does not take perform any dependency resolution, 
#  which is fine if you don't need it and just want speed. If you don't have multiple versions of gems
#  you could just use o3de_find_gem_with_registered_external_subdirs() which should be faster
# \arg:gem_name the gem name to find
# \arg:output_gem_path the path of the gem to set
# \arg:registered_external_subdirs a list of external subdirectories registered accross
#      all manifest files to look for gems
function(o3de_find_most_compatible_gem_with_registered_external_subdirs gem_name output_gem_path registered_external_subdirs)

    o3de_get_name_and_version_specifier(${gem_name} spec_name spec_op spec_version)

    # Look for a cached result if we've already found the most compatible gem path
    set(cached_path_property "${spec_name}:CachedPath")
    get_property(cached_path GLOBAL PROPERTY ${cached_path_property})
    if(cached_path)
        set(${output_gem_path} ${cached_path} PARENT_SCOPE)
        return()
    endif()

    set(most_compatible_gem_path "")

    o3de_get_gem_candidate_paths(${spec_name} "${registered_external_subdirs}" candidate_gem_paths)
    if(candidate_gem_paths)
        set(most_compatible_gem_version "")
        foreach(external_subdir IN LISTS candidate_gem_paths)
            set(candidate_gem_path ${external_subdir}/gem.json)
            o3de_read_json_key(candidate_gem_version ${candidate_gem_path} "version")
            if(NOT spec_op)
                # if there is no version specifier then every gem is compatible so pick
                # the one with the highest version
                set(is_compatible TRUE)
            else()
                o3de_get_version_compatible(${candidate_gem_version} ${spec_op} ${spec_version} is_compatible)
            endif()
            if(is_compatible)
                if(NOT most_compatible_gem_path)
                    set(most_compatible_gem_path ${external_subdir})
                    set(most_compatible_gem_version ${candidate_gem_version})
                elseif(${candidate_gem_version} VERSION_GREATER ${most_compatible_gem_version})
                    set(most_compatible_gem_path ${external_subdir})
                    set(most_compatible_gem_version ${candidate_gem_version})
                endif()
            endif()
        ...
        # Cache the result to avoid searching again
        set_property(GLOBAL PROPERTY ${cached_path_property} "${most_compatible_gem_path}")
    endif()

    set(${output_gem_path} ${most_compatible_gem_path} PARENT_SCOPE)
endfunction()
