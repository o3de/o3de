...
if(json_error OR NOT project_engine)
    if(NOT project_engine)
        ...
        cmake_path(GET CMAKE_CURRENT_SOURCE_DIR PARENT_PATH candidate_engine_path)
        set(reached_root_dir FALSE)
        while(NOT reached_root_dir)
            cmake_path(COMPARE "${this_engine_path}" EQUAL "${candidate_engine_path}" is_same_path)
            if(is_same_path)
                message(VERBOSE "The project folder is a subdirectory of this engine.")
                set(PACKAGE_VERSION_COMPATIBLE TRUE)
                return()
            else()
                cmake_path(GET candidate_engine_path PARENT_PATH parent_path)
                cmake_path(COMPARE "${candidate_engine_path}" EQUAL "${parent_path}" reached_root_dir)
                set(candidate_engine_path ${parent_path})
            endif()
        endwhile()
    endif()
