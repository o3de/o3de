    ...
    if(EXISTS ${gem_json_path})
        ...
        foreach(platform ${pal_platforms})
            ...
            if(gem_restricted_path)
                o3de_pal_dir(pal_dir ${gem_path}/Platform/${platform} ${gem_restricted_path} ${gem_path} ${gem_parent_relative_path})
                set(restricted_gem_json_path ${pal_dir}/gem_${platform_lower}.json)
                if(EXISTS ${restricted_gem_json_path})
                    read_json_external_subdirs(restricted_gem_external_subdirs ${restricted_gem_json_path})
                    foreach(restricted_gem_external_subdir ${restricted_gem_external_subdirs})
                        file(REAL_PATH ${restricted_gem_external_subdir} real_external_subdir BASE_DIRECTORY ${pal_dir})
...
function(add_engine_json_external_subdirectories)
    ...
    if(EXISTS ${engine_json_path})
        ...
        foreach(platform ${pal_platforms})
            string(TOLOWER ${platform} platform_lower)
            o3de_restricted_path(${engine_json_path} engine_restricted_path)
            if(engine_restricted_path)
                o3de_pal_dir(pal_dir ${LY_ROOT_FOLDER}/Platform/${platform} ${engine_restricted_path} ${LY_ROOT_FOLDER})
                set(restricted_engine_json_path ${pal_dir}/engine_${platform_lower}.json)
                if(EXISTS ${restricted_engine_json_path})
                    read_json_external_subdirs(restricted_engine_external_subdirs ${restricted_engine_json_path})
                    foreach(restricted_engine_external_subdir ${restricted_engine_external_subdirs})
                        file(REAL_PATH ${restricted_engine_external_subdir} real_external_subdir BASE_DIRECTORY ${pal_dir})
    ...
    if(EXISTS ${project_json_path})
        ...
        foreach(platform ${pal_platforms})
            ...
            if(project_restricted_path)
                o3de_pal_dir(pal_dir ${project_path}/Platform/${platform} ${project_restricted_path} ${project_path} ${project_parent_relative_path})
                set(restricted_project_json_path ${pal_dir}/project_${platform_lower}.json)
                if(EXISTS ${restricted_project_json_path})
                    read_json_external_subdirs(restricted_project_external_subdirs ${restricted_project_json_path})
                    foreach(restricted_project_external_subdir ${restricted_project_external_subdirs})
                        file(REAL_PATH ${restricted_project_external_subdir} real_external_subdir BASE_DIRECTORY ${pal_dir})
