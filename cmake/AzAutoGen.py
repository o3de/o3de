        ...
        if cmakeFile:
            ...
            with open(sourceCmakeFile, 'r') as file:
                ...
                for line in rawCmakeFileContent:
                    cleanLines = line.strip().splitlines() # clean each line by removing line breaker
                    if len(cleanLines) >= 1 \
                        and (cleanLines[0].endswith(".xml") \
                            or cleanLines[0].endswith(".json") \
                            or cleanLines[0].endswith(".jinja")):
                        result.append(cleanLines[0])
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument("inputCmakeFiles", help="cmake files which contains files to run azcg expansion rules against")
