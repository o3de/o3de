...
function(ly_file_read_cache path content)
    unset(file_content)
    string(SHA256 file_path_hash "${path}")
    set(file_cache_var_name "o3de_file_cache_${file_path_hash}")
    get_property(file_content GLOBAL PROPERTY ${file_cache_var_name})
    if(NOT file_content)
        #message(INFO "Adding '${path}' to cache (${file_cache_var_name})")
        ly_file_read(${path} file_content)
        set_property(GLOBAL PROPERTY ${file_cache_var_name} ${file_content})
    #else()
    #    message(VERBOSE " Found '${path}' in cache (${file_cache_var_name})")
    endif()
    set(${content} ${file_content} PARENT_SCOPE)
endfunction()
