...
class MainWindow(tkp.Tk):
    ...
    def __init__(self, export_config, is_sdk):
        ...
        self.init_project_build_targets(self.main_frame)

        self.init_asset_bundling_options(self.main_frame)

        self.init_archive_settings_frame(self.main_frame)
    ...
    def init_project_build_options(self, parent):
        ...
        self.add_simple_text_entry(parent=project_build_options_frame,
        ...
        self.add_simple_text_entry(parent=project_build_options_frame,
                                   label_text=f'{platform.system()} Launcher Build Path',
                                   config_key='default.launcher.build.path')

    def init_project_build_targets(self, parent):
        """
        Initialize + Configure the Project Build Targets

        :param parent:  The parent widget grid container
        """
        project_build_options_frame = tk.LabelFrame(parent, text="Project Build Targets")
        project_build_options_frame.grid(padx=8, sticky=tk.EW)
        ...
        sep = ttk.Separator(project_build_options_frame, orient=tk.HORIZONTAL)
        sep.grid(sticky=tk.EW, columnspan=3)
