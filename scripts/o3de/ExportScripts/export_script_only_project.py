...
# this is the install script for script-only projects.  It uses generic launchers
# instead of trying to build one, and relies on it being run against a pre-built install image
# rather than engine source code.  It cannot do monolithic builds.

# mostly copied from export_source_built_project.py
...
def export_script_only_project(ctx: exp.O3DEScriptExportContext,
    ...
    # in script-only mode, the tools must come from the pre-built installer and it cannot be engine centric
    tools_build_path = exp.LOCAL_ENGINE_PATH

    default_base_build_path = ctx.project_path / 'build'
    ...
    # script only mode requires project.json
    project_file_patterns_to_copy.append("project.json")
...
if "o3de_context" in globals():
    ...
    try:
        export_script_only_project(ctx=o3de_context,
