...
namespace MCore
{
    ...
    public:
         ...
         */
        MCORE_INLINE explicit Array(T* elems, uint32 num, uint16 memCategory = MCORE_MEMCATEGORY_ARRAY)
            : mLength(num)
            , mMaxLength(AllocSize(num))
            , mMemCategory(memCategory)
        {
            mData = (T*)MCore::Allocate(mMaxLength * sizeof(T), mMemCategory, MEMORYBLOCK_ID, MCORE_FILE, MCORE_LINE);
         ...
         * @param memCategory The memory category the array is in.
         */
        MCORE_INLINE explicit Array(uint32 initSize, uint16 memCategory = MCORE_MEMCATEGORY_ARRAY)
            : mData(nullptr)
            , mLength(initSize)
            , mMaxLength(initSize)
            , mMemCategory(memCategory)
         ...
         */
        Array(Array<T>&& other)                                                                                     { mData = other.mData; mLength = other.mLength; mMaxLength = other.mMaxLength; mMemCategory = other.mMemCategory; other.mData = nullptr; other.mLength = 0; other.mMaxLength = 0; }
         ...
         */
        MCORE_INLINE void Add(const Array<T>& a)
        {
            uint32 l = mLength;
        ...
        MCORE_INLINE void Swap(uint32 pos1, uint32 pos2)
        {
            if (pos1 != pos2)
            {
                MCore::Swap(GetItem(pos1), GetItem(pos2));
            }
        }
        ...
        /**
         * The same as Reserve, except that this also can shrink the memory to the specified size if more has been allocated already.
         * If the current length is larger than the specified minLength nothing will happen.
         * @param minLength The minimum length the array should have.
         */
        MCORE_INLINE void ReserveExact(uint32 minLength)
        {
         ...
         * Please note that the array has to be large enough. You can't move data past the end of the array.
        ...
        //Array<T>&             operator= (const Array<T>& other)                   { if (&other != this) { Clear(); mMemCategory = other.mMemCategory; Grow(other.mLength); for (uint32 i=0; i<mLength; ++i) Construct(i, other.mData[i]); } return *this; }
    ...
    private:
        ...
        MCORE_INLINE void   Alloc(uint32 num)                                       { mData = (T*)MCore::Allocate(num * sizeof(T), mMemCategory, MEMORYBLOCK_ID, MCORE_FILE, MCORE_LINE); }
        MCORE_INLINE void   Realloc(uint32 newSize)
        {
            ...
            if (mData)
            {
                mData = (T*)MCore::Realloc(mData, newSize * sizeof(T), mMemCategory, MEMORYBLOCK_ID, MCORE_FILE, MCORE_LINE);
            }
            else
            {
                mData = (T*)MCore::Allocate(newSize * sizeof(T), mMemCategory, MEMORYBLOCK_ID, MCORE_FILE, MCORE_LINE);
            }

            mMaxLength = newSize;
        }
        MCORE_INLINE void   Free()
        {
            mLength = 0;
            mMaxLength = 0;
            if (mData)
