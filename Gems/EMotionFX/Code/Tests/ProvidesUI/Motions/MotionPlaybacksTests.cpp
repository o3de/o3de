...
namespace EMotionFX
{
    TEST_F(PreviewMotionFixture, MotionCanBePlayed)
    {
        ...
        */
        ...
        EXPECT_EQ(tableWidget->rowCount(), 1) << "Expected the table to have no rows yet";

        // Create actor and actor instance.
        const char* actorFilename = "@gemroot:EMotionFX@/Code/Tests/TestAssets/Rin/rin.actor";
        ...
        const auto rect = tableWidget->visualItemRect(item);
        QTest::mouseClick(tableWidget->viewport(), Qt::LeftButton, {}, rect.center());
        QTest::mouseDClick(tableWidget->viewport(), Qt::LeftButton, {}, rect.center());
