...
namespace EMotionFX
{
    void PreviewMotionFixture::SetUp()
    {
        ...
        ExecuteCommands({
        "ImportMotion -filename @gemroot:EMotionFX@/Code/Tests/TestAssets/Rin/rin_idle.motion",
        "MotionSetAddMotion -motionSetID 0 -motionFilenamesAndIds @gemroot:EMotionFX@/Code/Tests/TestAssets/Rin/rin_idle.motion;rin_idle"
            });

        char resolvedPath[AZ::IO::MaxPathLength];
        EXPECT_TRUE(AZ::IO::FileIOBase::GetInstance()->ResolvePath("@gemroot:EMotionFX@/Code/Tests/TestAssets/Rin/rin_idle.motion", resolvedPath, AZ_ARRAY_SIZE(resolvedPath)));
