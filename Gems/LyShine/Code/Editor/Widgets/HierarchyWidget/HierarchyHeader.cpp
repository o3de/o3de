    ...
    , m_selectableIcon(UICANVASEDITOR_HIERARCHY_HEADER_ICON_PADLOCK)
{
    ...
    QObject::connect(
        ...
        [parent](int logicalIndex)
        {
            ...
            {
                CommandHierarchyItemToggleIsVisible::Push(parent->GetEditorWindow()->GetActiveStack(), parent, items);
            }
            else if (logicalIndex == kHierarchyColumnIsSelectable)
            {
                CommandHierarchyItemToggleIsSelectable::Push(parent->GetEditorWindow()->GetActiveStack(), parent, items);
            }
            else
            {
