...
#include "Helpers/GuideHelpers.h"
#include "Helpers/RecentFiles.h"
#include "Helpers/FileHelpers.h"
#include "Toolbar/AlignToolbarSection.h"
#include "Toolbar/ModeToolbar.h"
#include "Widgets/ViewportWidget/Interaction/ViewportAlign.h"
...
QAction* EditorWindow::AddMenuAction(const QString& text, bool enabled, QMenu* menu, AZStd::function<void(bool)> function)
{
    ...
    QObject::connect(action, &QAction::triggered, function);
    ...
    // Redo.
    {
        ...
        // list of shortcuts explicitly.
        {
            action->setShortcuts(QList<QKeySequence>{
                AzQtComponents::RedoKeySequence, QKeySequence(Qt::META | Qt::SHIFT | Qt::Key_Z), QKeySequence(QKeySequence::Redo) });
        }
    ...
    // Add Align sub-menu
    {
        ...
        // Add each sub-menu item. Store the QActions so that we can enable/disable them when align is allowed or not.
        {
            ...
            m_actionsEnabledWithAlignAllowed.push_back(AddMenuAction(
                ...
                [this]([[maybe_unused]] bool checked)
                {
                    ViewportAlign::AlignSelectedElements(this, ViewportAlign::AlignType::HorizontalRight);
                }));
        }
    }
}
...
void EditorWindow::AddMenu_View_LanguageSetting(QMenu* viewMenu)
{
    ...
    AZStd::string fullLocPath(
        AZStd::string(gEnv->pFileIO->GetAlias("@products@")) + "/" + AZStd::string(m_startupLocFolderName.toUtf8().constData()));
    ...
    for (auto subDirectory : infoList)
    {
        ...
        QObject::connect(
            ...
            [this, directoryName]([[maybe_unused]] bool checked)
            {
                ...
                AZ_Assert(
                    locFolderCvar, "sys_localization_folder no longer defined! This should be created in CSystem::CreateSystemVars().");
                ...
                AZ_Assert(languageCvar, "g_language no longer defined! This should be created in CSystem::CreateSystemVars().");
...
void EditorWindow::AddMenu_Preview()
{
    ...
    // Toggle preview.
    {
        ...
        QObject::connect(action, &QAction::triggered, this, &EditorWindow::ToggleEditorMode);
        menu->addAction(action);
        addAction(action); // Also add the action to the window until the shortcut dispatcher can find the menu action
    }
}
