...
CommandHierarchyItemToggleIsVisible::CommandHierarchyItemToggleIsVisible(
...
{
    setText(
        (items.size() == 1) ? QString("toggle visibility of \"%1\"").arg((*items.begin())->GetElement()->GetName().c_str())
                            : QString("toggle visibility"));
...
void CommandHierarchyItemToggleIsVisible::Push(UndoStack* stack, HierarchyWidget* hierarchy, const HierarchyItemRawPtrList& items)
{
    ...
    stack->push(new CommandHierarchyItemToggleIsVisible(stack, hierarchy, items));
}
