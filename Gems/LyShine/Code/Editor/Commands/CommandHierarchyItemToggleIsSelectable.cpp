...
#include "Undo/UndoStackExecutionScope.h"
...
CommandHierarchyItemToggleIsSelectable::CommandHierarchyItemToggleIsSelectable(
    UndoStack* stack, HierarchyWidget* hierarchy, const HierarchyItemRawPtrList& items)
    : QUndoCommand()
    , m_stack(stack)
...
{
    setText(
        (items.size() == 1) ? QString("toggle selectability of \"%1\"").arg((*items.begin())->GetElement()->GetName().c_str())
                            : QString("toggle selectability"));
...
void CommandHierarchyItemToggleIsSelectable::Push(UndoStack* stack, HierarchyWidget* hierarchy, const HierarchyItemRawPtrList& items)
{
    ...
    stack->push(new CommandHierarchyItemToggleIsSelectable(stack, hierarchy, items));
}
