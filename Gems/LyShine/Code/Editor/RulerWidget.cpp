...
void RulerWidget::mousePressEvent(QMouseEvent* ev)
{
    ...
    AZ::Vector2 localMousePos = QtHelpers::MapGlobalPosToLocalVector2(m_editorWindow->GetViewport(), ev->globalPosition());
    float dpiScaleFactor = m_editorWindow->GetViewport()->WidgetToViewportFactor();
...
void RulerWidget::mouseMoveEvent(QMouseEvent* ev)
{
    ...
    m_editorWindow->GetViewport()->SetRulerCursorPositions(ev->globalPosition());
}
