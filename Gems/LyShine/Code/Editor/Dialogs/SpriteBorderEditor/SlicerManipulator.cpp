...
SlicerManipulator::SlicerManipulator(
    SpriteBorder border, QSize& unscaledPixmapSize, QSize& scaledPixmapSize, ISprite* sprite, QGraphicsScene* scene, SlicerEdit* edit)
    : QGraphicsRectItem()
    , m_border(border)
    ...
    , m_sprite(sprite)
    , m_unscaledOverScaledFactor(
          ((float)m_unscaledPixmapSize.width() / (float)m_scaledPixmapSize.width()),
          ((float)m_unscaledPixmapSize.height() / (float)m_scaledPixmapSize.height()))
    , m_scaledOverUnscaledFactor((1.0f / m_unscaledOverScaledFactor.x()), (1.0f / m_unscaledOverScaledFactor.y()))
    ...
    , m_edit(edit)
{
    ...
    setRect(
        (m_isVertical ? -(UICANVASEDITOR_MANIPULATOR_GRASPABLE_THICKNESS_IN_PIXELS * 0.5f) : -UICANVASEDITOR_ARBITRARILY_LARGE_NUMBER),
    ...
    setPixelPosition(GetBorderValueInPixels(
        m_sprite, m_border, aznumeric_cast<float>(m_isVertical ? m_unscaledPixmapSize.width() : m_unscaledPixmapSize.height())));

    setFlag(QGraphicsItem::ItemIsMovable, true);
    setFlag(
        QGraphicsItem::ItemIsSelectable,
        true); // This allows using the CTRL key to select multiple manipulators and move them simultaneously.
...
void SlicerManipulator::SetPixmapSizes(const QSize& unscaledSize, const QSize& scaledSize)
{
    ...
    m_unscaledOverScaledFactor = QPointF(
        ((float)m_unscaledPixmapSize.width() / (float)m_scaledPixmapSize.width()),
        ((float)m_unscaledPixmapSize.height() / (float)m_scaledPixmapSize.height()));
    m_scaledOverUnscaledFactor = QPointF((1.0f / m_unscaledOverScaledFactor.x()), (1.0f / m_unscaledOverScaledFactor.y()));
}
...
QVariant SlicerManipulator::itemChange(GraphicsItemChange change, const QVariant& value)
{
    ...
    {
        const float totalScaledSizeInPixels =
            aznumeric_cast<float>(m_isVertical ? m_scaledPixmapSize.width() : m_scaledPixmapSize.height());
        float manipulatorPos = aznumeric_cast<float>(AZ::GetMax(m_isVertical ? value.toPointF().x() : value.toPointF().y(), 0.0));
        const float borderValue = aznumeric_cast<float>(
            m_isVertical ? (manipulatorPos * m_unscaledOverScaledFactor.x()) : (manipulatorPos * m_unscaledOverScaledFactor.y()));
        ...
        return QPointF((m_isVertical ? manipulatorPos : 0.0f), (m_isVertical ? 0.0f : manipulatorPos));
    }
