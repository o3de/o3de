...
namespace AZ
{
    namespace RPI
    {
        ...
        bool GetSubImagePixelValues(const AZ::Data::Asset<AZ::RPI::StreamingImageAsset>& imageAsset, AZStd::pair<uint32_t, uint32_t> topLeft, AZStd::pair<uint32_t, uint32_t> bottomRight, AZStd::function<void(const AZ::u32& x, const AZ::u32&y, const float& value)> callback, uint32_t componentIndex = 0, uint32_t mip = 0, uint32_t slice = 0);

        //! Process a region of image pixel values (uint) for specified mip and slice
        //! The visitor function `callback` is invoked on each pixel specified by the region
        //! NOTE: The topLeft coordinate is inclusive, whereas the bottomRight is exclusive
        bool GetSubImagePixelValuesUint(const AZ::Data::Asset<AZ::RPI::StreamingImageAsset>& imageAsset, AZStd::pair<uint32_t, uint32_t> topLeft, AZStd::pair<uint32_t, uint32_t> bottomRight, AZStd::function<void(const AZ::u32& x, const AZ::u32& y, const AZ::u32& value)> callback, uint32_t componentIndex = 0, uint32_t mip = 0, uint32_t slice = 0);

        //! Process a region of image pixel values (int) for specified mip and slice
        //! The visitor function `callback` is invoked on each pixel specified by the region
        //! NOTE: The topLeft coordinate is inclusive, whereas the bottomRight is exclusive
        bool GetSubImagePixelValuesInt(const AZ::Data::Asset<AZ::RPI::StreamingImageAsset>& imageAsset, AZStd::pair<uint32_t, uint32_t> topLeft, AZStd::pair<uint32_t, uint32_t> bottomRight, AZStd::function<void(const AZ::u32& x, const AZ::u32& y, const AZ::s32& value)> callback, uint32_t componentIndex = 0, uint32_t mip = 0, uint32_t slice = 0);
