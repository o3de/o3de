...
namespace AZ
{
    namespace RPI
    {
        ...
        // Layout to store the value of a Material-Property (with a ShaderInput-Connections) in a StructuredBuffer on the GPU and/or an
        // entry in an SRG Usually generated from the Material-Properties of a material-type, and assumes the shaders use a struct with the
        // same layout in the shader.
        // It provides WriteMaterialParameterStructureAzsli() to generate an azsli - file containing a `struct
        // MaterialParameters` definition uisng the layout.
        ...
        public:
            ...
            const AZ::Name& GetName() { return m_name; }
            const MaterialShaderParameterLayout::Index GetIndex() const { return m_index; }
