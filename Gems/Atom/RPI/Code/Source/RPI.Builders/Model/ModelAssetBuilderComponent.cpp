    ...
    namespace RPI
    {
        ...
        SceneAPI::Events::ProcessingResult ModelAssetBuilderComponent::BuildModel(ModelAssetBuilderContext& context)
        {
            ...
            if (lodRule)
            {
                ...
                autoGenerateLods = lodRule->GetAutoGenerateLods();
            ...
            {
                if (lodIndex >= sourceMeshContentListsByLod.size() && !autoGenerateLods)
                {
                    break;
                }
...

                {
                    ...
                    if (lodIndex >= sourceMeshContentListsByLod.size())
                    {
                        ...
                        for (ProductMeshContent& mesh : lodMeshes)
                        {
                            AZStd::vector<uint32_t> newIndices;
                            newIndices.resize(mesh.m_indices.size());

                            float targetSimplifyThreshold = lodRule->GetSimplifyIndexThreshold();

                            float targetError = FLT_MAX;
                            if (lodRule->GetSimplifyLimitError())
                            {
                                targetError = lodRule->GetSimplifyTargetError();
                            }

                            AZ::u32 simplifyOption = 0;

                            if (lodRule->GetSimplifyLockBorder())
                            {
                                simplifyOption |= meshopt_SimplifyLockBorder;
                            }

                            if (lodRule->GetSimplifySparse())
                            {
                                simplifyOption |= meshopt_SimplifySparse;
                            }

                            if (lodRule->GetSimplifyPrune())
                            {
                            ...
                            if (lodRule->GetSimplifyPreserveTopology())
                            {
                            ...
                            if (AZ::SceneAPI::Utilities::IsDebugEnabled())
                            {
                                AZ_Info(s_builderName, "      meshopt_simplify result: indices %d, result error %f\n", 
