            ...
            for (auto& contractStreamChannel : contract.m_streamChannels)
            {
                ...
                if (iter == mesh.m_streamInfo.end())
                {
                    if (contractStreamChannel.m_isOptional)
                    {
                        ...
                        RHI::StreamBufferView dummyBuffer{
                            *mesh.m_indexBufferView.GetBuffer(), 0, 0, RHI::GetFormatSize(dummyStreamFormat)
                        };
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        RHI::StreamBufferView bufferView(
                            *m_buffers[iter->m_bufferIndex]->GetRHIBuffer(), iter->m_byteOffset, iter->m_byteCount, iter->m_stride);
