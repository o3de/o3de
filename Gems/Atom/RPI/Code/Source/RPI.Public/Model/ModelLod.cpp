        ...
        RHI::ResultCode ModelLod::Init(const Data::Asset<ModelLodAsset>& lodAsset, const Data::Asset<ModelAsset>& modelAsset)
        {
            ...
            for (const ModelLodAsset::Mesh& mesh : lodAsset->GetMeshes())
            {
                ...
                if (indexBufferAsset)
                {
                    ...
                    meshInstance.m_indexBufferView = RHI::DeviceIndexBufferView(
            ...
            for (auto& contractStreamChannel : contract.m_streamChannels)
            {
                ...
                if (iter == mesh.m_streamInfo.end())
                {
                    if (contractStreamChannel.m_isOptional)
                    {
                        ...
                        RHI::DeviceStreamBufferView dummyBuffer{*mesh.m_indexBufferView.GetBuffer(), 0, 0, RHI::GetFormatSize(dummyStreamFormat)};
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        RHI::DeviceStreamBufferView bufferView(*m_buffers[iter->m_bufferIndex]->GetRHIBuffer(), iter->m_byteOffset, iter->m_byteCount, iter->m_stride);
