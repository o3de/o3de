...
namespace AZ
{
    namespace RPI
    {
        ...
        void Buffer::WaitForUpload()
        {
            if (m_streamFence)
            {
                [[maybe_unused]] auto deviceCount{RHI::RHISystemInterface::Get()->GetDeviceCount()};
                auto deviceMask{m_streamFence->GetDeviceMask()};

                //for (
                auto deviceIndex{0};// deviceIndex < deviceCount; ++deviceCount)
                {
                    if ((AZStd::to_underlying(deviceMask) >> deviceIndex) & 1 != 0)
                    {
