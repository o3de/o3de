            ...
            GetImageAssetResult GetImageAssetReference(Data::Asset<ImageAsset>& imageAsset, AZStd::string_view materialSourceFilePath, const AZStd::string imageFilePath)
            {
                ...
                else
                {
                    ...
                    if (!imageAssetId.IsSuccess())
                    {
                        ...
                        // So we use a a UUID that is clearly invalid, which the runtime and tools will interpret as a missing asset and represent
                        // it as such.
                        static constexpr Uuid InvalidAssetPlaceholderId{ "{00000000-0000-0000-0000-000000000000}" };
