...
namespace AZ::Render
{
    ...
    auto MeshInfoManager::AcquireMeshInfoEntry() -> MeshInfoHandle
    {
        ...
        const auto numEntries = AlignUpToPowerOfTwo(AZStd::max(MeshInfoMinEntries, static_cast<size_t>(m_meshInfoIndices.Max())));

        if (m_meshInfoData.size() < m_meshInfoIndices.Max())
        {
        ...
        MeshInfoEntry* entry)
    {
        ...
        for (int streamIndex = 0; streamIndex < inputContract.m_streamChannels.size(); ++streamIndex)
        {
            ...
            auto& streamBuffer = const_cast<RHI::StreamBufferView&>(streamIter[streamIndex]);
            entry->m_meshBuffers[semantic] = BufferViewIndexAndOffset::Create(streamBuffer, inputChannelFormat[streamIndex]);
        }
