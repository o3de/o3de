...
namespace AZ
{
    namespace Render
    {
        RPI::Ptr<FullscreenShadowComputePass> FullscreenShadowComputePass::Create(const RPI::PassDescriptor& descriptor)
        {
            RPI::Ptr<FullscreenShadowComputePass> pass = aznew FullscreenShadowComputePass(descriptor);
            return pass;
        }

        FullscreenShadowComputePass::FullscreenShadowComputePass(const RPI::PassDescriptor& descriptor)
            : Base(descriptor)
            , m_outputName("Output")
        ...
        void FullscreenShadowComputePass::FrameBeginInternal(FramePrepareParams params)
        {
            SetConstantData();
            Base::FrameBeginInternal(params);
        }

        RHI::Size FullscreenShadowComputePass::GetDepthBufferDimensions()
        {
        ...
        int FullscreenShadowComputePass::GetDepthBufferMSAACount()
        {
            RPI::PassAttachmentBinding* outputBinding = RPI::Pass::FindAttachmentBinding(m_outputName);
            return outputBinding->GetAttachment()->m_descriptor.m_image.m_multisampleState.m_samples;
        }

        void FullscreenShadowComputePass::SetConstantData()
        {
            ...
            AZ_Assert(setOk, "FullscreenShadowComputePass::SetConstantData() - could not set constant data");
        }
    }
}
