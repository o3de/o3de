    ...
    namespace Render
    {
        ...
        void MorphTargetComputePass::BuildCommandListInternal(const RHI::FrameGraphExecuteContext& context)
        {
            if (m_skinnedMeshFeatureProcessor)
            {
                ...
                m_skinnedMeshFeatureProcessor->SubmitMorphTargetDispatchItems(
                    commandList, context.GetDeviceIndex(), context.GetSubmitRange().m_startIndex, context.GetSubmitRange().m_endIndex);
            }
        }
    }   // namespace Render
}   // namespace AZ
