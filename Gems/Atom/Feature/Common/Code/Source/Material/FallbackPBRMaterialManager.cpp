...
namespace AZ::Render
{
    namespace GPU
    {
        ...
        // material data, must match the structure in FallbackPBRMaterial.azsli
        struct alignas(16) MaterialInfo
        {
            AZStd::array<float, 4> m_baseColor;

            AZStd::array<float, 4> m_irradianceColor;

            AZStd::array<float, 3> m_emissiveColor;
            ...
            int m_emissiveImage;
            uint32_t m_reflectionProbeCubeMapIndex;
            uint32_t m_pad;
    ...
    namespace FallbackPBR
    {
        ...
        void MaterialManager::ConvertMaterial(RPI::Material* material, MaterialParameters& convertedMaterial)
        {
            ...
            if (emissiveEnable)
            {
                ...
                uint32_t unit = util.GetProperty<float>(AZ_NAME_LITERAL("emissive.unit"), 0);
        ...
        void MaterialManager::OnCreateMeshInfoEntry(
            const MeshInfoHandle meshInfoHandle, ModelDataInstanceInterface* modelData, const size_t lodIndex, const size_t lodMeshIndex)
        {
            ...
            auto entry = aznew FallbackPBR::MaterialEntry{};
            entry->m_objectId = modelData->GetObjectId();
            entry->m_material = material;
            entry->m_materialChangeId = RPI::Material::DEFAULT_CHANGE_ID;
            m_materialData[meshInfoHandle.GetIndex()] = AZStd::move(entry);
        }

        void MaterialManager::OnDeleteMeshInfoEntry(const MeshInfoHandle meshInfoHandle)
        {
            if (!m_isEnabled)
            {
                return;
        ...
        void MaterialManager::UpdateFallbackPBRMaterialBuffer()
        {
            ...
            const auto numEntries = AZStd::max(m_materialData.size(), 1ull);
