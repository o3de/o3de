...
namespace AtomToolsFramework
{
    ...
    {
        // Most slots have no instructions, so skip building expressions that would run zero times.
        if (container.empty())
        {
            return;
        }
    ...
    {
        // Compiled once per process (this runs for every node and slot); const QRegularExpression is safe from parallel_for_each.
