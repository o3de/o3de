        ...
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            ...
            if (auto editContext = serializeContext->GetEditContext())
            {
                editContext->Class<DynamicNodeSlotConfig>("DynamicNodeSlotConfig", "Configuration settings for individual slots on a dynamic node.")
                    ...
                    ->DataElement(AZ::Edit::UIHandlers::Default, &DynamicNodeSlotConfig::m_supportsEditingOnNode, "Supports Eeiting On Node", "Enable this to allow editing the the slot value directly on the node UI.")
