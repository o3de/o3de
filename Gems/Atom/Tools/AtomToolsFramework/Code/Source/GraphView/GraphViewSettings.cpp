...
namespace AtomToolsFramework
{
    void GraphViewSettings::Reflect(AZ::ReflectContext* context)
    {
        if (auto serialize = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serialize->Class<GraphViewSettings>()
                ->Version(0)
                ->Field("styleManagerPath", &GraphViewSettings::m_styleManagerPath)
                ->Field("translationPath", &GraphViewSettings::m_translationPath)
                ->Field("nodeMimeType", &GraphViewSettings::m_nodeMimeType)
                ->Field("nodeSaveIdentifier", &GraphViewSettings::m_nodeSaveIdentifier)
            ...
            if (auto editContext = serialize->GetEditContext())
            {
                ...
                editContext->Class<GraphViewSettings>("Graph View Config", "")
                    ...
                    ->ClassElement(AZ::Edit::ClassElements::Group, "Moving")
                    ->Attribute(AZ::Edit::Attributes::AutoExpand, true)
                    ...
                    ->Attribute(AZ::Edit::Attributes::AutoExpand, true)
                    ->DataElement(AZ::Edit::UIHandlers::Default, &GraphViewSettings::m_connectionCurveType, "Connection Curve Type", "")
                    ->DataElement(AZ::Edit::UIHandlers::Default, &GraphViewSettings::m_dataConnectionCurveType, "Data Connection Curve Type", "")
    ...
    void GraphViewSettings::Initialize(const AZ::Crc32& toolId)
    {
        m_toolId = toolId;
