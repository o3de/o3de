...
namespace AZ
{
    namespace DX12
    {
        ...
        AZStd::vector<DescriptorHandle> ShaderResourceGroupPool::GetCBVsFromBufferViews(
        ...
        void ShaderResourceGroupPool::UpdateViewsDescriptorTable(
            ...
            if (forceUpdateViews || group.IsResourceTypeEnabledForCompilation(static_cast<uint32_t>(ResourceMask::BufferViewMask)))
            {
                for (const RHI::ShaderInputBufferDescriptor& shaderInputBuffer : groupLayout.GetShaderInputListForBuffers())
                {
                    ...
                    switch (descriptorRangeType)
                    {
                        ...
                        {
                            descriptorHandles =
                                GetSRVsFromImageViews<RHI::DeviceBufferView, BufferView>(bufferViews, D3D12_SRV_DIMENSION_BUFFER);
                            break;
                        }
                        case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
                        {
                            descriptorHandles =
                                GetUAVsFromImageViews<RHI::DeviceBufferView, BufferView>(bufferViews, D3D12_UAV_DIMENSION_BUFFER);
                            break;
                        }
            ...
            if (forceUpdateViews || group.IsResourceTypeEnabledForCompilation(static_cast<uint32_t>(ResourceMask::ImageViewMask)))
            {
                ...
                for (const RHI::ShaderInputImageDescriptor& shaderInputImage : groupLayout.GetShaderInputListForImages())
                {
                    ...
                    switch (descriptorRangeType)
                    {
                        case D3D12_DESCRIPTOR_RANGE_TYPE_SRV:
                        {
                            descriptorHandles = GetSRVsFromImageViews<RHI::DeviceImageView, ImageView>(
                                imageViews, ConvertSRVDimension(shaderInputImage.m_type));
                            break;
                        }
                        case D3D12_DESCRIPTOR_RANGE_TYPE_UAV:
                        {
                            descriptorHandles = GetUAVsFromImageViews<RHI::DeviceImageView, ImageView>(
                                imageViews, ConvertUAVDimension(shaderInputImage.m_type));
                            break;
                        }
                        default:
                        AZ_Assert(false, "Unhandled D3D12_DESCRIPTOR_RANGE_TYPE enumeration");
        ...
        void ShaderResourceGroupPool::UpdateSamplersDescriptorTable(
            DescriptorTable descriptorTable, RHI::DeviceShaderResourceGroup& group, const RHI::DeviceShaderResourceGroupData& groupData)
        {
            if (group.IsResourceTypeEnabledForCompilation(
                    static_cast<uint32_t>(RHI::DeviceShaderResourceGroupData::ResourceTypeMask::SamplerMask)))
            {
                const RHI::ShaderResourceGroupLayout& groupLayout = *groupData.GetLayout();
        ...
        void ShaderResourceGroupPool::UpdateUnboundedArrayDescriptorTables(
            ShaderResourceGroup& group, const RHI::DeviceShaderResourceGroupData& groupData)
        {
            const RHI::ShaderResourceGroupLayout& groupLayout = *groupData.GetLayout();
            auto& device = static_cast<Device&>(GetDevice());
            uint32_t shaderInputIndex = 0;
