...
namespace AZ
{
    namespace RHI
    {
        ...
        struct DeviceIndirectArguments
        {
            DeviceIndirectArguments() = default;

            DeviceIndirectArguments(
                uint32_t maxSequenceCount, const DeviceIndirectBufferView& indirectBuffer, uint64_t indirectBufferByteOffset)
                : DeviceIndirectArguments(maxSequenceCount, indirectBuffer, indirectBufferByteOffset, nullptr, 0)
            {}
            ...
            const DeviceIndirectBufferView* m_indirectBufferView = nullptr;

            //! Optional count buffer that contains the number of indirect commands in the indirect buffer.
            const DeviceBuffer* m_countBuffer = nullptr;
        };
    }
}
