...
namespace AZ
{
    namespace RHI
    {
        ...
        public:
            ...
            /// Builds a Bottom Level Acceleration Structure (BLAS) for ray tracing operations, which is made up of RayTracingGeometry
            /// entries
            virtual void BuildBottomLevelAccelerationStructure(const RHI::DeviceRayTracingBlas& rayTracingBlas) = 0;

            /// Builds a Top Level Acceleration Structure (TLAS) for ray tracing operations, which is made up of RayTracingInstance entries
            /// that refer to a BLAS entry
            virtual void BuildTopLevelAccelerationStructure(const RHI::DeviceRayTracingTlas& rayTracingTlas) = 0;
        ...
        inline void CommandList::ValidateSubmitIndex([[maybe_unused]] uint32_t submitIndex)
        {
            ...
            {
                AZ_Assert(
                    (submitIndex >= m_submitRange.m_startIndex) && (submitIndex < m_submitRange.m_endIndex),
