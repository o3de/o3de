    ...
    namespace RHI
    {
        ...
        struct IndirectArguments
        {
            ...
            IndirectArguments(uint32_t maxSequenceCount, const IndirectBufferView& indirectBuffer, uint64_t indirectBufferByteOffset)
                : IndirectArguments(maxSequenceCount, indirectBuffer, indirectBufferByteOffset, nullptr, 0)
            {}
            ...
            DeviceIndirectArguments GetDeviceIndirectArguments(int deviceIndex) const
            {
                return DeviceIndirectArguments{ m_maxSequenceCount,
                                                m_indirectBufferView->GetDeviceIndirectBufferView(deviceIndex),
                                                m_indirectBufferByteOffset,
                                                m_countBuffer->GetDeviceBuffer(deviceIndex).get(),
