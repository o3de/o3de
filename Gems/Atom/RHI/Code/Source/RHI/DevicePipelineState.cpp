...
namespace AZ
{
    namespace RHI
    {
        ...
        ResultCode DevicePipelineState::Init(
            Device& device, const PipelineStateDescriptorForDraw& descriptor, DevicePipelineLibrary* pipelineLibrary)
        {
            ...
            if (Validation::IsEnabled())
            {
                ...
                if (renderTargetConfiguration.m_subpassIndex >= renderTargetConfiguration.m_renderAttachmentLayout.m_subpassCount)
                {
                    AZ_Error(
                        "DevicePipelineState",
                        false,
                        "Invalid subpassIndex %d. SubpassCount is %d.",
                        renderTargetConfiguration.m_subpassIndex,
                        renderTargetConfiguration.m_renderAttachmentLayout.m_subpassCount);
                    return ResultCode::InvalidOperation;
                }
        ...
        ResultCode DevicePipelineState::Init(
            Device& device, const PipelineStateDescriptorForDispatch& descriptor, DevicePipelineLibrary* pipelineLibrary)
        {
            if (!ValidateNotInitialized())
            {
                return ResultCode::InvalidOperation;
        ...
        ResultCode DevicePipelineState::Init(
            Device& device, const PipelineStateDescriptorForRayTracing& descriptor, DevicePipelineLibrary* pipelineLibrary)
        {
            if (!ValidateNotInitialized())
            {
                return ResultCode::InvalidOperation;
