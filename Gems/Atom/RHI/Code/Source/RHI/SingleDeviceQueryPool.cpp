...
namespace AZ::RHI
{   
    ResultCode SingleDeviceQueryPool::Init(Device& device, const QueryPoolDescriptor& descriptor)
    {
        if (Validation::IsEnabled())
        {
            if (!descriptor.m_queriesCount)
            {
                AZ_Error("RHI", false, "SingleDeviceQueryPool size can't be zero");
    ...
    ResultCode SingleDeviceQueryPool::InitQuery(SingleDeviceQuery* query)
    {
        return InitQuery(&query, 1);
    }

    ResultCode SingleDeviceQueryPool::InitQuery(SingleDeviceQuery** queries, uint32_t queryCount)
    {
    ...
    ResultCode SingleDeviceQueryPool::ValidateQueries(SingleDeviceQuery** queries, uint32_t queryCount)
    {
        if (!queryCount)
        {
            AZ_Error("RHI", false, "SingleDeviceQuery count is 0");
        ...
        for (uint32_t i = 0; i < queryCount; ++i)
        {
            ...
            {
                AZ_Error("RHI", false, "SingleDeviceQuery does not belong to this pool");
                return RHI::ResultCode::InvalidArgument;
            }
    ...
    ResultCode SingleDeviceQueryPool::GetResults(SingleDeviceQuery* query, uint64_t* result, uint32_t resultsCount, QueryResultFlagBits flags)
    {
        return GetResults(&query, 1, result, resultsCount, flags);
    }

    ResultCode SingleDeviceQueryPool::GetResults(SingleDeviceQuery** queries, uint32_t queryCount, uint64_t* results, uint32_t resultsCount, QueryResultFlagBits flags)
    {
        ...
        AZStd::vector<SingleDeviceQuery*> sortedQueries(queries, queries + queryCount);
        ...
        {
            SingleDeviceQuery* sortedQuery = sortedQueries[i];
    ...
    ResultCode SingleDeviceQueryPool::GetResults(uint64_t* results, uint32_t resultsCount, QueryResultFlagBits flags)
    {
        AZ_Assert(resultsCount <= m_queries.size(), "Invalid size for writing the query results");
        AZStd::vector<SingleDeviceQuery*> queries = GetQueries();
        return GetResults(queries.data(), static_cast<uint32_t>(queries.size()), results, resultsCount, flags);
    }

    const QueryPoolDescriptor& SingleDeviceQueryPool::GetDescriptor() const
    {
        return m_descriptor;
    }

    const SingleDeviceQuery* SingleDeviceQueryPool::GetQuery(QueryHandle handle) const
    {
        AZStd::unique_lock<AZStd::mutex> lock(m_queriesMutex);
        return m_queries[handle.GetIndex()];
    }

    SingleDeviceQuery* SingleDeviceQueryPool::GetQuery(QueryHandle handle)
    {
        AZStd::unique_lock<AZStd::mutex> lock(m_queriesMutex);
        return m_queries[handle.GetIndex()];
    }

    void SingleDeviceQueryPool::ShutdownInternal()
    {
        AZStd::unique_lock<AZStd::mutex> lock(m_queriesMutex);
        m_queries.clear();
    }
    ...
    AZStd::vector<SingleDeviceQuery*> SingleDeviceQueryPool::GetQueries()
    {
        AZStd::vector<SingleDeviceQuery*> queries;
        AZStd::unique_lock<AZStd::mutex> lock(m_queriesMutex);
        AZStd::for_each(m_queries.begin(), m_queries.end(), [&queries](SingleDeviceQuery* query)
        {
            if (query)
            {
