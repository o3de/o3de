...
namespace AZ
{
    namespace RHI
    {
        ResultCode DeviceIndirectBufferWriter::Init(
            DeviceBuffer* buffer,
            size_t byteOffset,
            uint32_t byteStride,
        ...
        ResultCode DeviceIndirectBufferWriter::Init(
            ...
            void* memoryPtr, uint32_t byteStride, uint32_t maxCommandSequences, const DeviceIndirectBufferSignature& signature)
        {
            if (!ValidateInitializedState(ValidateInitializedStateExpect::NotInitialized))
            {
                return ResultCode::InvalidOperation;
        ...
        bool DeviceIndirectBufferWriter::ValidateArguments(

            uint32_t byteStride, uint32_t maxCommandSequences, const DeviceIndirectBufferSignature& signature) const
        {
            if (Validation::IsEnabled())
            {
                ...
                if (signature.GetByteStride() > byteStride)
                {
                    AZ_Assert(
                        false,
                        "Byte stride (%d) is smaller than the minimum required stride (%d)",
                        int(byteStride),
                        int(signature.GetByteStride()));
                    return false;
                }
        ...
        bool DeviceIndirectBufferWriter::ValidateRootConstantsCommand(IndirectCommandIndex index, uint32_t byteSize) const
        {
            if (Validation::IsEnabled())
            {
                ...
                uint32_t nextCommandOffset =
                    isLastCommand ? m_signature->GetByteStride() : m_signature->GetOffset(IndirectCommandIndex(index.GetIndex() + 1));
        ...
        DeviceIndirectBufferWriter* DeviceIndirectBufferWriter::SetVertexView(uint32_t slot, const DeviceStreamBufferView& view)
        {
            ...
            {
                IndirectCommandIndex index =
                    m_signature->GetLayout().FindCommandIndex(IndirectCommandDescriptor(IndirectBufferViewArguments{ slot }));
        ...
        {
            if (ValidateInitializedState(ValidateInitializedStateExpect::Initialized) && m_buffer && m_targetMemory)
            {
