...
namespace AZ
{
    namespace RHI
    {
        ...
        const DrawPacket* DrawPacketBuilder::End()
        {
            ...
            const VirtualAddress drawItemsOffset =
                linearAllocator.Allocate(sizeof(DrawItem) * m_drawRequests.size(), AZStd::alignment_of<DrawItem>::value);
            ...
            const VirtualAddress shaderResourceGroupsOffset = linearAllocator.Allocate(
                sizeof(const ShaderResourceGroup*) * m_shaderResourceGroups.size(), AZStd::alignment_of<const ShaderResourceGroup*>::value);

            const VirtualAddress uniqueShaderResourceGroupsOffset = linearAllocator.Allocate(
                sizeof(const ShaderResourceGroup*) * m_drawRequests.size(), AZStd::alignment_of<const ShaderResourceGroup*>::value);
            ...
            const VirtualAddress streamBufferViewsOffset =
                linearAllocator.Allocate(sizeof(StreamBufferView) * m_streamBufferViewCount, AZStd::alignment_of<StreamBufferView>::value);
            ...
            {
                auto shaderResourceGroups =
                    reinterpret_cast<const ShaderResourceGroup**>(allocationData + shaderResourceGroupsOffset.m_ptr);
                for (size_t i = 0; i < m_shaderResourceGroups.size(); ++i)
                {
            ...
            {
                auto shaderResourceGroups =
                    reinterpret_cast<const ShaderResourceGroup**>(allocationData + uniqueShaderResourceGroupsOffset.m_ptr);
