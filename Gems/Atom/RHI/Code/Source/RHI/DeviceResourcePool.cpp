    ...
    namespace RHI
    {
        ...
        bool DeviceResourcePool::ValidateIsUnregistered(const DeviceResource* resource) const
        {
            if (Validation::IsEnabled())
            {
                ...
                {
                    AZ_Error(
                        "DeviceResourcePool", false, "'%s': DeviceResource is null or registered on another pool.", GetName().GetCStr());
                    return false;
                }
            }
            ...
            if (Validation::IsEnabled())
            {
                if (m_isProcessingFrame)
                {
                    AZ_Error(
                        "DeviceResourcePool",
                        false,
                        "'%s' Attempting an operation that is invalid when processing the frame.",
                        GetName().GetCStr());
                    return false;
                }
            }
        ...
        ResultCode DeviceResourcePool::Init(
            Device& device, const ResourcePoolDescriptor& descriptor, const PlatformMethod& platformInitMethod)
        {
            if (Validation::IsEnabled())
            {
                if (IsInitialized())
