...
namespace AZ
{
    namespace RHI
    {
        DeviceBufferInitRequest::DeviceBufferInitRequest(
            DeviceBuffer& buffer,
        ...
        DeviceBufferMapRequest::DeviceBufferMapRequest(
            DeviceBuffer& buffer,
            size_t byteOffset,
            size_t byteCount)
            : m_buffer{&buffer}
        ...
        bool DeviceBufferPool::ValidateInitRequest(const DeviceBufferInitRequest& initRequest) const
        {
            if (Validation::IsEnabled())
            {
                ...
                {
                    AZ_Error("DeviceBufferPool", false, "Buffer bind flags don't match pool bind flags in pool '%s'", GetName().GetCStr());
                    return false;
                }
        ...
        bool DeviceBufferPool::ValidateMapRequest(const DeviceBufferMapRequest& request) const
        {
            if (Validation::IsEnabled())
            {
                ...
                {
                    AZ_Warning(
                        "DeviceBufferPool", false, "Trying to map zero bytes from buffer '%s'.", request.m_buffer->GetName().GetCStr());
                    return false;
                }
        ...
        void DeviceBufferPool::OnFrameBegin()
        {
            if (Validation::IsEnabled())
            {
                AZ_Error(
                    "DeviceBufferPool",
                    GetMapRefCount() == 0 || GetDescriptor().m_heapMemoryLevel != HeapMemoryLevel::Device,
                    "There are currently buffers mapped on buffer pool '%s'. All buffers must be "
                    "unmapped when the frame is processing.",
                    GetName().GetCStr());
            }
