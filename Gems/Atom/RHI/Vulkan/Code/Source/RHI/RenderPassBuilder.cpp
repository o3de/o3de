    ...
    namespace Vulkan
    {
        ...
        void RenderPassBuilder::AddScopeAttachments(const Scope& scope)
        {
            ...
            for (const RenderAttachment& attachment : renderAttachments)
            {
                ...
                const ImageView* attachmentImageView =
                    static_cast<const ImageView*>(scopeAttachment->GetImageView()->GetDeviceImageView(0).get());
                ...
                case RHI::ScopeAttachmentUsage::RenderTarget:
                {
                    ...
                    if (findIter == m_attachmentsMap.end())
                    {
                        ...
                        m_framebufferDesc.m_attachmentImageViews.push_back(static_cast<const ImageView*>(
                            scopeAttachment->GetImageView()->GetDeviceImageView(scope.GetDeviceIndex()).get()));
            ...
            for (size_t index = 0; index < scope.GetBufferAttachments().size(); ++index)
            {
                ...
                const BufferView* bufferView =
                    static_cast<const BufferView*>(scopeAttachment->GetBufferView()->GetDeviceBufferView(scope.GetDeviceIndex()).get());
        ...
        VkImageLayout RenderPassBuilder::GetInitialLayout(const Scope& scope, const RHI::ImageScopeAttachment& attachment) const
        {
            ...
            const ImageView* imageView =
                static_cast<const ImageView*>(attachment.GetImageView()->GetDeviceImageView(scope.GetDeviceIndex()).get());
