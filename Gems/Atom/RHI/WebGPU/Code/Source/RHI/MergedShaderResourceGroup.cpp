...
namespace AZ::WebGPU
{
    ...
    const char* MergedShaderResourceGroup::RootConstantBufferName = "RootConstantsBuffer";    
    ...
    RHI::DeviceShaderResourceGroupData MergedShaderResourceGroup::MergeShaderResourceData(
        const ShaderResourceGroupArray& srgList, const BufferView* rootConstantBufferView) const
    {
        ...
        if (rootConstantBufferView)
        {
            RHI::ShaderInputBufferIndex shaderInputIndex =
                srgLayout->FindShaderInputBufferIndex(GenerateMergedShaderInputName(AZ::Name(RootConstantBufferName)));
            if (!shaderInputIndex.IsValid())
            {
                AZ_Assert(
                    false, "Failed to find merged shader input for Root Constants %s", RootConstantBufferName);
            }
            else
            {
                data.SetBufferView(shaderInputIndex, rootConstantBufferView);
            }
        }
    ...
    bool MergedShaderResourceGroup::NeedsCompile(const BufferView* rootConstantBufferView) const
    {
        ...
        if (m_rootConstantBufferView  || rootConstantBufferView)
        {
            if (m_rootConstantBufferView && rootConstantBufferView)
            {
                return &m_rootConstantBufferView->GetBuffer() != &rootConstantBufferView->GetBuffer() ||
                    m_rootConstantBufferView->GetDescriptor() != rootConstantBufferView->GetDescriptor();
            }
            return true;
        }
    ...
    void MergedShaderResourceGroup::Compile(const BufferView* rootConstantBufferView)
    {
        AZStd::unique_lock<AZStd::shared_mutex> lock(m_compileMutex);
        RHI::DeviceShaderResourceGroupData srgData = MergeShaderResourceData(m_mergedShaderResourceGroupList, rootConstantBufferView);
        ...
        m_rootConstantBufferView = rootConstantBufferView;
    }
}
