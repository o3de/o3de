...
namespace AZ::WebGPU
{
    ...
    {
        // WebGPU doesn't support multithreading yet
        RHI::JobPolicy graphJobPolicy = RHI::JobPolicy::Serial;
    ...
    RHI::ResultCode FrameGraphExecuter::InitInternal(const RHI::FrameGraphExecuterDescriptor& descriptor)
    {
        for (auto& [deviceIndex, platformLimitsDescriptor] : descriptor.m_platformLimitsDescriptors)
        {
            ...
            if (RHI::ConstPtr<PlatformLimitsDescriptor> webGPUPlatformLimitsDesc =
                    azrtti_cast<const PlatformLimitsDescriptor*>(rhiPlatformLimitsDescriptor))
            {
                m_frameGraphExecuterData[deviceIndex] = webGPUPlatformLimitsDesc->m_frameGraphExecuterData;
            }
        }
    ...
    void FrameGraphExecuter::BeginInternal(const RHI::FrameGraph& frameGraph)
    {
        ...
        for (RHI::Scope* scopeBase : frameGraph.GetScopes())
        {
            ...
            // Detect if we are able to continue merging.
            {
                ...
                const bool exceededSwapChainLimit =
                    (mergedSwapchainCount + swapchainCount) > m_frameGraphExecuterData[scope.GetDeviceIndex()].m_swapChainsPerCommandList;
                ...
                const bool flushMergedScopes =
                    exceededCommandCost || exceededSwapChainLimit || hardwareQueueMismatch || onFenceBoundaries || deviceMismatch;
        ...
        if (mergedScopes.size())
        {
            ...
            multiScopeContextGroup->Init(
                static_cast<Device&>(mergedScopes.front()->GetDevice()), AZStd::move(mergedScopes), m_mergedScopeId);
        }
#endif
    }
    ...
    void FrameGraphExecuter::ExecuteGroupInternal(RHI::FrameGraphExecuteGroup& groupBase)
    {
        ...
        static_cast<Device*>(&group.GetDevice())
            ->GetCommandQueueContext()
            .ExecuteWork(group.GetHardwareQueueClass(), group.MakeWorkRequest());
    }
}
