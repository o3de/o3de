...
#include <WebGPU_Traits_Platform.h>

namespace AZ
{
    namespace WebGPU
    {
        [[maybe_unused]] static const char* WebGPUShaderPlatformName = "WebGPUShaderPlatform";
        static const char* WindowsAzslShaderHeader = "Builders/ShaderHeaders/Platform/Windows/WebGPU/AzslcHeader.azsli";
        static const char* MacAzslShaderHeader = "Builders/ShaderHeaders/Platform/Mac/WebGPU/AzslcHeader.azsli";
        static const char* LinuxAzslShaderHeader = "Builders/ShaderHeaders/Platform/Linux/WebGPU/AzslcHeader.azsli";
        static const char* WindowsHlslPlatformHeader = "Builders/ShaderHeaders/Platform/Windows/WebGPU/PlatformHeader.hlsli";
        static const char* MacHlslPlatformHeader = "Builders/ShaderHeaders/Platform/Mac/WebGPU/PlatformHeader.hlsli";
        static const char* LinuxHlslPlatformHeader = "Builders/ShaderHeaders/Platform/Linux/WebGPU/PlatformHeader.hlsli";
        ...
        {
            return AZ::WebGPU::PipelineLayoutDescriptor::Create();
        }
            ...
            [[maybe_unused]] const bool useSpecializationConstants) const
        {
            ...
            {
                AZ_Error(WebGPUShaderPlatformName, false, "Failed to cross-compile HLSL shader to WGSL");
                return false;
            }
            ...
            {
                AZ_Error(WebGPUShaderPlatformName, false, "Compiled shader for %s is invalid", shaderSourcePath.c_str());
                return false;
            }
            ...
            ByProducts& byProducts) const
        {
            ...
            {
                AZ_Error(WebGPUShaderPlatformName, false, "Unsupported shader stage");
                return false;
            }
            ...
            if (platform.m_identifier == "pc")
            {
                prependFile = WindowsHlslPlatformHeader;
            }
            else if (platform.m_identifier == "mac")
            {
                prependFile = MacHlslPlatformHeader;
            }
            else if (platform.m_identifier == "linux")
            {
                prependFile = LinuxHlslPlatformHeader;
            }
            else
            {
                prependFile = WindowsHlslPlatformHeader;
            ...
            const auto dxcEntryPoint =
                (shaderStageType == RHI::ShaderHardwareStage::RayTracing) ? "" : AZStd::string::format("-E %s", entryPoint.c_str());
            ...
            // There might be a more ideal way to count the number of dynamic branches, perhaps using DXC libs, but doing it this way is
            // quick and easy to set up.
            auto objectCodeLoadResult = AZ::RHI::LoadFileString(objectCodeOutputFile.c_str());
            if (objectCodeLoadResult)
            {
                // The regex here is based on https://www.khronos.org/registry/spir-v/specs/unified1/SPIRV.html#_control_flow, which lists
                // terminating instructions as:
                //    OpBranch
                //    OpBranchConditional
                //    OpSwitch
                //    OpReturn
                ...
                byProducts.m_dynamicBranchCount = aznumeric_cast<uint32_t>(
                    AZ::RHI::RegexCount(objectCodeLoadResult.GetValue(), "^ *(OpBranch|OpBranchConditional|OpSwitch) "));
            }
            else
            {
