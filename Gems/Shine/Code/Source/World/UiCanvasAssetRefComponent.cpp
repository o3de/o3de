...
AZ::EntityId UiCanvasAssetRefComponent::LoadCanvas()
{
    ...
    if (!canvasPath.empty())
    {
        ...
        {
            AZ::Interface<IShine>::Get()->ReleaseCanvasDeferred(m_canvasEntityId);
            m_canvasEntityId.SetInvalid();
        }

        m_canvasEntityId = AZ::Interface<IShine>::Get()->LoadCanvas(canvasPath.c_str());
...
{
    Shine::ShineRequestBus::Broadcast(&Shine::ShineRequests::EditUICanvas, GetCanvasPathname());
}
