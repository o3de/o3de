...
namespace
{
    ...
    void SetVertex(Shine::UiPrimitiveVertex& vert, const Vec2& pos, uint32 color, const Vec2& uv)
    {
        vert.xy = pos;
    ...
    void SetVertex(Shine::UiPrimitiveVertex& vert, const AZ::Vector2& pos, uint32 color, const AZ::Vector2& uv)
    {
    ...
    void FillVerts(Shine::UiPrimitiveVertex* verts, [[maybe_unused]] uint32 numVerts, uint32 numX, uint32 numY, uint32 packedColor, const AZ::Matrix4x4& transform,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void UiImageComponent::Render(Shine::IRenderGraph* renderGraph)
{
    ...
    if (m_cachedPrimitive.m_numVertices > 0)
    {
        ...
        if (m_cachedPrimitive.m_vertices[0].color.a != desiredPackedAlpha)
        {
            ...
            Shine::UCol desiredPackedColor = m_cachedPrimitive.m_vertices[0].color;
...
{
    if (AZ::Interface<IShine>::Get()->DoesSpriteTextureAssetExist(spritePath))
    {
    ...
    if (serializeContext)
    {
        ...
        if (ec)
        {
            ...
            editInfo->DataElement(AZ::Edit::UIHandlers::ComboBox, &UiImageComponent::m_blendMode, "BlendMode", "The blend mode used to draw the image")
                ->EnumAttribute(Shine::BlendMode::Normal, "Normal")
                ->EnumAttribute(Shine::BlendMode::Add, "Add")
                ->EnumAttribute(Shine::BlendMode::Screen, "Screen")
                ->EnumAttribute(Shine::BlendMode::Darken, "Darken")
                ->EnumAttribute(Shine::BlendMode::Lighten, "Lighten")
                ->Attribute(AZ::Edit::Attributes::ChangeNotify, &UiImageComponent::OnEditorRenderSettingChange);
...
void UiImageComponent::Init()
{
    ...
    if (!m_sprite)
    {
        if (IsSpriteTypeAsset())
        {
            ...
            {
                m_sprite = AZ::Interface<IShine>::Get()->LoadSprite(m_spritePathname.GetAssetPath().c_str());
            }
        }
        else if (m_spriteType == UiImageInterface::SpriteType::RenderTarget)
        {
            ...
            {
                m_sprite = AZ::Interface<IShine>::Get()->CreateSprite(m_attachmentImageAsset);
            }
        }
        else
        {
...
void UiImageComponent::RenderSingleQuad(const AZ::Vector2* positions, const AZ::Vector2* uvs, uint32 packedColor)
{
    ...
    Shine::UiPrimitiveVertex vertices[numVertices];
...
void UiImageComponent::RenderRadialFilledQuad(const AZ::Vector2* positions, const AZ::Vector2* uvs, uint32 packedColor)
{
    ...
    Shine::UiPrimitiveVertex verts[numVertices];
    for (int i = 1; i < 5; ++i)
    {
...
void UiImageComponent::RenderRadialCornerFilledQuad(const AZ::Vector2* positions, const AZ::Vector2* uvs, uint32 packedColor)
{
    ...
    Shine::UiPrimitiveVertex verts[numVerts];
    int vertexOffset = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void UiImageComponent::ClipAndRenderForSlicedRadialFill(uint32 numVertsPerSide, uint32 numVerts, const Shine::UiPrimitiveVertex* verts, uint32 totalIndices, const uint16* indices)
{
    ...
    Shine::UiPrimitiveVertex renderVerts[numIndicesIn9Slice * 4]; // ClipToLine doesn't check for duplicate vertices for speed, so this is the maximum we'll need.
    ...
    if (m_fillAmount < 0.5f)
    {
        ...
        {
            Shine::UiPrimitiveVertex intermediateVerts[maxTemporaryVerts];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void UiImageComponent::ClipAndRenderForSlicedRadialCornerOrEdgeFill(uint32 numVertsPerSide, uint32 numVerts, const Shine::UiPrimitiveVertex* verts, uint32 totalIndices, const uint16* indices)
{
    ...
    Shine::UiPrimitiveVertex renderVerts[numIndicesIn9Slice * 2]; // ClipToLine doesn't check for duplicate vertices for speed, so this is the maximum we'll need.
...
////////////////////////////////////////////////////////////////////////////////////////////////////
int UiImageComponent::ClipToLine(const Shine::UiPrimitiveVertex* vertices, const uint16* indices, Shine::UiPrimitiveVertex* renderVertices, uint16* renderIndices, int& vertexOffset, int renderIndexOffset, const Vec2& lineOrigin, const Vec2& lineEnd)
{
    Vec2 lineVector = lineEnd - lineOrigin;
    Shine::UiPrimitiveVertex lastVertex = vertices[indices[2]];
    Shine::UiPrimitiveVertex currentVertex;
    int verticesAdded = 0;
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        if (currentPointDeterminant < epsilon)
        {
            if (lastPointDeterminant > -epsilon && fabs(currentPointDeterminant) > epsilon && fabs(lastPointDeterminant) > epsilon)
            {
                ...
                Shine::UiPrimitiveVertex intersectPoint;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void UiImageComponent::RenderTriangleList(const Shine::UiPrimitiveVertex* vertices, const uint16* indices, int numVertices, int numIndices)
{
    if (numVertices != m_cachedPrimitive.m_numVertices)
    {
        ClearCachedVertices();
        m_cachedPrimitive.m_vertices = new Shine::UiPrimitiveVertex[numVertices];
    ...
    memcpy(m_cachedPrimitive.m_vertices, vertices, sizeof(Shine::UiPrimitiveVertex) * numVertices);
...
void UiImageComponent::OnSpritePathnameChange()
{
    ...
    if (!m_spritePathname.GetAssetPath().empty())
    {
        ...
        newSprite = AZ::Interface<IShine>::Get()->LoadSprite(m_spritePathname.GetAssetPath().c_str());
    }
...
void UiImageComponent::OnSpriteAttachmentImageAssetChange()
{
    ...
    if (m_attachmentImageAsset)
    {
        newSprite = AZ::Interface<IShine>::Get()->CreateSprite(m_attachmentImageAsset);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Shine::AZu32ComboBoxVec UiImageComponent::PopulateIndexStringList() const
{
    ...
    if (m_sprite)
    {
        ...
        if (numCells != 0)
        {
            return Shine::GetEnumSpriteIndexList(GetEntityId(), 0, numCells - 1);
        }
    }

    return Shine::AZu32ComboBoxVec();
}
    ...
    AZ::SerializeContext::DataElementNode& classElement)
{
    ...
    {
        if (!Shine::ConvertSubElementFromAzStringToAssetRef<LmbrCentral::TextureAsset>(context, classElement, "SpritePath"))
        {
            return false;
        }
    }
    ...
    {
        if (!Shine::RemoveLeadingForwardSlashesFromAssetPath(context, classElement, "SpritePath"))
        {
            return false;
        }
    }
    ...
    {
        if (!Shine::ConvertSubElementFromVector3ToAzColor(context, classElement, "Color"))
        {
            return false;
        }
    }
    ...
    {
        if (!Shine::RemoveRenderTargetAsString(context, classElement, "RenderTargetName"))
        {
            return false;
        }
    }
