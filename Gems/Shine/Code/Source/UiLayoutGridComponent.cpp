...
float UiLayoutGridComponent::GetTargetWidth(float maxWidth)
{
    ...
    int numColumns = 0;
    if (Shine::IsUiLayoutCellSizeSpecified(maxWidth))
    {
    ...
    AZ::SerializeContext::DataElementNode& classElement)
{
    ...
    {
        if (!Shine::ConvertSubElementFromVec2ToVector2(context, classElement, "Spacing"))
        {
            return false;
        }

        if (!Shine::ConvertSubElementFromVec2ToVector2(context, classElement, "CellSize"))
        {
            return false;
        }
    }
