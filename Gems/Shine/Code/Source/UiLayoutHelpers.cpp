...
namespace
{
    ...
    float GetLayoutCellTargetWidth(AZ::EntityId elementId, bool ignoreDefaultLayoutCells)
    {
        ...
        if (!Shine::IsUiLayoutCellSizeSpecified(value))
        {
            value = 0.0f;
        ...
        float minValue = Shine::UiLayoutCellUnspecifiedSize;
        UiLayoutCellBus::EventResult(minValue, elementId, &UiLayoutCellBus::Events::GetMinWidth);
        if (!Shine::IsUiLayoutCellSizeSpecified(minValue))
        {
            minValue = 0.0f;
            if (!ignoreDefaultLayoutCells)
            {
        ...
        if (Shine::IsUiLayoutCellSizeSpecified(maxValue) && maxValue < value)
        {
            value = maxValue;
        }
...
namespace UiLayoutHelpers
{
    ...
    LayoutCellSize::LayoutCellSize()
        : m_minSize(Shine::UiLayoutCellUnspecifiedSize)
        , m_targetSize(Shine::UiLayoutCellUnspecifiedSize)
        , m_maxSize(Shine::UiLayoutCellUnspecifiedSize)
        , m_extraSizeRatio(Shine::UiLayoutCellUnspecifiedSize)
    {
    }
    ...
    void GetLayoutCellWidths(AZ::EntityId elementId, bool ignoreDefaultLayoutCells, LayoutCellSizes& layoutCellsOut)
    {
        ...
        for (auto childEntityId : childEntityIds)
        {
            ...
            // If not overridden, get the default cell values
            if (!Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_minSize))
            {
            ...
            if (!Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_targetSize))
            {
            ...
            if (!Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_extraSizeRatio))
            {
            ...
            if (Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_maxSize) && layoutCell.m_maxSize < layoutCell.m_targetSize)
            {
        ...
        if (availableSize > 0.0f)
        {
            ...
            while (!cellsAcceptingExtraSize.empty())
            {
                ...
                for (auto& cellExtraSizeInfo : cellsAcceptingExtraSize)
                {
                    ...
                    if (Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_maxSize) && layoutCell.m_maxSize < newSize)
                    {
    ...
    float CalculateSingleElementSize(const LayoutCellSize& layoutCell, float availableSize)
    {
        ...
        else
        {
            ...
            if (Shine::IsUiLayoutCellSizeSpecified(layoutCell.m_maxSize) && layoutCell.m_maxSize < size)
            {
