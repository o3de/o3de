...
namespace Shine
{
    AZ::RPI::Ptr<ShinePass> ShinePass::Create(const AZ::RPI::PassDescriptor& descriptor)
    {
        return aznew ShinePass(descriptor);
    }

    ShinePass::ShinePass(const AZ::RPI::PassDescriptor& descriptor)
        : Base(descriptor)
    {
    }
    ...
    ShinePass::~ShinePass()
    {
        ShinePassRequestBus::Handler::BusDisconnect();
    }

    void ShinePass::ResetInternal()
    {
        ShinePassRequestBus::Handler::BusDisconnect();
    ...
    void ShinePass::SetRenderPipeline(AZ::RPI::RenderPipeline* pipeline)
    {
        if (pipeline == nullptr)
        {
            // The pipeline being set to null means this pass will soon be destroyed. Disconnect from the bus so if a
            // new ShinePass is being created to replace it, it will be able to connect.
            ShinePassRequestBus::Handler::BusDisconnect();
        }
        ParentPass::SetRenderPipeline(pipeline);
    }

    void ShinePass::BuildInternal()
    {
        AZ::RPI::Scene* scene = GetScene();
        if (scene)
        {
            // Listen for rebuild requests
            ShinePassRequestBus::Handler::BusConnect(scene->GetId());
        }
    ...
    void ShinePass::CreateChildPassesInternal()
    {
        AZ::RPI::Scene* scene = GetScene();
        if (scene)
        {
            // Get the current list of render targets being used across all loaded UI Canvases
            Shine::AttachmentImagesAndDependencies attachmentImagesAndDependencies;
            ShinePassDataRequestBus::EventResult(
                attachmentImagesAndDependencies,
                scene->GetId(),
                &ShinePassDataRequestBus::Events::GetRenderTargets
            );
    ...
    void ShinePass::RebuildRttChildren()
    {
        QueueForBuildAndInitialization();
    }

    AZ::RPI::RasterPass* ShinePass::GetRttPass(const AZStd::string& name)
    {
    ...
    AZ::RPI::RasterPass* ShinePass::GetUiCanvasPass()
    {
        return m_uiCanvasChildPass.get();
    }

    void ShinePass::AddRttChildPasses(Shine::AttachmentImagesAndDependencies attachmentImagesAndDependencies)
    {
    ...
    void ShinePass::AddRttChildPass(AZ::Data::Instance<AZ::RPI::AttachmentImage> attachmentImage, AttachmentImages attachmentImageDependencies)
    {
        ...
        if (attachmentImage)
        {
            ...
            AZ_Assert(rttChildPass, "[ShinePass] Unable to create a RttChildPass.");
    ...
    void ShinePass::AddUiCanvasChildPass(Shine::AttachmentImagesAndDependencies AttachmentImagesAndDependencies)
    {
        if (!m_uiCanvasChildPass)
        {
            AZ::RPI::PassSystemInterface* passSystem = AZ::RPI::PassSystemInterface::Get();
            m_uiCanvasChildPass = azrtti_cast<ShineChildPass*>(passSystem->CreatePassFromTemplate(AZ::Name("ShineChildPassTemplate"), AZ::Name("ShineChildPass")).get());
            AZ_Assert(m_uiCanvasChildPass, "[ShinePass] Unable to create a ShineChildPass.");
        }
    ...
    AZ::RPI::Ptr<ShineChildPass> ShineChildPass::Create(const AZ::RPI::PassDescriptor& descriptor)
    {
        return aznew ShineChildPass(descriptor);
    }

    ShineChildPass::ShineChildPass(const AZ::RPI::PassDescriptor& descriptor)
        : RasterPass(descriptor)
    {
    }

    ShineChildPass::~ShineChildPass()
    {
    }

    void ShineChildPass::SetupFrameGraphDependencies(AZ::RHI::FrameGraphInterface frameGraph)
    {
