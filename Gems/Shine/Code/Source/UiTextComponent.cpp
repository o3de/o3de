...
namespace
{
        ...
        float elementWidth)
    {
        ...
        for (UiTextComponent::DrawBatch& drawBatch : drawBatches)
        {
            ...
            while (uint32_t ch = *pChar)
            {
                ...
                curChar += Shine::GetMultiByteCharSize(ch);
                batchCurChar += Shine::GetMultiByteCharSize(ch);
                ++pChar;
            }
    ...
    //! Converts the vertex format used by FFont to the format being used by the dynamic draw context in Shine.
    //!
    //! Note that the formats are currently identical, but this may change with the removal of more legacy code
    void FontVertexToUiVertex(const SVF_P2F_C4B_T2F_F4B* fontVertices, Shine::UiPrimitiveVertex* uiVertices, int numVertices)
    {
...
int UiTextComponent::DrawBatch::GetNumChars() const
{
    if (GetType() == UiTextComponent::DrawBatch::Type::Text)
    {
        return Shine::GetUtf8StringLength(text);
    }
    else if (GetType() == UiTextComponent::DrawBatch::Type::Image)
    {
        return 1;
...
void UiTextComponent::DrawBatch::Split(int atCharIndex, DrawBatch& newDrawBatchOut)
{
    ...
    if (GetType() == UiTextComponent::DrawBatch::Type::Text)
    {
        AZ_Assert(atCharIndex >= 0 && atCharIndex < Shine::GetUtf8StringLength(text), "Text index out of range. Can't split batch");

        // Set text for new batch
        int numBytesToSplit = Shine::GetByteLengthOfUtf8Chars(text.c_str(), atCharIndex);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void UiTextComponent::Render(Shine::IRenderGraph* renderGraph)
{
    ...
    Shine::BlendMode blendMode = Shine::BlendMode::Normal;
    ...
    if (m_selectionStart != -1)
    {
        ...
        {
            Shine::UiPrimitive* primitive = renderGraph->GetDynamicQuadPrimitive(rect.pt, packedColor);
    ...
    {
        AZ::FFont* font = static_cast<AZ::FFont*>(batch->m_font); // SHINE_ATOM_TODO - move IFont.h out of CryCommon/engine code
...
////////////////////////////////////////////////////////////////////////////////////////////////////
Shine::PathnameType UiTextComponent::GetFont()
{
    return m_fontFilename.GetAssetPath().c_str();
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void UiTextComponent::SetFont(const Shine::PathnameType& fontPath)
{
...
int UiTextComponent::GetCharIndexFromCanvasSpacePoint(AZ::Vector2 point, bool mustBeInBoundingBox)
{
    ...
    for (const DrawBatchLine& batchLine : drawBatchLines.batchLines)
    {
        ...
        if (multiLineText && pickOffset.GetY() >= accumulatedHeight)
        {
            ...
            {
                indexIter += Shine::GetUtf8StringLength(drawBatch.text);
            }
        ...
        for (const DrawBatch& drawBatch : batchLine.drawBatchList)
        {
            ...
            {
                ++pChar;
                curLineIndexIter += Shine::GetMultiByteCharSize(ch);
...
float UiTextComponent::GetTargetWidth(float maxWidth)
{
    ...
    bool forceNoWrap = !Shine::IsUiLayoutCellSizeSpecified(maxWidth);
...
float UiTextComponent::GetTargetHeight(float maxHeight)
{
    ...
    const bool haveMaxHeight = Shine::IsUiLayoutCellSizeSpecified(maxHeight);
...
float UiTextComponent::CalculateHorizontalClipOffset()
{
    ...
    if (ShouldClip() && m_wrapTextSetting != WrapTextSetting::Wrap && cursorIsValid)
    {
        ...
        if (textOverflowing)
        {
            ...
            // - cursorClippedRight
            // - cursorClippedLeft
            int bytesToSelectionEnd = Shine::GetByteLengthOfUtf8Chars(displayedText.c_str(), m_selectionEnd);
...
void UiTextComponent::RenderToCache(float alpha)
{
    ...
    ColorF color = Shine::MakeColorF(m_overrideColor.GetAsVector3(), alpha);
    ...
    for (const DrawBatchLine& drawBatchLine : drawBatchLines.batchLines)
    {
        ...
        for (const DrawBatch& drawBatch : drawBatchLine.drawBatchList)
        {
            if (drawBatch.GetType() == UiTextComponent::DrawBatch::Type::Text)
            {
                ...
                {
                    ColorF color = Shine::MakeColorF(drawBatch.color, 1.0f);  // need ColorF to do srgb conversion
                ...
                if (numQuads > 0)
                {
                    ...
                    cacheBatch->m_cachedPrimitive.m_vertices = new Shine::UiPrimitiveVertex[numQuads * 4];
            ...
            else if (drawBatch.GetType() == UiTextComponent::DrawBatch::Type::Image)
            {
                ...
                cacheImageBatch->m_cachedPrimitive.m_vertices = new Shine::UiPrimitiveVertex[4];
                for (int i = 0; i < 4; ++i)
                {
    ...
    const float ellipsePos)
{
    ...
    while (uint32_t ch = *pChar)
    {
        ...
        stringBufferIndex += Shine::GetMultiByteCharSize(ch);
    ...
    AZ::SerializeContext::DataElementNode& classElement)
{
    ...
    {
        if (!Shine::RemoveLeadingForwardSlashesFromAssetPath(context, classElement, "FontFileName"))
        {
            return false;
        }
    }
