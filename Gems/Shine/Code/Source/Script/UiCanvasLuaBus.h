...
public:
    ...
    virtual AZ::EntityId FindElementById(Shine::ElementId id) = 0;

    //! This flavor of FindElementByName differs slightly from the UiCanvasBus version
    //! in that it returns an EntityId, which is a bit friendly for passing around in
    //! Lua.
    virtual AZ::EntityId FindElementByName(const Shine::NameType& name) = 0;
...
public:
    ...
    AZ::EntityId FindElementById(Shine::ElementId id) override;
    AZ::EntityId FindElementByName(const Shine::NameType& name) override;
