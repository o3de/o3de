...
void UiRenderer::OnBootstrapSceneReady(AZ::RPI::Scene* bootstrapScene)
{
    ...
    // Load the UI shader
    const char* uiShaderFilepath = "Shine/Shaders/ShineUI.azshader";
    ...
    {
        AZ_Error(LogName, false, "Failed to create or initialize a dynamic draw context for Shine. \
            This can happen if the Shine pass hasn't been added to the main render pipeline.");
    }
}
...
AZ::RPI::ScenePtr UiRenderer::CreateScene(AZStd::shared_ptr<AZ::RPI::ViewportContext> viewportContext)
{
    ...
    const char* pipelineAssetPath = "passes/MainRenderPipeline.azasset"; // [SHINE_ATOM_TODO][GHI #6272] Use a custom UI pipeline
...
AZ::RHI::Ptr<AZ::RPI::DynamicDrawContext> UiRenderer::CreateDynamicDrawContextForRTT(const AZStd::string& rttName)
{
    ...
    ShinePassRequestBus::EventResult(rttPass, sceneId, &ShinePassRequestBus::Events::GetRttPass, rttName);
    if (!rttPass)
    {
        return nullptr;
    }
