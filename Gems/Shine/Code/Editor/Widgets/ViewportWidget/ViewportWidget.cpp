...
#include "Helpers/CanvasHelpers.h"
#include "Helpers/QtHelpers.h"
#include "Helpers/SelectionHelpers.h"
#include "Helpers/ViewportHelpers.h"
#include "Interaction/ViewportNudge.h"
#include "Interaction/ViewportPivot.h"
#include "Interaction/ViewportSnap.h"
#include "RulerWidget/RulerWidget.h"
#include "Toolbar/PreviewToolbar.h"
...
namespace
{
                                ...
                                const AzFramework::ModifierKeyMask activeModifierKeys = AzFramework::ModifierKeyMask::None)
    {
        ...
        AZ::Interface<IShine>::Get()->ExecuteQueuedEvents();
...
ViewportWidget::~ViewportWidget()
{
    ...
    ShinePassDataRequestBus::Handler::BusDisconnect();
    ...
    // Notify Shine that this is no longer a valid UiRenderer.
    // Only one viewport/renderer is currently supported in the UI Editor
    CShine* Shine = static_cast<CShine*>(AZ::Interface<IShine>::Get());
    Shine->SetUiRendererForEditor(nullptr);
}
...
void ViewportWidget::InitUiRenderer()
{
    ...
    // Notify Shine that this is the UiRenderer to be used for rendering
    // UI canvases that are loaded in the UI Editor.
    // Only one viewport/renderer is currently supported in the UI Editor
    CShine* Shine = static_cast<CShine*>(AZ::Interface<IShine>::Get());
    Shine->SetUiRendererForEditor(m_uiRenderer);

    m_draw2d = AZStd::make_shared<CDraw2d>(GetViewportContext());

    ShinePassDataRequestBus::Handler::BusConnect(GetViewportContext()->GetRenderScene()->GetId());
}
...
{
#ifdef SHINE_EDITOR_TODO // still need this?
...
Shine::AttachmentImagesAndDependencies ViewportWidget::GetRenderTargets()
{
    Shine::AttachmentImagesAndDependencies canvasTargets;
...
void ViewportWidget::RenderEditMode()
{
    ...
    if (ViewportInteraction::InteractionMode::ROTATE == m_viewportInteraction->GetMode())
    {
        ...
        Shine::EntityArray selectedElements = SelectionHelpers::GetTopLevelSelectedElements(m_editorWindow->GetHierarchy(), selection);
