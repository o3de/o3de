...
#include "Commands/CommandPropertiesChange.h"
...
SerializeHelpers::SerializedEntryList& HierarchyClipboard::Serialize(
    ...
    bool isUndo)
{
    ...
    {
        SelectionHelpers::GetListOfTopLevelSelectedItems(widget, selectedItems, widget->invisibleRootItem(), itemsToSerialize);
    }
...
bool HierarchyClipboard::Unserialize(HierarchyWidget* widget, SerializeHelpers::SerializedEntryList& entryList, bool isUndo)
{
...
void HierarchyClipboard::CreateElementsFromClipboard(
    HierarchyWidget* widget, const QTreeWidgetItemRawPtrQList& selectedItems, bool createAsChildOfSelection)
{
    ...
    CommandHierarchyItemCreateFromData::Push(
        widget->GetEditorWindow()->GetActiveStack(),
        widget,
        selectedItems,
        createAsChildOfSelection,
        [widget, xml](HierarchyItem* parent, Shine::EntityArray& listOfNewlyCreatedTopLevelElements)
        {
...
void HierarchyClipboard::BeginUndoableEntitiesChange(EditorWindow* editorWindow, SerializeHelpers::SerializedEntryList& preChangeState)
{
    ...
    HierarchyClipboard::Serialize(
        editorWindow->GetHierarchy(), editorWindow->GetHierarchy()->selectedItems(), nullptr, preChangeState, true);
}

void HierarchyClipboard::EndUndoableEntitiesChange(
    EditorWindow* editorWindow, const char* commandName, SerializeHelpers::SerializedEntryList& preChangeState)
{
    ...
    CommandPropertiesChange::Push(editorWindow->GetActiveStack(), editorWindow->GetHierarchy(), preChangeState, commandName);

    // Notify other systems (e.g. Animation) for each UI entity that changed
    Shine::EntityArray selectedElements =
        SelectionHelpers::GetTopLevelSelectedElements(editorWindow->GetHierarchy(), editorWindow->GetHierarchy()->selectedItems());
