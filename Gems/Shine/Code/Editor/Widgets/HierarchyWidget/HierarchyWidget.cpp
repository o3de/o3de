    ...
    , m_isInited(false)
{
    ...
    QObject::connect(
        ...
        [this](QTreeWidgetItem* item, int column)
        {
            ...
            {
                CommandHierarchyItemToggleIsSelectable::Push(m_editorWindow->GetActiveStack(), this, HierarchyItemRawPtrList({ i }));
            }
    ...
    QObject::connect(
        ...
        {
            CommandHierarchyItemToggleIsExpanded::Push(m_editorWindow->GetActiveStack(), this, HierarchyItem::RttiCast(item));
        });
...
void HierarchyWidget::CreateItems(const Shine::EntityArray& elements)
{
    ...
    for (auto& e : elementList)
    {
        ...
        new HierarchyItem(m_editorWindow, *parent, childIndex, e->GetName().c_str(), e);
    }
...
void HierarchyWidget::RecreateItems(const Shine::EntityArray& elements)
{
    // remember the currently selected items so we can restore them
    EntityHelpers::EntityIdList selectedEntityIds = SelectionHelpers::GetSelectedElementIds(this, selectedItems(), false);
...
{
    HierarchyItemRawPtrList items = SelectionHelpers::GetSelectedHierarchyItems(this, selectedItems);
    SetUserSelection(items.empty() ? nullptr : &items);
}

void HierarchyWidget::CurrentSelectionHasChanged(
...
void HierarchyWidget::DataHasChanged(const QModelIndex& topLeft, const QModelIndex& bottomRight, [[maybe_unused]] const QVector<int>& roles)
{
    if (topLeft == bottomRight)
    {
        ...
        CommandHierarchyItemRename::Push(m_editorWindow->GetActiveStack(), this, entityId, element->GetName().c_str(), toName);
    }
}
...
void HierarchyWidget::ApplyElementIsExpanded()
{
    ...
    blockSignals(true);
    {
        HierarchyHelpers::TraverseListAndAllChildren(
            allItems,
    ...
    const QMimeData* mimeData, QTreeWidgetItem* targetWidgetItem, bool onElement, int childIndex, const QPoint* newElementPosition)
{
        ...
        if (!ComponentHelpers::CanAddComponentsToSelectedEntities(componentTypes, &firstIncompatibleComponentType))
        {
            ...
            if (!entityId.IsValid() || componentTypeId.IsNull())
            {
                const AZStd::string message =
                    AZStd::string::format("Failed to add components to target element \"%s\".", entityName.c_str());
            ...
            else
            {
                ...
                AZ::ComponentDescriptorBus::EventResult(
                    descriptor, firstIncompatibleComponentType.second, &AZ::ComponentDescriptorBus::Events::GetDescriptor);
                AZStd::string componentName(descriptor ? descriptor->GetName() : "<unknown>");
                const AZStd::string message = AZStd::string::format(
                    "Failed to add components to target element \"%s\". Component \"%s\" is not compatible.",
                    entityName.c_str(),
                    componentName.c_str());
                QMessageBox::warning(m_editorWindow, tr("Asset Drop"), QString(message.c_str()));
            }
    ...
    else
    {
        ...
        CommandHierarchyItemCreate::Push(
            ...
            [this, componentAssetPairs, newElementPosition](AZ::Entity* element)
            {
                if (element)
                {
                    ...
                    ComponentHelpers::EntityComponentPair firstIncompatibleComponentType =
                        AZStd::make_pair(AZ::EntityId(), AZ::Uuid::CreateNull());
                    if (!ComponentHelpers::CanAddComponentsToEntity(componentTypes, element->GetId(), &firstIncompatibleComponentType))
                    {
                        ...
                        else
                        {
                            ...
                            AZStd::string componentName(descriptor ? descriptor->GetName() : "<unknown>");
                            const AZStd::string message = AZStd::string::format(
                                "Failed to add components to new element. Component \"%s\" is not compatible.", componentName.c_str());
                            QMessageBox::warning(m_editorWindow, tr("Asset Drop"), QString(message.c_str()));
                        }
                        return;
                    }
...
void HierarchyWidget::DropMimeDataAssets(
    const QMimeData* mimeData, const AZ::EntityId& targetEntityId, bool onElement, int childIndex, const QPoint* newElementPosition)
{
    if (!m_isInited)
    {
        return;
...
void HierarchyWidget::PickItem(HierarchyItem* item)
{
    ...
    if (entityId.IsValid())
    {
        AzToolsFramework::EditorPickModeRequestBus::Broadcast(&AzToolsFramework::EditorPickModeRequests::PickModeSelectEntity, entityId);
...
void HierarchyWidget::ReparentItems(const QTreeWidgetItemRawPtrList& baseParentItems, const HierarchyItemRawPtrList& childItems)
{
    CommandHierarchyItemReparent::Push(m_editorWindow->GetActiveStack(), this, childItems, baseParentItems);
}
...
void HierarchyWidget::ToggleVisibility(HierarchyItem* hierarchyItem)
{
    ...
    CommandHierarchyItemToggleIsVisible::Push(m_editorWindow->GetActiveStack(), this, items);
}

void HierarchyWidget::DeleteSelectedItems(const QTreeWidgetItemRawPtrQList& selectedItems)
{
    CommandHierarchyItemDelete::Push(m_editorWindow->GetActiveStack(), this, selectedItems);
...
void HierarchyWidget::Cut()
{
    ...
    HierarchyClipboard::CopySelectedItemsToClipboard(this, selection);
    DeleteSelectedItems(selection);
}

void HierarchyWidget::Copy()
{
    HierarchyClipboard::CopySelectedItemsToClipboard(this, selectedItems());
}

void HierarchyWidget::PasteAsSibling()
{
    HierarchyClipboard::CreateElementsFromClipboard(this, selectedItems(), false);
}
    ...
    if (!selection.empty())
    {
        ...
        for (auto item : selection)
        {
            ...
            AzToolsFramework::EditorOnlyEntityComponentRequestBus::Event(
                i->GetEntityId(), &AzToolsFramework::EditorOnlyEntityComponentRequests::SetIsEditorOnlyEntity, editorOnly);
