...
HierarchyItem::HierarchyItem(
    EditorWindow* editWindow, QTreeWidgetItem& parent, int childIndex, const QString label, AZ::Entity* optionalElement)
    : QObject()
    ...
    , m_nonSnappedZRotation(0.0f)
{
    ...
    if (!optionalElement)
    {
        ...
        element->CreateComponent(Shine::UiTransform2dComponentUuid);
    ...
    // Connect signals.
    {
        ...
        QObject::connect(this, SIGNAL(SignalItemAdd(HierarchyItem*)), m_editorWindow->GetHierarchy(), SLOT(HandleItemAdd(HierarchyItem*)));

        QObject::connect(
            this, SIGNAL(SignalItemRemove(HierarchyItem*)), m_editorWindow->GetHierarchy(), SLOT(HandleItemRemove(HierarchyItem*)));
    }
...
void HierarchyItem::UpdateIcon()
{
    // Eye icon.
    {
        ...
        if (isVisible)
        {
            ...
            textureName =
                (m_mouseIsHovering
                     ? UICANVASEDITOR_HIERARCHY_ICON_OPEN_HOVER
                     : (areAllAncestorsVisible ? UICANVASEDITOR_HIERARCHY_ICON_OPEN : UICANVASEDITOR_HIERARCHY_ICON_OPEN_HIDDEN));
        }
        else
        {
        ...
        setIcon(
            ...
            QIcon(textureName).pixmap(UICANVASEDITOR_HIERARCHY_HEADER_ICON_SIZE, UICANVASEDITOR_HIERARCHY_HEADER_ICON_SIZE));
    }
    ...
    // Padlock icon.
    {
        ...
        else
        {
            // This item is locked.
            textureName =
                (m_mouseIsHovering ? UICANVASEDITOR_HIERARCHY_ICON_PADLOCK_ENABLED_HOVER : UICANVASEDITOR_HIERARCHY_ICON_PADLOCK_ENABLED);
        }
...
void HierarchyItem::ReplaceElement(const AZStd::string& xml)
{
    ...
    SerializeHelpers::RestoreSerializedElements(
        m_editorWindow->GetCanvas(), parentEntity, insertBeforeEntity, m_editorWindow->GetEntityContext(), xml, false, nullptr);
...
void HierarchyItem::UpdateEditorOnlyInfo()
{
    ...
    AzToolsFramework::EditorOnlyEntityComponentRequestBus::EventResult(
        isEditorOnly, m_elementId, &AzToolsFramework::EditorOnlyEntityComponentRequests::IsEditorOnlyEntity);
