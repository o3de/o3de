...
CommandHierarchyItemCreate::CommandHierarchyItemCreate(
    UndoStack* stack,
    ...
    if (m_entries.empty())
    {
        ...
        for (auto& parentEntityId : m_parents)
        {
            ...
            HierarchyItem* hierarchyItem =
                new HierarchyItem(m_hierarchy->GetEditorWindow(), *parent, m_childIndex, QString(uniqueName.c_str()), nullptr);
...
void CommandHierarchyItemCreate::Push(
    UndoStack* stack,
    HierarchyWidget* hierarchy,
    ...
    PostCreationCallback postCreationCB)
{
    ...
    stack->push(new CommandHierarchyItemCreate(
        stack, hierarchy, SelectionHelpers::GetSelectedElementIds(hierarchy, selectedItems, true), childIndex, postCreationCB));
}
