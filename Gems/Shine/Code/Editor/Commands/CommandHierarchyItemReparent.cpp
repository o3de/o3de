...
CommandHierarchyItemReparent::CommandHierarchyItemReparent(
...
void CommandHierarchyItemReparent::Reparent(ChildItemList& sourceChildren, ChildItemList& destChildren, bool& childrenSorted)
{
    ...
    if (!childrenSorted)
    {
        ...
        destChildren.sort(
            [](const ChildItem& i1, const ChildItem& i2)
            {
                return i1.m_row < i2.m_row;
            });
...
void CommandHierarchyItemReparent::Push(
    UndoStack* stack,
    HierarchyWidget* hierarchy,
    const HierarchyItemRawPtrList& items,
    [[maybe_unused]] const QTreeWidgetItemRawPtrList& itemParents)
{
    stack->push(new CommandHierarchyItemReparent(stack, hierarchy, items));
}
