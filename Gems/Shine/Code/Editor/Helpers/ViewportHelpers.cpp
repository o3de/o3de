...
#include "Widgets/ViewportWidget/Interaction/ViewportPivot.h"
...
namespace ViewportHelpers
{
    ...
    {
        Qt::CursorShape sizingCursors[]{ Qt::SizeVerCursor, Qt::SizeBDiagCursor, Qt::SizeHorCursor, Qt::SizeFDiagCursor };
    ...
    bool IsPointInIconRect(
        AZ::Vector2 point, AZ::Vector2 iconCenter, AZ::Vector2 iconSize, float leftPart, float rightPart, float topPart, float bottomPart)
    {
        ...
        return (left < point.GetX() && point.GetX() < right && top < point.GetY() && point.GetY() < bottom);
    }
    ...
    void GetVerticalTargetPoints(
        const UiTransformInterface::RectPoints& elemRect, float x, AZ::Vector2& topTarget, AZ::Vector2& bottomTarget)
    {
    ...
    UiTransform2dInterface::Offsets MoveGrabbedEdges(
        const UiTransform2dInterface::Offsets& offset, const ViewportHelpers::ElementEdges& grabbedEdges, const AZ::Vector2& v)
    {
    ...
    UiTransform2dInterface::Anchors MoveGrabbedAnchor(
        const UiTransform2dInterface::Anchors& anchor,
        const ViewportHelpers::SelectedAnchors& grabbedAnchors,
        bool keepTogetherHorizontally,
        bool keepTogetherVertically,
        const AZ::Vector2& v)
    {
    ...
    void DrawRotationValue(
        const AZ::Entity* element, ViewportInteraction* viewportInteraction, const ViewportPivot* viewportPivot, Draw2dHelper& draw2d)
    {
    ...
    void DrawCursorText(const AZStd::string& textLabel, Draw2dHelper& draw2d, const ViewportWidget* viewport)
    {
        const AZ::Vector2 textLabelOffset(10.0f, -10.0f);
        QPoint viewportCursorPos = viewport->mapFromGlobal(QCursor::pos());
        AZ::Vector2 textPos =
            AZ::Vector2(aznumeric_cast<float>(viewportCursorPos.x()), aznumeric_cast<float>(viewportCursorPos.y())) + textLabelOffset;
