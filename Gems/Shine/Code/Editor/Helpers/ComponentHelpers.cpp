...
namespace ComponentHelpers
{
    ...
    AZStd::vector<ComponentTypeData> GetAllComponentTypesThatCanAppearInAddComponentMenu()
    {
        ...
        const AZStd::list<AZ::ComponentDescriptor*>* ShineComponentDescriptors;
        UiSystemBus::BroadcastResult(ShineComponentDescriptors, &UiSystemBus::Events::GetShineComponentDescriptors);

        // Gather all components that match our filter and group by category.
        serializeContext->EnumerateDerived<AZ::Component>(
            [ &componentsList, ShineComponentDescriptors ](const AZ::SerializeContext::ClassData* classData, const AZ::Uuid& knownType) -> bool
            {
                (void)knownType;
                ...
                if (Internal::AppearsInUIComponentMenu(*classData, false))
                {
                    ...
                    bool isShineComponent = false;
                    for (auto descriptor : * ShineComponentDescriptors)
                    {
                        if (descriptor->GetUuid() == classData->m_typeId)
                        {
                            isShineComponent = true;
                            break;
                        }
                    }
                    ComponentTypeData data;
                    data.classData = classData;
                    data.isShineComponent = isShineComponent;
