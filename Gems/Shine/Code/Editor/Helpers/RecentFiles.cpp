...
QStringList ReadRecentFiles()
{
    ...
    int count = AZStd::min(settings.beginReadArray(UICANVASEDITOR_SETTINGS_RECENT_FILES_KEY), UICANVASEDITOR_SETTINGS_RECENT_FILES_COUNT_MAX);
...
void WriteRecentFiles(const QStringList& recentFiles)
{
    ...
    int count = std::min(static_cast<int>(recentFiles.size()), UICANVASEDITOR_SETTINGS_RECENT_FILES_COUNT_MAX);
