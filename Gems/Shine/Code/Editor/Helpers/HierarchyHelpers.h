...
namespace HierarchyHelpers
{
    ...
    using Creator = AZStd::function<void( HierarchyItem* parent,
                                        Shine::EntityArray& listOfNewlyCreatedTopLevelElements )>;

    Shine::EntityArray CreateItemsAndElements(HierarchyWidget* widget,
        const QTreeWidgetItemRawPtrQList& selectedItems,
        ...
        T& listToValidate)
    {
        ...
        for (auto && p : listToValidate)
        {
            ...
            if (AZStd::find(allItems.begin(), allItems.end(), item) == allItems.end())
            {
                // Not found.
                return false;
            }
    ...
    void TraverseListAndAllChildren(T& rootList,
        AZStd::function<void(typename T::value_type)> traverser)
    {
        // Note: This is a breadth-first traversal through all items.
