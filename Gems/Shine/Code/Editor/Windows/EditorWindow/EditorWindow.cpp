...
EditorWindow::~EditorWindow()
{
    ...
    {
        AZ::Interface<IShine>::Get()->ReleaseCanvas(m_previewModeCanvasEntityId, false);
    }
...
Shine::EntityArray EditorWindow::GetSelectedElements()
{
    Shine::EntityArray elements = SelectionHelpers::GetSelectedElements(
        m_hierarchy,
...
{
    AZ::Uuid transformComponentUuid = Shine::UiTransform2dComponentUuid;
...
{
    AZ::Interface<IShine>::Get()->ReleaseCanvas(canvasMetadata.m_canvasEntityId, true);
}
...
bool EditorWindow::LoadCanvas(const QString& canvasFilename, bool autoLoad, bool changeActiveCanvasToThis)
{
    ...
    if (canvasFilename.isEmpty())
    {
        canvasEntityId = AZ::Interface<IShine>::Get()->CreateCanvasInEditor(entityContext);
    }
    else
    {
        ...
        canvasEntityId = AZ::Interface<IShine>::Get()->LoadCanvasInEditor(assetIdPathname.c_str(), sourceAssetPathName.c_str(), entityContext);
...
bool EditorWindow::CanChangeActiveCanvas()
{
    ...
    if (canvasMetadata)
    {
        if (canvasMetadata->m_entityContext->HasPendingRequests())
        {
            return false;
        }
    }
...
UiPrefabManager* EditorWindow::GetPrefabManager()
{
...
bool EditorWindow::CanUnloadCanvas(UiCanvasMetadata& canvasMetadata)
{
    if (GetChangesHaveBeenMade(canvasMetadata))
    {
        QString name = tr("UI canvas \"%1\"").arg(canvasMetadata.m_canvasDisplayName.c_str());
...
void EditorWindow::UpdateChangedStatusOnAssetChange([[maybe_unused]] const AzFramework::EntityContextId& contextId, [[maybe_unused]] const AZ::Data::Asset<AZ::Data::AssetData>& asset)
{
}
...
void EditorWindow::OnCurrentCanvasTabChanged(int index)
{
    ...
    if (!CanChangeActiveCanvas())
    {
        ...
        QMessageBox::information(this,
            tr("Pending Operations"),
            tr("The current UI canvas still has pending operations. Please wait until complete before changing tabs."));
...
int EditorWindow::GetCanvasMaxHierarchyDepth(const Shine::EntityArray& rootChildElements)
{
    int depth = 0;
