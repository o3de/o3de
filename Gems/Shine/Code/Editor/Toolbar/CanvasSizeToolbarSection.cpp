...
#include "Commands/CommandCanvasSize.h"
#include "Commands/CommandCanvasSizeToolbarIndex.h"
...
void CanvasSizeToolbarSection::InitWidgets(QToolBar* parent, bool addSeparator)
{
    ...
    // Canvas presets ComboBox
    {
        ...
        QObject::connect(
            ...
            m_combobox, // IMPORTANT: We HAVE to use static_cast<>() to specify which overload of QComboBox::currentIndexChanged() we want.
            [this](int index)
            {
    ...
    // Width and height line edit boxes for inputting custom canvas sizes
    {
        ...
        m_lineEditCanvasHeightConnection = QObject::connect(
            m_lineEditCanvasHeight,
            &QLineEdit::editingFinished,
            m_lineEditCanvasHeight,
            [this]()
            {
...
const QString& CanvasSizeToolbarSection::IndexToString(int index)
{
    ...
    {
        QString description(QString("%1 x %2 (%3)")
...
void CanvasSizeToolbarSection::LineEditWidthEditingFinished()
{
    ...
    if (newWidth != m_canvasSizePresets[customSizeIndex].width)
    {
        if (m_isChangeUndoable)
        {
            ...
            int presetIndex = GetPresetIndexFromSize(
                AZ::Vector2(aznumeric_cast<float>(newWidth), aznumeric_cast<float>(m_canvasSizePresets[customSizeIndex].height)));
            ...
            else
            {
                ...
                CommandCanvasSize::Push(
                    m_editorWindow->GetActiveStack(),
                    m_editorWindow->GetCanvasSizeToolbarSection(),
                    AZ::Vector2(
                        aznumeric_cast<float>(m_canvasSizePresets[customSizeIndex].width),
                        aznumeric_cast<float>(m_canvasSizePresets[customSizeIndex].height)),
                    AZ::Vector2(aznumeric_cast<float>(newWidth), aznumeric_cast<float>(m_canvasSizePresets[customSizeIndex].height)),
                    false);
            }
        }
...
void CanvasSizeToolbarSection::LineEditHeightEditingFinished()
{
    ...
    if (newHeight != m_canvasSizePresets[customSizeIndex].height)
    {
        if (m_isChangeUndoable)
        {
            // Check if the size is now set to one of the presets
            int presetIndex = GetPresetIndexFromSize(
                AZ::Vector2(aznumeric_cast<float>(m_canvasSizePresets[customSizeIndex].width), aznumeric_cast<float>(newHeight)));
            if (presetIndex != customSizeIndex)
            {
                // Change combo box index which will trigger an OnComboBoxIndexChanged event and add an undoable command
                m_combobox->setCurrentIndex(presetIndex);
...
bool CanvasSizeToolbarSection::ParseCanvasSizePresetsJson(const QJsonDocument& jsonDoc)
{
    ...
    for (QJsonArray::const_iterator citer = canvasPresetArray.begin(); citer != canvasPresetArray.end(); ++citer)
    {
        ...
        const bool expectedObject =
            canvasPresetObj["width"].isDouble() && canvasPresetObj["height"].isDouble() && canvasPresetObj["title"].isString();
        ...
        QString formattedTitle(QString("%1 x %2 (%3)").arg(QString::number(width)).arg(QString::number(height)).arg(title));
...
{
    ComboBoxOptions::iterator matchingResolution = AZStd::find_if(
        m_canvasSizePresets.begin(),
        m_canvasSizePresets.end(),
...
void ReferenceCanvasSizeToolbarSection::SetCanvasSizeByComboBoxIndex()
{
    ...
    AZ::Vector2 canvasSize(
        aznumeric_cast<float>(m_canvasSizePresets[comboIndex].width), aznumeric_cast<float>(m_canvasSizePresets[comboIndex].height));
...
void ReferenceCanvasSizeToolbarSection::OnComboBoxIndexChanged(int index)
{
    ...
    if (index == GetCustomSizeIndex())
    {
        ...
        CommandCanvasSize::Push(
            ...
            m_editorWindow->GetCanvasSizeToolbarSection(),
            AZ::Vector2(
                aznumeric_cast<float>(m_canvasSizePresets[prevIndex].width), aznumeric_cast<float>(m_canvasSizePresets[prevIndex].height)),
            AZ::Vector2(aznumeric_cast<float>(m_canvasSizePresets[index].width), aznumeric_cast<float>(m_canvasSizePresets[index].height)),
            true);
    }
    else
    {
        CommandCanvasSizeToolbarIndex::Push(
            m_editorWindow->GetActiveStack(), m_editorWindow->GetCanvasSizeToolbarSection(), m_comboIndex, index);
    }
}
