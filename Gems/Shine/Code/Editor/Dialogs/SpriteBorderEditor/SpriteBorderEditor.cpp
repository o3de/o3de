    ...
    , m_spritePath(path)
{
    ...
    m_sprite = AZ::Interface<IShine>::Get()->LoadSprite(m_spritePath.toLatin1().constData());
...
void SpriteBorderEditor::DisplaySelectedCell(AZ::u32 cellIndex)
{
    ...
    QRect croppedRect = m_unscaledSpriteSheet.rect();
    {
        ...
        int maxX =
            aznumeric_cast<int>(cellUvCoords.BottomRight().GetX() > 0.0f ? croppedRect.right() * cellUvCoords.BottomRight().GetX() : 0);
        int minY = aznumeric_cast<int>(cellUvCoords.TopLeft().GetY() > 0.0f ? croppedRect.bottom() * cellUvCoords.TopLeft().GetY() : 0);
        int maxY =
            aznumeric_cast<int>(cellUvCoords.BottomRight().GetY() > 0.0f ? croppedRect.bottom() * cellUvCoords.BottomRight().GetY() : 0);
    ...
    QPixmap croppedPixmap =
        m_unscaledSpriteSheet.copy(croppedRect)
            .scaled(UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_WIDTH, UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_HEIGHT, Qt::KeepAspectRatio);
    ...
    for (int i = 0; i < numSliceBorders; ++i)
    {
        ...
        m_manipulators[i]->SetPixmapSizes(
            QSize(aznumeric_cast<int>(cellSize.GetX()), aznumeric_cast<int>(cellSize.GetY())), croppedPixmap.size());
...
void SpriteBorderEditor::AddConfigureSection(QGridLayout* gridLayout, int& rowNum)
{
    ...
    AZStd::function<void()> rowColChangedCallback = [this, numRowsLineEdit, numColsLineEdit]()
    {
    ...
    innerLayout->setContentsMargins(sectionContentLeftMargin, sectionContentTopMargin, right, sectionContentBottomMargin);
...
void SpriteBorderEditor::AddSelectCellSection(QGridLayout* gridLayout, int& rowNum)
{
    ...
    gridLayout->addWidget(
        new QLabel(QString("<h2>Select cell</h2>"), this), rowNum++, 0, cellSelectionLabelRowSpan, cellSelectionLabelColSpan);
    ...
    QPixmap scaledPixmap;
    {
        ...
        if ((m_unscaledSpriteSheet.height() <= 0) && (m_unscaledSpriteSheet.width() <= 0))
        {
            m_hasBeenInitializedProperly = false;
            return;
        }
        ...
        if (isVertical)
        {
            scaledPixmap = m_unscaledSpriteSheet.scaledToHeight(
                aznumeric_cast<int>(UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_HEIGHT - (borderMarginTotal)));
        }
        else
        {
            widthMultiplier = widthToHeightRatio >= 2.0f ? 2 : 1;
            scaledPixmap = m_unscaledSpriteSheet.scaledToWidth(
                aznumeric_cast<int>(UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_WIDTH * widthMultiplier - (borderMarginTotal)));
        }
    }
    ...
    QGraphicsScene* graphicsScene =
        new QGraphicsScene(0.0f, 0.0f, scaledPixmap.width() + borderMarginTotal, scaledPixmap.height() + borderMarginTotal, this);
    ...
    // row/col grid of sprite-sheet cells.
    {
        ...
        for (uint row = 0; row < m_numRows; ++row)
        {
            ...
            if (!finalRow)
            {
                ...
                QLineF bottomDashedRowBorderBlack(
                    0.0f, yOffset + bottomOfCellOffset, scaledPixmap.width() + borderMarginTotal, yOffset + bottomOfCellOffset);
        ...
        for (uint col = 1; col < m_numCols; ++col)
        {
            ...
            // image already has a border.
            QLineF rightDashedCellBorderBlack(xOffset, 0.0f, xOffset, scaledPixmap.height() + borderMarginTotal);
            graphicsScene->addLine(rightDashedCellBorderBlack, cellDividerPenBlack);

            QLineF rightDashedCellBorderWhite(xOffset, 0.0f, xOffset, scaledPixmap.height() + borderMarginTotal);
    ...
    for (uint row = 0; row < m_numRows; ++row)
    {
        ...
        for (uint col = 0; col < m_numCols; ++col)
        {
            ...
            QObject::connect(this, &SpriteBorderEditor::ResettingUi, cellSelectRectItem, &CellSelectRectItem::StopProcessingInput);
...
void SpriteBorderEditor::AddPropertiesSection(QGridLayout* gridLayout, int& rowNum)
{
    ...
    m_cellPropertiesGraphicsScene =
        new QGraphicsScene(0.0f, 0.0f, UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_WIDTH, UICANVASEDITOR_SPRITEBORDEREDITOR_SCENE_HEIGHT, this);
    ...
    QSize scaledPixmapSize;
    {
        QString fullPath =
            Path::GamePathToFullPath((m_sprite->GetTexturePathname() + "." + AZ::RPI::StreamingImageAsset::Extension).c_str());
        m_unscaledSpriteSheet = QPixmap(fullPath);
        if ((m_unscaledSpriteSheet.size().height() <= 0) && (m_unscaledSpriteSheet.size().width() <= 0))
        {
            m_hasBeenInitializedProperly = false;
            return;
        }
    ...
    QGridLayout* rightColumnLayout = new QGridLayout();
    {
        ...
        QGridLayout* textureSizeLabelLayout = new QGridLayout();
        {
            ...
            textureSizeLabelLayout->setContentsMargins(interElementSpacing, top, right, bottom);
        ...
        QGridLayout* propertyFieldsLayout = new QGridLayout();
        {
            ...
            propertyFieldsLayout->setContentsMargins(interElementSpacing, top, right, bottom);
            ...
            {
                SlicerEdit* edit = new SlicerEdit(this, b, unscaledPixmapSize, m_sprite);

                SlicerManipulator* manipulator =
                    new SlicerManipulator(b, unscaledPixmapSize, scaledPixmapSize, m_sprite, m_cellPropertiesGraphicsScene, edit);
