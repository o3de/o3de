...
#include <ShineBuilder/UiCanvasBuilderWorker.h>
...
class ShineSystemTestComponent
    : public Shine::ShineSystemComponent
{
    friend class ShineEditorTest;
};

class ShineEditorTest
    : public ::testing::Test
{
protected:
    ...
    void SetUp() override
    {
        ...
        AZ::Test::AddActiveGem("Shine", *registry, fileIo);
        ...
        m_componentDescriptors.push_back(AZStd::unique_ptr<AZ::ComponentDescriptor>(ShineSystemComponent::CreateDescriptor()));
        ...
        m_sysComponent = AZStd::make_unique<ShineSystemTestComponent>();
    ...
    AZStd::unique_ptr<ShineSystemTestComponent> m_sysComponent;
...
{
    constexpr char testFileFolder[] = "@gemroot:Shine@/Code/Tests/";
...
TEST_F(ShineEditorTest, ProcessUiCanvas_ReturnsDependencyOnSpriteAndTexture)
{
    using namespace Shine;
...
TEST_F(ShineEditorTest, FindLoadedCanvasByPathName_FT)
{
    UiCanvasManager canvasManager;

    //find loaded canvas, should return invalid id
    AZ::EntityId entityId = canvasManager.FindLoadedCanvasByPathName("@gemroot:Shine@/Code/Tests/TestAssets/Canvases/empty.uicanvas", false);
    EXPECT_FALSE(entityId.IsValid());

    //load a new canvas
    entityId = canvasManager.FindLoadedCanvasByPathName("@gemroot:Shine@/Code/Tests/TestAssets/Canvases/empty.uicanvas", true);
