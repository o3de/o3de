...
namespace OpenParticleSystemEditor
{
    ...
    void GradientColorDialog::Initialize()
    {
        ...
        m_gradientColorPickerWidget->SetGradientChangedCB([=](QGradientStops _stops)
            {
                this->OnGradientChanged(_stops);
            });
        connect(m_lineEditLocation, &QLineEdit::editingFinished, this, [=]()
            {
        ...
        m_gradientColorPickerWidget->SetLocationChangedCB([=](short location, [[maybe_unused]] QColor color)
            {
                ShowKeys(location, color);
                OnChangeUpdateDisplayedGradient();
            });
        m_gradientColorPickerWidget->SetDefaultLocationChangedCB([=]()
            {
                ShowKeys();
                OnChangeUpdateDisplayedGradient();
            });
