...
namespace OpenParticleSystemEditor
{
    ...
    void Behavior::Start()
    {
        ...
        OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
            m_cameraEntityId, &OpenParticleEditorViewportInputControllerRequestBus::Handler::GetCameraEntityId);
        AZ_Assert(m_cameraEntityId.IsValid(), "Failed to find m_cameraEntityId");
        OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
            m_distanceToTarget, &OpenParticleEditorViewportInputControllerRequestBus::Handler::GetDistanceToTarget);
        OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
            m_targetPosition, &OpenParticleEditorViewportInputControllerRequestBus::Handler::GetTargetPosition);
        OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
            m_radius, &OpenParticleEditorViewportInputControllerRequestBus::Handler::GetRadius);
    }
    ...
    void Behavior::TickInternal([[maybe_unused]] float x, [[maybe_unused]] float y, float z)
    {
        ...
        OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
            isCameraCentered, &OpenParticleEditorViewportInputControllerRequestBus::Handler::IsCameraCentered);

        // if camera is looking at the model (locked to the model) we don't want to zoom past the model's center
        if (isCameraCentered)
        {
            m_distanceToTarget = AZ::GetMax(m_distanceToTarget, 0.01f);
        }
        ...
        if (!isCameraCentered)
        {
            ...
            OpenParticleEditorViewportInputControllerRequestBus::Broadcast(
                &OpenParticleEditorViewportInputControllerRequestBus::Handler::SetTargetPosition, m_targetPosition);
            OpenParticleEditorViewportInputControllerRequestBus::BroadcastResult(
                m_distanceToTarget, &OpenParticleEditorViewportInputControllerRequestBus::Handler::GetDistanceToTarget);
        }
    }
