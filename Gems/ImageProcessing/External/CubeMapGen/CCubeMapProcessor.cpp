...
namespace ImageProcessing
{
    ...
    //--------------------------------------------------------------------------------------
    void VectToTexelCoord(float *a_XYZ, int32 a_Size, int32 *a_FaceIdx, int32 *a_U, int32 *a_V )
    {
       float nvcU, nvcV;
       float absXYZ[3];
       float maxCoord;
    ...
    {
        float phi = 2 * CP_PI * vXi[0];
        float cosTheta = sqrtf((1 - vXi[1]) / ( 1 + (roughness * roughness - 1) * vXi[1]));
        ...
        float vUpVectorX[3] = {1, 0, 0};
        float vUpVectorZ[3] = {0, 0, 1};
        float vTangentX[3];
        float vTangentY[3];
        float vTempVec[3];
    ...
    void CCubeMapProcessor::FilterCubeSurfacesGGX(CImageSurface *a_SrcCubeMap, CImageSurface *a_DstCubeMap, int32 a_SampleCount, float a_Roughness,
        ...
        int32 a_SampleCountGGX)
    {
       ...
       for(i=0; i<(m_NumMipLevels-1) && !m_shutdownWorkerThreadSignal; i++)
       {
          ...
          if (a_FilterType == CP_FILTER_TYPE_GGX)
          {
            ...
            float smoothness = VM_MAX(1.0f - (float)(i + 1) / (float)(numUsableMips - 1), 0.0f);
