...
#include <Source/NetworkEntity/INetworkEntityDomain.h>
...
namespace Multiplayer
{
    ...
    void EntityReplicationManager::UpdateWindow()
    {
        ...
        if (m_replicationWindow->ReplicationSetUpdateReady())
        {
            ...
            while (newWindowIter != newWindow.end() && currWindowIter != m_entityReplicatorMap.end())
            {
                ...
                {
                    AddEntityReplicator(newWindowIter->first, newWindowIter->second.m_networkRole);
                    ++newWindowIter;
                }
                ...
                else // same entity
                {
                    ...
                    EntityReplicator* currReplicator = currWindowIter->second.get();
                    if (currReplicator->GetRemoteNetworkRole() != newWindowIter->second.m_networkRole)
                    {
                        currReplicator = AddEntityReplicator(newWindowIter->first, newWindowIter->second.m_networkRole);
                    }
    ...
    void EntityReplicationManager::SetEntityDomain(AZStd::unique_ptr<INetworkEntityDomain> entityDomain)
    {
        m_remoteEntityDomain = AZStd::move(entityDomain);
    }

    INetworkEntityDomain* EntityReplicationManager::GetEntityDomain()
    {
    ...
    void EntityReplicationManager::MigrateEntityInternal(NetEntityId netEntityId)
    {
        ...
        if (netBindComponent && netBindComponent->GetNetEntityRole() == NetEntityRole::ServerAuthority)
        {
            ...
            {
                //EntityReplicationManagerNotificationBus::Event(localEnt->GetId(), &EntityReplicationManagerNotificationBus::Events::OnPreEntityMigration, NetworkEntityHandle, GetRemoteHostId(), GetConnection().GetConnectionId());
            }
            ...
            if (m_updateMode == EntityReplicationManager::Mode::LocalServerToRemoteServer)
            {
                //EntityReplicationManagerNotificationBus::Event(localEnt->GetId(), &EntityReplicationManagerNotificationBus::Events::OnPostEntityMigration, networkEntityHandle, GetRemoteHostId(), GetConnection().GetConnectionId());
            }
