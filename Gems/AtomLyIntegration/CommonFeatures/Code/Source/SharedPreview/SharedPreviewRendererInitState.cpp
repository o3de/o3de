...
namespace AZ
{
    namespace LyIntegration
    {
        ...
        void SharedPreviewRendererInitState::Start()
        {
            ...
            ThumbnailFeatureProcessorProviderBus::BroadcastResult(
                results, &ThumbnailFeatureProcessorProviderBus::Handler::GetCustomFeatureProcessors);
            ...
            Outcome<AZStd::shared_ptr<AzFramework::Scene>, AZStd::string> createSceneOutcome = sceneSystem->CreateScene(data->m_sceneName);
            AZ_Assert(
                createSceneOutcome, createSceneOutcome.GetError().c_str()); // This should never happen unless scene creation has changed.
            ...
            MakePerspectiveFovMatrixRH(viewToClipMatrix, Constants::QuarterPi, AspectRatio, NearDist, FarDist, true);
            data->m_view->SetViewToClipMatrix(viewToClipMatrix);

            data->m_renderPipeline->SetDefaultView(data->m_view);

            // Create lighting preset
            data->m_lightingPresetAsset =
                AZ::RPI::AssetUtils::LoadAssetByProductPath<AZ::RPI::AnyAsset>(SharedPreviewRendererData::LightingPresetPath);
            if (data->m_lightingPresetAsset.IsReady())
            {
                ...
                if (preset)
                {
                    ...
                    auto exposureControlSettingInterface = postProcessFeatureProcessor->GetOrCreateSettingsInterface(EntityId())
                                                               ->GetOrCreateExposureControlSettingsInterface();
                    auto directionalLightFeatureProcessor =
                        data->m_scene->GetFeatureProcessor<Render::DirectionalLightFeatureProcessorInterface>();
            ...
            AzFramework::EntityContextRequestBus::EventResult(
                data->m_modelEntity, data->m_entityContext->GetContextId(), &AzFramework::EntityContextRequestBus::Events::CreateEntity,
            ...
            Data::AssetCatalogRequestBus::BroadcastResult(
                defaultModelAssetId, &Data::AssetCatalogRequestBus::Events::GetAssetIdByPath, m_context->GetData()->DefaultModelPath,
                RPI::ModelAsset::RTTI_Type(), false);
            AZ_Error(
                "SharedPreviewRenderer", defaultModelAssetId.IsValid(), "Default model asset is invalid. Verify the asset %s exists.",
            ...
            Data::AssetCatalogRequestBus::BroadcastResult(
                defaultMaterialAssetId, &Data::AssetCatalogRequestBus::Events::GetAssetIdByPath, m_context->GetData()->DefaultMaterialPath,
                RPI::MaterialAsset::RTTI_Type(), false);
            AZ_Error(
                "SharedPreviewRenderer", defaultMaterialAssetId.IsValid(), "Default material asset is invalid. Verify the asset %s exists.",
        ...
        void SharedPreviewRendererInitState::Stop()
        {
        }
    } // namespace LyIntegration
} // namespace AZ
