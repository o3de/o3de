...
2. Encapsulation:
   ...
   2. The DCCsi avoids direct manipulation of the users installation of any DCC tool, instead it attempts to use data-driven configuration and boostrapping to launch a tool with O3DE extensions.  
   
   3. Tools can run Independantly and Standalone, with minimal dependancies. 
...
However, foundation.py was generically written to function against any pyton.exe first, then the plumbing into the config.py modules happened later.  The following instructions are for devs who might run foundation.py for a DCC application that does not yet have integration folder (allowing package dependancies to be installed for that DCC app, before development for that DCC app has begun.)
...
- *settings.local.json* allows the user to add new settings or locally override the value of any setting.  When the config.py envionment is executed and stood up, this is the last file to load to ensure that local settings are always enabled.
