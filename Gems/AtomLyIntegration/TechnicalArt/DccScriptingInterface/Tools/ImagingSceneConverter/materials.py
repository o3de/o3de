...
# --------------------------------------------------------------------------
# -- Global Definitions --
_MODULENAME = 'Tools.ImagingSceneConverter.materials'
...
def unpack_mdl_texture(channels, pbr_type):
    pass
...
def get_material_info():
    material_dictionary = {}
    for mesh in pm.ls(v=True, geometry=True):
        pm.select(mesh)
        target_object = pm.filterExpand(selectionMask=12, fullPath=True)
        if pm.nodeType(target_object) == 'mesh':
            target_object = pm.listRelatives(target_object, parent=True, fullPath=True)
            get_mesh_shapes = pm.ls(dag=1, o=1, s=1, sl=1)
            sg = pm.listConnections(get_mesh_shapes, type='shadingEngine')
            material = pm.ls(pm.listConnections(sg), materials=1)[0]

            if not material in material_dictionary.keys():
                material_dictionary[str(material)] = {'sg': sg, 'type': type(material),
                                                 'assigned': [target_object], 'textures': {}}
            else:
                material_dictionary[str(material)]['assigned'].append(target_object)

            # Get attached textures
            file_nodes = pm.listHistory(sg, type='file')
            if file_nodes:
                file_node_dict = {}
                for node in file_nodes:
                    file_node_dict[node] = pm.getAttr("%s.fileTextureName" % node)
                material_dictionary[str(material)]['textures'].update(file_node_dict)
...
def get_material_assignments():
    ...
    :return: Dictionary - material keys and assigned mesh values
    """
    assignments = {}
    scene_geo = mc.ls(v=True, geometry=True)
    for mesh in scene_geo:
        mesh_material = get_mesh_materials(mesh)
        if mesh_material:
            # _LOGGER.info('Mesh [{}] -- MeshMaterial: {}'.format(mesh, mesh_material))
            if mesh_material[0] not in assignments.keys():
                assignments[mesh_material[0]] = [mesh]
            else:
                assignments[mesh_material[0]].append(mesh)
    return assignments
...
def get_material_textures(material_name):
    ...
    try:
        ...
        for node in file_nodes:
            ...
            # _LOGGER.info('File Texture Node:::> {}'.format(texture_name))
...
def get_scene_material_information():
    ...
    :return: Dictionary - Material Info: Material Name, Shading Group, Material Type, Textures, Assignments
    """
    ...
    for material_name in scene_materials:
        try:
            geo = material_assignments[material_name]
        except KeyError:
            geo = ''
...
def set_camera_attributes():
    """
    Corrects Near/Far Clip planes so visual display of objects aren't clipped if user
    scene units were set to Maya default of centimeters
    :return: None
    """
...
def enable_stingray_textures(material_name):
