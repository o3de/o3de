...
{
    void DefaultMultiplayerSpawnerComponent::Activate()
    {
        AZ::Interface<IMultiplayerSpawner>::Register(this);
    }

    void DefaultMultiplayerSpawnerComponent::Deactivate()
    {
        AZ::Interface<IMultiplayerSpawner>::Unregister(this);
    }

    void DefaultMultiplayerSpawnerComponent::Reflect(AZ::ReflectContext* context)
    {
        if (const auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serializeContext->Class<DefaultMultiplayerSpawnerComponent, AZ::Component>()
                ->Version(1)
                ->Field("PlayerSpawnable", &DefaultMultiplayerSpawnerComponent::m_playerSpawnable)
                ->Field("SpawnPoints", &DefaultMultiplayerSpawnerComponent::m_spawnPoints)
                ;

            if (AZ::EditContext* editContext = serializeContext->GetEditContext())
            {
                editContext->Class<DefaultMultiplayerSpawnerComponent>(
                               "Default Multiplayer Spawner",
                               "The default multiplayer spawner that comes included with the Multiplayer gem. Attach this component to any level's root entity which needs to spawn a network player.")
    ...
    void DefaultMultiplayerSpawnerComponent::GetProvidedServices(AZ::ComponentDescriptor::DependencyArrayType& provided)
    {
        provided.push_back(AZ_CRC_CE("MultiplayerSpawnerService"));
    }

    void DefaultMultiplayerSpawnerComponent::GetIncompatibleServices([[maybe_unused]] AZ::ComponentDescriptor::DependencyArrayType& incompatible)
    {
        incompatible.push_back(AZ_CRC_CE("MultiplayerSpawnerService"));
    }

    NetworkEntityHandle DefaultMultiplayerSpawnerComponent::OnPlayerJoin(
        [[maybe_unused]] uint64_t userId, [[maybe_unused]] const MultiplayerAgentDatum& agentDatum)
    {
        ...
        {
            AZLOG_WARN("DefaultMultiplayerSpawnerComponent is missing spawn points. Spawning new player at the origin.")
        }
        else
        {
    ...
    void DefaultMultiplayerSpawnerComponent::OnPlayerLeave(ConstNetworkEntityHandle entityHandle, [[maybe_unused]] const ReplicationSet& replicationSet, [[maybe_unused]] AzNetworking::DisconnectReason reason)
    {
        AZ::Interface<IMultiplayer>::Get()->GetNetworkEntityManager()->MarkForRemoval(entityHandle);
    }
} // namespace DefaultMultiplayerSpawnerComponent
