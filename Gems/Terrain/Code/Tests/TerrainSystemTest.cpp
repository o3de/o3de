...
namespace UnitTest
{
    ...
    TEST_F(TerrainSystemTest, TerrainProcessAsyncCancellation)
    {
        ...
        // Generate some input positions (doesn't really matter how many as we'll cancel while the first is being processed).
        AZStd::vector<AZ::Vector3> inPositions;
        for (int i = 0; i < 64; ++i)
        {
        ...
        AZStd::atomic_bool asyncRequestStarted = false;
        AZStd::atomic_bool asyncRequestCancelled = false;
        auto perPositionCallback = [&asyncRequestStarted, &asyncRequestCancelled]([[maybe_unused]] const AzFramework::SurfaceData::SurfacePoint& surfacePoint, [[maybe_unused]] bool terrainExists)
        {
            // Once the async request has been cancelled we should get no more completion callbacks.
            EXPECT_FALSE(asyncRequestCancelled);

            // Indicate that the async request has started.
            asyncRequestStarted = true;

            // Wait until the async request has been cancelled before allowing it to continue.
            while (!asyncRequestCancelled) {}
        };
        ...
        auto completionCallback = [&completionMutex, &completionVariable, &completionFlag](AZStd::shared_ptr<AzFramework::Terrain::TerrainDataRequests::TerrainJobContext> terrainJobContext)
        {
        ...
        AZStd::shared_ptr<AzFramework::Terrain::TerrainDataRequests::TerrainJobContext> terrainJobContext = terrainSystem->CreateNewTerrainJobContext();
        ...
        asyncParams->m_terrainJobContext = terrainJobContext;
