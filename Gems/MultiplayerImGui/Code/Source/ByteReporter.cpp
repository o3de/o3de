...
{
    void ByteReporter::ReportBytes(size_t byteSize)
    {
        m_count++;
    ...
    void ByteReporter::AggregateBytes(size_t byteSize)
    {
        m_aggregateBytes += byteSize;
    }

    void ByteReporter::ReportAggregateBytes()
    {
        ReportBytes(m_aggregateBytes);
        m_aggregateBytes = 0;
    }

    float ByteReporter::GetAverageBytes() const
    {
        if (m_count == 0)
        {
            AZ_Warning("ByteReporter", m_totalBytes == 0, "Attempted to average bytes with a zero count.");
            return 0.0f;
        }
    ...
    size_t ByteReporter::GetMaxBytes() const
    {
        return m_maxBytes;
    }

    size_t ByteReporter::GetMinBytes() const
    {
        return m_minBytes;
    }

    size_t ByteReporter::GetTotalBytes() const
    {
        return m_totalBytes;
    }

    float ByteReporter::GetKbitsPerSecond()
    {
    ...
    void ByteReporter::Combine(const ByteReporter& other)
    {
