    ...
    if (bLoading)
    {
        ...
        if (paramIdVersion <= 2)
        {
            ...
            for (int i = 0; i < num; ++i)
            {
                ...
                static const unsigned int local_OLD_APARAM_USER = 100; // Is also defined in AnimNode.cpp
                paramType = static_cast<AnimParamType>(static_cast<int>(paramType.GetType()) + local_OLD_APARAM_USER);
