...
namespace ScriptCanvasEditor
{
    ...
    Modifier::Modifier(QWidget* parent /*= nullptr*/)
        : AzQtComponents::StyledDialog(parent)
        , m_ui(new Ui::Modifier())
    {
        m_ui->setupUi(this);
        ...
        connect(m_ui->scanButton, &QPushButton::pressed, this, &Modifier::OnScan);
        connect(m_ui->closeButton, &QPushButton::pressed, this, &Modifier::OnClose);
        connect(m_ui->upgradeAllButton, &QPushButton::pressed, this, &Modifier::OnUpgradeAll);
    ...
    void Modifier::Log(const char* format, ...)
    {
    ...
    void Modifier::OnClose()
    {
        reject();
    }

    bool Modifier::IsUpgrading() const
    {
    ...
    AZStd::string Modifier::BackupGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "Modifier::BackupGraph: Failed to find file: %s", asset.GetHint().c_str());
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "Modifier::BackupGraph: Error creating backup: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
            return "Failed to copy source file to backup location";
        }

        Log("Modifier::BackupGraph: Backed up: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
        return "";
    }

    void Modifier::UpgradeGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        m_inProgress = true;
        m_upgradeComplete = false;
        ...
        if (asset.GetType() == azrtti_typeid<ScriptCanvasAsset>())
        {
            ...
            AZ_Assert(scriptCanvasEntity, "Modifier::UpgradeGraph The Script Canvas asset must have a valid entity");
    ...
    void Modifier::OnGraphUpgradeComplete(AZ::Data::Asset<AZ::Data::AssetData>& asset, bool /*skipped*/ /*= false*/)
    {
    ...
    void Modifier::OnSourceFileReleased(AZ::Data::Asset<AZ::Data::AssetData> asset)
    {
        AZStd::string relativePath, fullPath;
        AZ::Data::AssetCatalogRequestBus::BroadcastResult(relativePath, &AZ::Data::AssetCatalogRequests::GetAssetPathById, asset.GetId());
        bool fullPathFound = false;
    ...
    void Modifier::PerformMove(AZ::Data::Asset<AZ::Data::AssetData> asset, AZStd::string source, AZStd::string target
    ...
    void Modifier::GraphUpgradeComplete
    ...
    void Modifier::GraphUpgradeCompleteUIUpdate
        ( const AZ::Data::Asset<AZ::Data::AssetData> asset, OperationResult result, AZStd::string_view message)
    {
    ...
    void Modifier::InspectAsset(AZ::Data::Asset<AZ::Data::AssetData>& asset, AZ::Data::AssetInfo& assetInfo)
    {
    ...
    void Modifier::ScanComplete(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
    ...
    bool Modifier::CaptureLogFromTraceBus(const char* window, const char* message)
    {
    ...
    bool Modifier::OnPreError(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Error): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool Modifier::OnPreWarning(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Warning): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool Modifier::OnException(const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Exception): %s", message);
        return CaptureLogFromTraceBus("Script Canvas", msg.c_str());
    }

    bool Modifier::OnPrintf(const char* window, const char* message)
    {
        return CaptureLogFromTraceBus(window, message);
    }

    void Modifier::closeEvent(QCloseEvent* event)
    {
        m_keepEditorAlive.reset();
