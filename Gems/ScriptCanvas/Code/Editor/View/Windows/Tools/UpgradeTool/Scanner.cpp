...
namespace ScriptCanvasEditor
{
    ...
    Scanner::Scanner(QWidget* parent /*= nullptr*/)
        ...
        , m_ui(new Ui::Scanner())
    {
        ...
        connect(m_ui->scanButton, &QPushButton::pressed, this, &Scanner::OnScan);
        connect(m_ui->closeButton, &QPushButton::pressed, this, &Scanner::OnClose);
        connect(m_ui->upgradeAllButton, &QPushButton::pressed, this, &Scanner::OnUpgradeAll);
    ...
    void Scanner::Log(const char* format, ...)
    {
    ...
    void Scanner::OnClose()
    {
        reject();
    }

    bool Scanner::IsUpgrading() const
    {
    ...
    AZStd::string Scanner::BackupGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "Scanner::BackupGraph: Failed to find file: %s", asset.GetHint().c_str());
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "Scanner::BackupGraph: Error creating backup: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
            return "Failed to copy source file to backup location";
        }

        Log("Scanner::BackupGraph: Backed up: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
        return "";
    }

    void Scanner::UpgradeGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        m_inProgress = true;
        m_upgradeComplete = false;
        ...
        if (asset.GetType() == azrtti_typeid<ScriptCanvasAsset>())
        {
            ...
            AZ_Assert(scriptCanvasEntity, "Scanner::UpgradeGraph The Script Canvas asset must have a valid entity");
    ...
    void Scanner::OnGraphUpgradeComplete(AZ::Data::Asset<AZ::Data::AssetData>& asset, bool /*skipped*/ /*= false*/)
    {
    ...
    void Scanner::OnSourceFileReleased(AZ::Data::Asset<AZ::Data::AssetData> asset)
    {
        AZStd::string relativePath, fullPath;
        AZ::Data::AssetCatalogRequestBus::BroadcastResult(relativePath, &AZ::Data::AssetCatalogRequests::GetAssetPathById, asset.GetId());
        bool fullPathFound = false;
    ...
    void Scanner::PerformMove(AZ::Data::Asset<AZ::Data::AssetData> asset, AZStd::string source, AZStd::string target
    ...
    void Scanner::GraphUpgradeComplete
    ...
    void Scanner::GraphUpgradeCompleteUIUpdate
        ( const AZ::Data::Asset<AZ::Data::AssetData> asset, OperationResult result, AZStd::string_view message)
    {
    ...
    void Scanner::InspectAsset(AZ::Data::Asset<AZ::Data::AssetData>& asset, AZ::Data::AssetInfo& assetInfo)
    {
    ...
    void Scanner::ScanComplete(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
    ...
    bool Scanner::CaptureLogFromTraceBus(const char* window, const char* message)
    {
    ...
    bool Scanner::OnPreError(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Error): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool Scanner::OnPreWarning(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Warning): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool Scanner::OnException(const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Exception): %s", message);
        return CaptureLogFromTraceBus("Script Canvas", msg.c_str());
    }

    bool Scanner::OnPrintf(const char* window, const char* message)
    {
