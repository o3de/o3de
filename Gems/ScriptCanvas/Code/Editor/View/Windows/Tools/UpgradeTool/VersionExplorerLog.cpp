...
namespace ScriptCanvasEditor
{
    ...
    VersionExplorerLog::VersionExplorerLog(QWidget* parent /*= nullptr*/)
        : AzQtComponents::StyledDialog(parent)
        , m_ui(new Ui::VersionExplorerLog())
    {
        m_ui->setupUi(this);
        ...
        connect(m_ui->scanButton, &QPushButton::pressed, this, &VersionExplorerLog::OnScan);
        connect(m_ui->closeButton, &QPushButton::pressed, this, &VersionExplorerLog::OnClose);
        connect(m_ui->upgradeAllButton, &QPushButton::pressed, this, &VersionExplorerLog::OnUpgradeAll);
    ...
    VersionExplorerLog::~VersionExplorerLog()
    {
    ...
    void VersionExplorerLog::Log(const char* format, ...)
    {
    ...
    void VersionExplorerLog::OnClose()
    {
        reject();
    }

    bool VersionExplorerLog::IsUpgrading() const
    {
        return m_inProgressAsset != m_assetsToUpgrade.end() && m_inProgress;
    }

    void VersionExplorerLog::OnSystemTick()
    {
        switch (m_state)
        {
    ...
    void VersionExplorerLog::OnUpgradeAll()
    {
    ...
    AZStd::string VersionExplorerLog::BackupGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "VersionExplorerLog::BackupGraph: Failed to find file: %s", asset.GetHint().c_str());
        ...
        {
            AZ_Warning(ScriptCanvas::k_VersionExplorerWindow.data(), false, "VersionExplorerLog::BackupGraph: Error creating backup: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
            return "Failed to copy source file to backup location";
        }

        Log("VersionExplorerLog::BackupGraph: Backed up: %s  ---> %s\n", sourceFilePath.c_str(), targetFilePath.c_str());
        return "";
    }

    void VersionExplorerLog::UpgradeGraph(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
        m_inProgress = true;
        m_upgradeComplete = false;
        ...
        if (asset.GetType() == azrtti_typeid<ScriptCanvasAsset>())
        {
            ...
            AZ_Assert(scriptCanvasEntity, "VersionExplorerLog::UpgradeGraph The Script Canvas asset must have a valid entity");
    ...
    void VersionExplorerLog::OnGraphUpgradeComplete(AZ::Data::Asset<AZ::Data::AssetData>& asset, bool /*skipped*/ /*= false*/)
    {
    ...
    void VersionExplorerLog::OnSourceFileReleased(AZ::Data::Asset<AZ::Data::AssetData> asset)
    {
        AZStd::string relativePath, fullPath;
        AZ::Data::AssetCatalogRequestBus::BroadcastResult(relativePath, &AZ::Data::AssetCatalogRequests::GetAssetPathById, asset.GetId());
        bool fullPathFound = false;
    ...
    void VersionExplorerLog::PerformMove(AZ::Data::Asset<AZ::Data::AssetData> asset, AZStd::string source, AZStd::string target
    ...
    void VersionExplorerLog::GraphUpgradeComplete
    ...
    void VersionExplorerLog::GraphUpgradeCompleteUIUpdate
        ( const AZ::Data::Asset<AZ::Data::AssetData> asset, OperationResult result, AZStd::string_view message)
    {
    ...
    void VersionExplorerLog::FinalizeUpgrade()
    {
        Log("FinalizeUpgrade!");
        m_inProgress = false;
    ...
    void VersionExplorerLog::OnScan()
    {
        m_assetsToUpgrade.clear();
    ...
    void VersionExplorerLog::DoScan()
    {
    ...
    void VersionExplorerLog::BackupComplete()
    {
        m_currentAssetRowIndex = 0;
        m_ui->progressBar->setValue(0);
        DoScan();
    ...
    void VersionExplorerLog::InspectAsset(AZ::Data::Asset<AZ::Data::AssetData>& asset, AZ::Data::AssetInfo& assetInfo)
    {
    ...
    void VersionExplorerLog::UpgradeSingle
    ...
    void VersionExplorerLog::ScanComplete(const AZ::Data::Asset<AZ::Data::AssetData>& asset)
    {
    ...
    void VersionExplorerLog::FinalizeScan()
    {
        Log("FinalizeScan()");
    ...
    void VersionExplorerLog::FlushLogs()
    {
        if (m_logs.empty())
        {
            return;
    ...
    bool VersionExplorerLog::CaptureLogFromTraceBus(const char* window, const char* message)
    {
    ...
    bool VersionExplorerLog::OnPreError(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Error): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool VersionExplorerLog::OnPreWarning(const char* window, const char* /*fileName*/, int /*line*/, const char* /*func*/, const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Warning): %s", message);
        return CaptureLogFromTraceBus(window, msg.c_str());
    }

    bool VersionExplorerLog::OnException(const char* message)
    {
        AZStd::string msg = AZStd::string::format("(Exception): %s", message);
        return CaptureLogFromTraceBus("Script Canvas", msg.c_str());
    }

    bool VersionExplorerLog::OnPrintf(const char* window, const char* message)
    {
        return CaptureLogFromTraceBus(window, message);
    }

    void VersionExplorerLog::closeEvent(QCloseEvent* event)
    {
        m_keepEditorAlive.reset();
