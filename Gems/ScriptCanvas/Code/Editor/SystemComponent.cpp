...
namespace ScriptCanvasEditor
{
        ...
        , AzToolsFramework::AssetBrowser::SourceFileCreatorList& creators)
    {
        ...
        {
            QString defaultFilename = "NewScript";
            QString scriptCanvasExtension = ScriptCanvasEditor::SourceDescription::GetFileExtension();

            // Passing the path through QFileInfo to fix the separators because AZ::IO::Path
            // will remove the last directory in the path otherwise
            QFileInfo fileInfo;
            fileInfo.setFile(QDir(fullSourceFolderNameInCallback), defaultFilename + scriptCanvasExtension);

            AZ::IO::Path fullAzFilePath = fileInfo.absoluteFilePath().toStdString().c_str();
