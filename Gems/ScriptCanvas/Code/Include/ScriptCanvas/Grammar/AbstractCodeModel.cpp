...
namespace ScriptCanvas
{
    namespace Grammar
    {
            ...
            while (outputSource)
            {
                ...
                for (const auto& scNodeAndOutputSlot : scriptCanvasNodesConnectedToInput)
                {
                    ...
                    while (lexicalId != AZ_CRC_CE("NONE"))
                    {
                        ...
                        auto firstSlot = *outputSCNode->GetAllSlots().front();
                            ...
                            if (firstNode == FirstNode::Self || child.m_execution == outputChild)
                            {
                                for (const auto& sourceOutputVarPair : child.m_output)
                                {
                                    ...
                                    if (outputSlot->GetId() == sourceOutputVarPair.second->m_source->m_sourceSlotId)
                                    {
                                        ...
                                        variableData->m_output.push_back(sourceOutputVarPair);
                                        return variableData;
                                    }
                                }
                            }
        ...
        void AbstractCodeModel::ParseInputDatum(ExecutionTreePtr execution, const Slot& input)
        {
            ...
            else
            {
                if (auto sourceVariableOperationData = ParseConnectedInputData(input, execution, nodes, FirstNode::Parent))
                {
                    ...
                    {
                        ParseSmallOperators(execution, sourceVariableOperationData);
                    }
                    execution->AddInput(
                        { &input,
        ...
        void AbstractCodeModel::ParseSmallOperators(ExecutionTreePtr execution, VariableOperatorData* variableData)
        {
            for (AZ::Crc32 operation : variableData->m_smallOperations)
            {
