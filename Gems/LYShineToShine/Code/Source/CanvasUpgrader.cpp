...
namespace LYShineToShine
{
    ...
    AZStd::vector<AZStd::string> CanvasUpgrader::ExtractEntitiesFromSliceComponent(const AZStd::string& sliceComponentXml) const
    {
        AZStd::vector<AZStd::string> entities;

        // Find the Entities vector: field="Entities"
        AZStd::string entitiesBlock = ExtractXmlBlock(sliceComponentXml, "Entities");
        if (entitiesBlock.empty())
        {
            return entities;
        }

        // Extract each <Class name="AZ::Entity" field="element" ...>...</Class> from inside
        size_t searchPos = 0;
        while (true)
        {
            AZStd::string entity = ExtractXmlBlock(entitiesBlock, "element", searchPos);
            if (entity.empty())
            {
                break;
            }

            // Find position and size before potentially moving the string
            auto foundPos = entitiesBlock.find(entity, searchPos);
            size_t entityLen = entity.size();

            // Verify it's an AZ::Entity
            if (entity.find("AZ::Entity") != AZStd::string::npos)
            {
                entities.push_back(AZStd::move(entity));
            }

            // Move search past this entity
            if (foundPos != AZStd::string::npos)
            {
                searchPos = foundPos + entityLen;
            }
            else
            {
                break;
    ...
    AZStd::string CanvasUpgrader::BuildV4Canvas(
        [[maybe_unused]] const AZStd::string& objectStreamVersion,
        ...
        const AZStd::vector<AZStd::string>& prefabInstanceXmlBlocks) const
    {
        ...
        // ObjectStream header — use version 3 for the stream format
        result += "<ObjectStream version=\"3\">\n";

        // UiCanvasFileObject at version 4
        result += "\t<Class name=\"UiCanvasFileObject\" version=\"4\" type=\"{1F02632F-F113-49B1-85AD-8CD0FA78B8AA}\">\n";

        // CanvasEntity — re-indent to 2 tabs
        result += "\t\t";
        result += canvasEntityXml;
        result += "\n";

        // ChildEntities vector
        // The type UUID for AZStd::vector<AZ::Entity*> is {21786AF0-2606-5B9A-86EB-0892E2820E6C}
        if (entityXmlBlocks.empty())
        {
            result += "\t\t<Class name=\"AZStd::vector\" field=\"ChildEntities\" type=\"{21786AF0-2606-5B9A-86EB-0892E2820E6C}\"/>\n";
        }
        else
        {
            result += "\t\t<Class name=\"AZStd::vector\" field=\"ChildEntities\" type=\"{21786AF0-2606-5B9A-86EB-0892E2820E6C}\">\n";
            for (const auto& entityXml : entityXmlBlocks)
            {
                result += "\t\t\t";
                result += entityXml;
                result += "\n";
        ...
        // PrefabInstances vector
        // The type UUID for AZStd::vector<UiPrefabInstance> is determined by reflection
        if (prefabInstanceXmlBlocks.empty())
        {
            result += "\t\t<Class name=\"AZStd::vector\" field=\"PrefabInstances\" type=\"{82B5232B-D439-5A74-8551-078E1BD244E5}\"/>\n";
        }
        else
        {
            result += "\t\t<Class name=\"AZStd::vector\" field=\"PrefabInstances\" type=\"{82B5232B-D439-5A74-8551-078E1BD244E5}\">\n";
            for (const auto& prefabXml : prefabInstanceXmlBlocks)
            {
                result += "\t\t\t";
                result += prefabXml;
                result += "\n";
    ...
    AZStd::string CanvasUpgrader::SerializeEntityToXml(
        AZ::Entity* entity,
        AZ::SerializeContext* serializeContext,
        const char* fieldName) const
    {
        // Serialize entity to XML
        AZStd::vector<AZ::u8> buffer;
        AZ::IO::ByteContainerStream<decltype(buffer)> outStream(&buffer);

        if (!AZ::Utils::SaveObjectToStream(outStream, AZ::ObjectStream::ST_XML, entity, serializeContext))
        {
            return {};
        }
        ...
        // The output is: <ObjectStream version="3">\n\t<Class name="AZ::Entity" ...>...</Class>\n</ObjectStream>\n
        // We need to extract just the <Class ...>...</Class> and inject field="fieldName"
        AZStd::string xmlStr(reinterpret_cast<const char*>(buffer.data()), buffer.size());

        // Find the first <Class after the ObjectStream opening tag
        auto classStart = xmlStr.find("<Class ");
        if (classStart == AZStd::string::npos)
        {
            return {};
        }

        // Find </ObjectStream> at the end
        auto streamEnd = xmlStr.rfind("</ObjectStream>");
        if (streamEnd == AZStd::string::npos)
        {
            return {};
        }

        // Extract the entity XML between ObjectStream tags
        AZStd::string entityXml = xmlStr.substr(classStart, streamEnd - classStart);

        // Trim trailing whitespace
        while (!entityXml.empty() &&
            (entityXml.back() == '\n' || entityXml.back() == '\r' ||
             entityXml.back() == '\t' || entityXml.back() == ' '))
        {
            entityXml.pop_back();
        }

        // Inject field="fieldName" into the opening <Class tag
        // Insert after "<Class " (position 6)
        if (fieldName && fieldName[0] != '\0')
        {
            AZStd::string fieldAttr = AZStd::string::format("field=\"%s\" ", fieldName);
            entityXml.insert(6, fieldAttr); // 6 = strlen("<Class ")
        }
